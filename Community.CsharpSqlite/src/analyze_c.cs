...
*/
        ...
        private static readonly _aTable[] aTable =
        {
...
*/
        ...
        )
        {
            ...
            var jZeroRows = -1; /* Jump from here if number of rows is zero */
            int iDb; /* Index of database containing pTab */
            var regTabname = iMem++; /* Register containing table name */
            var regIdxname = iMem++; /* Register containing index name */
            var regSampleno = iMem++; /* Register containing next sample number */
            var regCol = iMem++; /* Content of a column analyzed table */
            var regRec = iMem++; /* Register holding completed record */
            var regTemp = iMem++; /* Temporary use register */
            var regRowid = iMem++; /* Rowid for the inserted record */
  ...
  int regFirst = iMem++;       /* Index of first sample to record */
            ...
            if (v == null || NEVER(pTab == null)) return;
            if (pTab.tnum == 0)
...
return;
            ...
            for (pIdx = pTab.pIndex; pIdx != null; pIdx = pIdx.pNext)
            {
                ...
                if (iMem + 1 + nCol * 2 > pParse.nMem) pParse.nMem = iMem + 1 + nCol * 2;
...
** store it.  */
        ...
        */
        ...
        sqlite3VdbeAddOp2( v, OP_AddImm, regRecno, 1 );
                ...
                for (i = 0; i < nCol; i++)
                {
                    var addr2 = sqlite3VdbeCurrentAddr(v) - nCol * 2;
                    if (i == 0) sqlite3VdbeJumpHere(v, addr2 - 1); /* Set jump dest for the OP_IfNot */
                ...
                */
                ...
                if (jZeroRows < 0) jZeroRows = sqlite3VdbeAddOp1(v, OP_IfNot, iMem);
                for (i = 0; i < nCol; i++)
                {
...
*/
        private static int analysisLoader(object pData, long argc, object Oargv, object NotUsed)
        {
            ...
            if (argv == null || argv[0] == null || argv[2] == null) return 0;
