...
//# define UPDATE_MAX_BLOBSIZE(P)
        ...
        )
        {
            ...
            */
            ...
            //if ( SQLITE_OK == sqlite3VdbeMemGrow( pMem, nByte, 0 ) )
            {
                ...
                pCx.iDb = iDb;
                pCx.nField = nField;
                if (nField != 0) pCx.aType = new uint[nField]; // (u32)&pMem.z[ROUND8(sizeof( VdbeCursor ))];
                if (isBtreeCursor != 0)
                {
                    pCx.pCursor =
            ...
            if (affinity == SQLITE_AFF_TEXT)
            {
                ...
                */
                ...
                if ((pRec.flags & (MEM_Blob | MEM_Str)) == (MEM_Blob | MEM_Str))
                {
                    ...
                    for (var i = 0; i < pRec.zBLOB.Length; i++)
                        sb.Append((char)pRec.zBLOB[i]);
                    pRec.z = sb.ToString();
                    sqlite3_free(ref pRec.zBLOB);
                    pRec.flags = (ushort)(pRec.flags & ~MEM_Blob);
                }

                pRec.flags = (ushort)(pRec.flags & ~(MEM_Real | MEM_Int));
            }
    ...
    */
        private static readonly StringBuilder zCsr = new StringBuilder(100);
        private static void sqlite3VdbeMemPrettyPrint(Mem pMem, StringBuilder zBuf)
        {
            ...
            if ((f & MEM_Blob) != 0)
            {
                ...
                {
                    sqlite3_snprintf(100, zCsr, "%02X", pMem.zBLOB[i] & 0xFF);
                    zBuf.Append(zCsr); //zCsr += sqlite3Strlen30(zCsr);
                }

                for (i = 0; i < 16 && i < pMem.n; i++)
                {
                    var z = (char)pMem.zBLOB[i];
                    if (z < 32 || z > 126)
                        zBuf.Append('.'); //*zCsr++ = '.';
                    else
            ...
            else if ((f & MEM_Str) != 0)
            {
                ...
                {
                    var c = pMem.z != null ? (byte)pMem.z[j] : pMem.zBLOB[j];
                    if (c >= 0x20 && c < 0x7f)
    ...
    */
        ...
        */
        ...
        )
        {
...
#endif
            var aMem = p.aMem; /* Copy of p.aMem */
            ...
            var lastRowid = db.lastRowid; /* Saved value of the last insert ROWID */
#if VDBE_PROFILE
...
sqlite3VdbeIOTraceSql( p );
                ...
                && (p.db.flags & SQLITE_VdbeListing) != 0)
            {
                ...
                for (i = 0; i < p.nOp; i++) sqlite3VdbePrintOp(Console.Out, i, aOp[i]);
            }
                                ...
                                */
                                case OP_Copy:
                                {
                                    ...
                                    if ((pOut.flags & MEM_Ephem) != 0 && sqlite3VdbeMemMakeWriteable(pOut) != 0)
                                        goto no_mem; //Deephemeralize( pOut );
                                    ...
                                    */
                                    ...
                                    for (i = 0; i < pOp.p2; i++)
                                    {
                                        ...
                                        sqlite3VdbeMemNulTerminate(
                                            p.pResultSet[i]); //sqlite3VdbeMemNulTerminate(pMem[i]);
                                ...
                                */
                                case OP_Concat:
                                {
                                    ...
                                    if ((pIn1.flags & (MEM_Str | MEM_Blob)) == 0 &&
                                        sqlite3VdbeMemStringify(pIn1, encoding) !=
                                        0) goto no_mem; // Stringify(pIn1, encoding);
                                    if ((pIn2.flags & (MEM_Str | MEM_Blob)) == 0 &&
                                        sqlite3VdbeMemStringify(pIn2, encoding) !=
                                        0) goto no_mem; // Stringify(pIn2, encoding);
                                    nByte = pIn1.n + pIn2.n;
                                    if (nByte > db.aLimit[SQLITE_LIMIT_LENGTH]) goto too_big;
                                    ...
                                    if (pIn2.z != null && pIn2.z.Length >= pIn2.n)
                                    {
                                        if (pIn1.z != null)
                                        {
                                            pOut.z = pIn2.z.Substring(0, pIn2.n) + (pIn1.n < pIn1.z.Length
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                var sb = new StringBuilder(pIn1.n);
                                                for (var i = 0; i < pIn1.n; i++)
                                                    sb.Append(pIn1.zBLOB[i]);
                                                pOut.z = pIn2.z.Substring(0, pIn2.n) + sb;
                                            }
                                            else // UTF-8 Blob
                                            {
              ...
              UPDATE_MAX_BLOBSIZE( pOut );
                      ...
                      */
                                case OP_ToText:
                                {
                                    ...
                                    pIn1.flags |= (ushort)((pIn1.flags & MEM_Blob) >> 3);
                                    applyAffinity(pIn1, SQLITE_AFF_TEXT, encoding);
                                    rc = ExpandBlob(pIn1);
                                    Debug.Assert((pIn1.flags & MEM_Str) != 0 /*|| db.mallocFailed != 0 */);
                                    pIn1.flags = (ushort)(pIn1.flags & ~(MEM_Int | MEM_Real | MEM_Blob | MEM_Zero));
#if SQLITE_TEST
                      ...
                      */
                                ...
                                */
                                ...
                                case OP_Ge:
                                {
                                    ...
                                    var res = 0; /* Result of the comparison of pIn1 against pIn3 */
                                    char affinity; /* Affinity to use for comparison */
                                    ushort flags1; /* Copy of initial value of pIn1->flags */
                                    ushort flags3; /* Copy of initial value of pIn3->flags */
                                    pIn1 = aMem[pOp.p1];
                                    pIn3 = aMem[pOp.p3];
                                    flags1 = pIn1.flags;
                                    flags3 = pIn3.flags;
                                    ...
                                    pIn1.flags = (ushort)((pIn1.flags & ~MEM_TypeMask) | (flags1 & MEM_TypeMask));
                                    pIn3.flags = (ushort)((pIn3.flags & ~MEM_TypeMask) | (flags3 & MEM_TypeMask));
                                    break;
                                }
...
c = sqlite3VdbeIntValue(pIn1)!=0;
                                ...
                                */
                                case OP_Column:
                                {
                                    ...
                                    uint[] aType; /* aType[i] holds the numeric type of the i-th column */
                                    uint[] aOffset; /* aOffset[i] is offset to start of data for i-th column */
                                    ...
                                    uint szField = 0; /* Number of bytes in the content of a field */
                                    ...
                                    */
                                    ...
                                    if (pCrsr != null)
                                    {
                                        ...
                                        else if (pC.isIndex)
                                        {
                                            ...
                                            Debug.Assert(rc ==
                                                         SQLITE_OK); /* True because of CursorMoveto() call above */
                                            /* sqlite3BtreeParseCellPtr() uses getVarint32() to extract the
                                            ** payload size, so it is impossible for payloadSize64 to be
                                            ** larger than 32 bits. */
                                            Debug.Assert(((ulong)payloadSize64 & SQLITE_MAX_U32) ==
                                                         (ulong)payloadSize64);
                                    ...
                                    else if (pC.pseudoTableReg > 0)
                                    {
                                        ...
                                        payloadSize = (uint)pReg.n;
                                        zRec = pReg.zBLOB;
                                        pC.cacheStatus = (pOp.p5 & OPFLAG_CLEARCACHE) != 0
                                            ? CACHE_STALE
                                            : p.cacheCtr;
                                    ...
                                    if (payloadSize > (uint)db.aLimit[SQLITE_LIMIT_LENGTH]) goto too_big;
                                    ...
                                    else
                                    {
                                        ...
                                        */
                                        for (i = 0; i < nField; i++)
                                            ...
                                            {
                                                aOffset[i] = offset;
                                                zIdx += getVarint32(zData, zIdx,
                                                    out aType[i]); //getVarint32(zIdx, aType[i]);
                                    ...
                                    */
                                    if (aOffset[p2] != 0)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            rc = sqlite3VdbeMemFromBtree(pCrsr, (int)aOffset[p2], len, pC.isIndex,
                                                sMem);
                                            if (rc != SQLITE_OK) goto op_column_out;
                                            zData = sMem.zBLOB;
                                            sMem.zBLOB = null;
              ...
              UPDATE_MAX_BLOBSIZE( pDest );
                                ...
                                */
                                case OP_MakeRecord:
                                {
                                    ...
                                    ulong nData; /* Number of bytes of data space */
                                    int nHdr; /* Number of bytes of header space */
                                    long nByte; /* Data space required for this record */
                                    int nZero; /* Number of zero bytes at the end of the record */
                                    int nVarint; /* Number of bytes in a varint */
                                    uint serial_type; /* Type field */
                                    ...
                                    for (var pD0 = 0; pD0 < nField; pD0++)
                                    {
                                        ...
                                        if ((pRec.flags & MEM_Zero) != 0)
                                          ...
                                          ** We do not allow blobs with a prefix and a zero-filled tail. */
                                            ...
                                            nHdr += nVarint = sqlite3VarintLen((ulong)nHdr);
                                            if (nVarint < sqlite3VarintLen((ulong)nHdr)) nHdr++;
                                            nByte = (long)((ulong)nHdr + nData - (ulong)nZero);
                                            if (nByte > db.aLimit[SQLITE_LIMIT_LENGTH]) goto too_big;
                                            ...
                                            */
                                            ...
                                            for (var pD0 = 0;
                                                 pD0 < nField;
                                                 pD0++) //for (pRec = pData0; pRec <= pLast; pRec++)
                                            {
                                                pRec = p.aMem[pOp.p1 + pD0];
                                                serial_type = sqlite3VdbeSerialType(pRec, file_format);
                                                 ...
                                                 pD0++) //for (pRec = pData0; pRec <= pLast; pRec++)
                                            {
                                                /* serial data */
                                                pRec = p.aMem[pOp.p1 + pD0];
                                                i += (int)sqlite3VdbeSerialPut(zNewRecord, i, (int)nByte - i, pRec,
                                                    file_format);
                                            }
                                  ...
                                  */
                                            case OP_Savepoint:
                                            {
                                                ...
                                                */
                                                ...
                                                Debug.Assert(p1 == SAVEPOINT_BEGIN || p1 == SAVEPOINT_RELEASE ||
                                            ...
                                            ** that the db->aVTrans[] array is empty.  */
                                                        ...
                                                        pNew =
                                                            new Savepoint(); // sqlite3DbMallocRaw( db, sizeof( Savepoint ) + nName + 1 );
                                                        if (pNew != null)
                                                        {
                                                ...
                                                else
                                                {
                                                    ...
                                                    ** an error is returned to the user.  */
                                                    for (
                                                        ...
                                                        pSavepoint != null && !pSavepoint.zName.Equals(zName,
                                                    ...
                                                    if (null == pSavepoint)
                                                    {
                                                        sqlite3SetString(ref p.zErrMsg, db, "no such savepoint: %s",
                                                            zName);
                                                        rc = SQLITE_ERROR;
                                                    }
                                                    else if (
                                                        db.writeVdbeCnt > 0 || (p1 == SAVEPOINT_ROLLBACK &&
                                                            db.activeVdbeCnt > 1)
                                                    )
                                                    {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        ** is committed.
                                                        */
                                                        var isTransaction = pSavepoint.pNext == null &&
                                                        ...
                                                        else
                                                        {
                                                            ...
                                                            {
                                                                rc = sqlite3BtreeSavepoint(db.aDb[ii].pBt, p1,
                                                                    iSavepoint);
                                                                if (rc != SQLITE_OK) goto abort_due_to_error;
                                                            }

                                                            if (p1 == SAVEPOINT_ROLLBACK &&
                                                                (db.flags & SQLITE_InternChanges) != 0)
                                                            {
                                            ...
                                            */
                                            case OP_AutoCommit:
                                            {
                                                ...
                                                else if (desiredAutoCommit != db.autoCommit)
                                                {
                                                    ...
                                                    {
                                                        db.autoCommit = (byte)desiredAutoCommit;
                                                        if (sqlite3VdbeHalt(p) == SQLITE_BUSY)
                                                        {
                                                            p.pc = pc;
                                                            db.autoCommit = (byte)(desiredAutoCommit == 0 ? 1 : 0);
                                                            p.rc = rc = SQLITE_BUSY;
                                                            goto vdbe_return;
                                                        }
                                                    }
                                            ...
                                            */
                                            case OP_SetCookie:
                                            {
                                                ...
                                                rc = sqlite3BtreeUpdateMeta(pDb.pBt, pOp.p2, (uint)pIn3.u.i);
                                                ...
                                                else if (pOp.p2 == BTREE_FILE_FORMAT)
                                                {
                                                    ...
                                                    pDb.pSchema.file_format = (byte)pIn3.u.i;
                                                }
                                                ...
                                                if (rc == SQLITE_OK)
                                                {
                                                    ...
                                                    */
                                                    if (pOp.p4.pKeyInfo != null)
                                                    {
                                                        ...
                                                        rc = sqlite3BtreeCreateTable(pCx.pBt, ref pgno,
                                                            BTREE_BLOBKEY);
                                                        if (rc == SQLITE_OK)
                                                        {
                                                    ...
                                                    {
                                                        rc = sqlite3BtreeCursor(pCx.pBt, MASTER_ROOT, 1, null,
                                                            pCx.pCursor);
                                                        pCx.isTable = true;
                                                    }
                                                }
                                            ...
                                            */
                                            ...
                                            case OP_SeekGt:
                                            {
                                                ...
                                                UnpackedRecord r;
                                                int nField;
                                                long iKey; /* The rowid we are to seek to */
                                                ...
                                                if (pC.pCursor != null)
                                                {
                                                    ...
                                                    if (pC.isTable)
                                                    {
                                                        ...
                                                        ** loss of information, then special processing is required... */
                                                        ...
                                                        rc = sqlite3BtreeMovetoUnpacked(pC.pCursor, null, iKey, 0,
                                                            ref res);
                                                        if (rc != SQLITE_OK) goto abort_due_to_error;
                                                        if (res == 0)
                                                        {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        */
                                                        r.flags =
                                                            (ushort)(UNPACKED_INCRKEY * (1 & (oc - OP_SeekLt)));
                                                        Debug.Assert(oc != OP_SeekGt ||
                                                                     r.flags == UNPACKED_INCRKEY);
                                                        ...
                                                        for (var rI = 0; rI < r.nField; rI++)
                                                            r.aMem[rI] =
                                                                aMem[pOp.p3 + rI]; // r.aMem = aMem[pOp.p3];
#if SQLITE_DEBUG
                                                        {
                                                            int i;
...
#endif
                                                        ExpandBlob(r.aMem[0]);
                                                        rc = sqlite3BtreeMovetoUnpacked(pC.pCursor, r, 0, 0,
                                                            ref res);
                                                        if (rc != SQLITE_OK) goto abort_due_to_error;
                                                        pC.rowidIsValid = false;
                                                    }
              ...
              sqlite3_found_count.iValue++;
                                                ...
                                                if (ALWAYS(pC.pCursor != null))
                                                {
                                                    ...
                                                    {
                                                        r.pKeyInfo = pC.pKeyInfo;
                                                        r.nField = (ushort)pOp.p4.i;
                                                        r.aMem = new Mem[r.nField];
                                                        for (var i = 0; i < r.aMem.Length; i++)
                                                        {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        Debug.Assert((pIn3.flags & MEM_Zero) ==
                                                                     0); /* zeroblobs already expanded */
                                                        pIdxKey = sqlite3VdbeRecordUnpack(pC.pKeyInfo, pIn3.n,
                                                            pIn3.zBLOB,
                                                            aTempRec, 0); //sizeof( aTempRec ) );
                                                        if (pIdxKey == null) goto no_mem;
                                                        pIdxKey.flags |= UNPACKED_PREFIX_MATCH;
                                                    }

                                                    rc = sqlite3BtreeMovetoUnpacked(pC.pCursor, pIdxKey, 0, 0,
                                                        ref res);
                                                    if (pOp.p4.i == 0) sqlite3VdbeDeleteUnpackedRecord(pIdxKey);
                                            ...
                                            */
                                            case OP_IsUnique:
                                            {
                                                ...
                                                long R; /* Rowid stored in register P3 */
                                                ...
                                                if (pCrsr != null)
                                                {
                                                    ...
                                                    ** register P3 and fall through to the next instruction.  */
                                                    rc = sqlite3BtreeMovetoUnpacked(pCrsr, r, 0, 0,
                                                        ref pCx.seekResult);
...
const int MAX_ROWID = i32.MaxValue;//#   define MAX_ROWID 0x7fffffff
                                    ...
                                    */
                                                    const long
                                                        MAX_ROWID = long
                                                            .MaxValue; // (i64)( (((u64)0x7fffffff)<<32) | (u64)0xffffffff )
#endif
                                                    ...
                                                    if (!pC.useRandomRowid)
                                                    {
                                                        ...
                                                        if (v == 0)
                                                        {
                                                            ...
                                                            else
                                                            {
                                                                ...
                                                                Debug.Assert(rc ==
                                                                             SQLITE_OK); /* Cannot fail following BtreeLast() */
                                                                if (v == MAX_ROWID)
                                                        ...
                                                        if (pOp.p3 != 0)
                                                        {
                                                            ...
                                                            Debug.Assert((pMem.flags & MEM_Int) !=
                                                        ...
                                                        sqlite3BtreeSetCachedRowid(pC.pCursor,
                                                            v < MAX_ROWID ? v + 1 : 0);
                                                    }
...
** an AUTOINCREMENT table. */
                                                               ...
                                                               && ++cnt < 100)
                                                        {
                                                            ...
                                                            sqlite3_randomness(sizeof(long), ref v);
                                                            if (cnt < 5)
                                                                /* try "small" random rowids for the initial attempts */
                                                                v &= 0xffffff;
                                                            else
                                                                v &= MAX_ROWID >>
                                                                     1; /* ensure doesn't go negative */
                                                            v++; /* ensure non-zero */
                                                        }
                                            ...
                                            */
                                            ...
                                            case OP_InsertInt:
                                            {
                                                ...
                                                int
                                                    seekResult; /* Result of prior seek or 0 if no USESEEKRESULT flag */
                                                ...
                                                seekResult = (pOp.p5 & OPFLAG_USESEEKRESULT) != 0
                                                    ? pC.seekResult
                                                    : 0;
                                                ...
                                                if (rc == SQLITE_OK && db.xUpdateCallback != null &&
                                                    pOp.p4.z != null)
                                                {
                                                    zDb = db.aDb[pC.iDb].zName;
                                                    zTbl = pOp.p4.z;
                                                    op = (pOp.p5 & OPFLAG_ISUPDATE) != 0
                                                        ? SQLITE_UPDATE
                                                        : SQLITE_INSERT;
                                                    Debug.Assert(pC.isTable);
                                            ...
                                            */
                                            case OP_Delete:
                                            {
                                                ...
                                                pC = p.apCsr[pOp.p1];
                                                Debug.Assert(pC != null);
                                                Debug.Assert(pC.pCursor !=
                                                             null); /* Only valid for real tables, no pseudotables */
                                                ...
                                                */
                                                if (db.xUpdateCallback != null && pOp.p4.z != null)
                                                {
                                                    Debug.Assert(pC.isTable);
                                                    Debug.Assert(pC
                                                        .rowidIsValid); /* lastRowid set by previous OP_NotFound */
                                                    iKey = pC.lastRowid;
                                                }
                                                ...
                                                **/
                                                    ...
                                                    pOp.p4.z != null)
                                                {
                                                    ...
                                                    var zTbl = pOp.p4.z;
                                                    db.xUpdateCallback(db.pUpdateArg, SQLITE_DELETE, zDb, zTbl,
                                                        iKey);
                                                    Debug.Assert(pC.iDb >= 0);
                                                }
                                            ...
                                            case OP_RowData:
                                            {
                                                ...
                                                */
                                                ...
                                                if (pC.isIndex)
                                                {
                                                    ...
                                                    Debug.Assert(rc ==
                                                                 SQLITE_OK); /* True because of CursorMoveto() call above */
                                                    if (n64 > db.aLimit[SQLITE_LIMIT_LENGTH]) goto too_big;
                                                    n = (uint)n64;
                                                }
                                                else
                                                {
                                                    ...
                                                    Debug.Assert(rc == SQLITE_OK); /* DataSize() cannot fail */
                                                    if (n > (uint)db.aLimit[SQLITE_LIMIT_LENGTH]) goto too_big;
                                                    if (sqlite3VdbeMemGrow(pOut, (int)n, 0) != 0) goto no_mem;
                                                }
                                            ...
                                            case OP_Rowid:
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    else
                                                    {
                                                        rc = sqlite3BtreeKeySize(pC.pCursor, ref v);
                                                        Debug.Assert(rc ==
                                                                     SQLITE_OK); /* Always so because of CursorMoveto() above */
                                                    }
                                                }
                ...
                sqlite3_search_count.iValue++;
                                            ...
                                            */
                                            case OP_IdxInsert:
                                            {
                                                ...
                                                if (ALWAYS(pCrsr != null))
                                                {
                                                    ...
                                                    {
                                                        nKey = pIn2.n;
                                                        zKey = (pIn2.flags & MEM_Blob) != 0
                                                            ? pIn2.zBLOB
                                                            : Encoding.UTF8.GetBytes(pIn2.z);
                                                        rc = sqlite3BtreeInsert(pCrsr, zKey, nKey, null, 0, 0,
                                                            pOp.p3 != 0 ? 1 : 0,
                                            ...
                                            */
                                            case OP_IdxDelete:
                                            {
                                                ...
                                                {
                                                    r.pKeyInfo = pC.pKeyInfo;
                                                    r.nField = (ushort)pOp.p3;
                                                    r.flags = 0;
                                                    r.aMem = new Mem[r.nField];
                                                    for (var ra = 0; ra < r.nField; ra++)
                                                    {
              ...
              iCnt = db.activeVdbeCnt;
                                                ...
                                                else
                                                {
                                                    ...
                                                    if (rc == SQLITE_OK && iMoved != 0)
                                                    {
                                                        ...
                                                        Debug.Assert(resetSchemaOnFault == 0 ||
                                                                     resetSchemaOnFault == iDb + 1);
                                                        resetSchemaOnFault = (byte)(iDb + 1);
                                                    }
#endif
                                                }
                                                ...
                                                */
                                                ...
                                                /* Used to be a conditional */
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        rc = sqlite3_exec(db, zSql, sqlite3InitCallback, initData,
                                                            0);
                                                        if (rc == SQLITE_OK)
                                                            rc = initData.rc;
                                  ...
                                  */
                                            case OP_IntegrityCk:
                                            {
                                                ...
                                                int[] aRoot =
                                                    null; /* Array of rootpage numbers for tables to be checked */
                                                int j; /* Loop counter */
                                                ...
                                                aRoot = sqlite3Malloc(aRoot,
                                                    nRoot +
                                                    1); // sqlite3DbMallocRaw(db, sizeof(int) * (nRoot + 1));
                                                if (aRoot == null)
                                                    goto no_mem;
                                                Debug.Assert(pOp.p3 > 0 && pOp.p3 <= p.nMem);
                                                pnErr = aMem[pOp.p3];
                                                ...
                                                pIn1 = aMem[pOp.p1];
                                                for (j = 0; j < nRoot; j++)
                                                    aRoot[j] = (int)sqlite3VdbeIntValue(
                                                        p.aMem[pOp.p1 + j]); // pIn1[j]);
                                                aRoot[j] = 0;
              ...
              UPDATE_MAX_BLOBSIZE( pIn1 );
                                            ...
                                            */
                                            case OP_RowSetTest:
                                            {
                                                ...
                                                */
                                                ...
                                                if (iSet >= 0) sqlite3RowSetInsert(pIn1.u.pRowSet, pIn3.u.i);
                                                break;
                                            }
                                                ...
                                                ** is already allocated. Otherwise, it must be initialized.  */
                                                if ((pRt.flags & MEM_Frame) == 0)
                                                {
                                                    ...
                                                    */
                                                    ...
                                                    for (var i = 0; i < pFrame.nChildCsr; i++)
                                                ...
                                                p.nCursor = (ushort)pFrame.nChildCsr;
                                  ...
                                  */
                                            case OP_MemMax:
                                            {
                                                ...
                                                pIn2 = aMem[pOp.p2];
                                                sqlite3VdbeMemIntegerify(pIn2);
                                                if (_pIn1.u.i < pIn2.u.i) _pIn1.u.i = pIn2.u.i;
                                                break;
                                            }
                                            ...
                                            */
                                            case OP_AggStep:
                                            {
                                                ...
                                                if ((ctx.pFunc.flags & SQLITE_FUNC_NEEDCOLL) != 0)
                                                {
                                                    ...
                                                    Debug.Assert(p.aOp[pc - 1].p4type ==
                                                                 P4_COLLSEQ); //pOp[-1].p4type == P4_COLLSEQ );
                                                    Debug.Assert(p.aOp[pc - 1].opcode ==
                                                                 OP_CollSeq); // pOp[-1].opcode == OP_CollSeq );
...
#endif
                                                if (sqlite3VdbeMemTooBig(pMem)) goto too_big;
                                                break;
                                            }
...
break;
                                  ...
                                  */
                                            case OP_VFilter:
                                            {
                                                ...
                                                Debug.Assert(
                                                    (pQuery.flags & MEM_Int) != 0 && pArgc.flags == MEM_Int);
                                                nArg = (int)pArgc.u.i;
                                                ...
                                                /* Invoke the xFilter method */
                                                {
                                                    ...
                                                    {
                                                        apArg[i] = aMem[
                                                            pOp.p3 + 1 + i + 1]; //apArg[i] = pArgc[i + 1];
                                                        sqlite3VdbeMemStoreType(apArg[i]);
                                                    }

                                                    p.inVtabMethod = 1;
                                                    rc = pModule.xFilter(pVtabCursor, iQuery, pOp.p4.z, nArg,
                                                        apArg);
                                                    p.inVtabMethod = 0;
                                                    importVtabErrMsg(p, pVtab);
                                                    if (rc == SQLITE_OK) res = pModule.xEof(pVtabCursor);
                                  ...
                                  */
                                            case OP_VColumn:
                                            {
                                                ...
                                                sContext =
                                                    new sqlite3_context(); //memset( &sContext, 0, sizeof( sContext ) );
                                                ...
                                                */
                                                ...
                                                if (sContext.isError != 0) rc = sContext.isError;
                                                ...
                                                */
                                                ...
                                                REGISTER_TRACE(p, pOp.p3, pDest);
                                                UPDATE_MAX_BLOBSIZE(pDest);
                                                if (sqlite3VdbeMemTooBig(pDest)) goto too_big;
                                                break;
                                            }
#endif //* SQLITE_OMIT_VIRTUALTABLE */
                                  ...
                                  */
                                            case OP_VNext:
                                            {
                                                ...
                                                */
                                                ...
                                                p.inVtabMethod = 0;
                                                importVtabErrMsg(p, pVtab);
                                                if (rc == SQLITE_OK) res = pModule.xEof(pCur.pVtabCursor);
                                  ...
                                  */
                                            case OP_VUpdate:
                                            {
                                                ...
                                                Debug.Assert(pOp.p2 == 1 || pOp.p5 == OE_Fail ||
                                                             pOp.p5 == OE_Rollback
                                                             || pOp.p5 == OE_Abort || pOp.p5 == OE_Ignore ||
                                                             pOp.p5 == OE_Replace
                                                );
                                                pVtab = pOp.p4.pVtab.pVtab;
                                                pModule = pVtab.pModule;
                                                ...
                                                {
                                                    var vtabOnConflict = db.vtabOnConflict;
                                                    apArg = p.apArg;
                                                    //pX = aMem[pOp.p3];
                                                    for (i = 0; i < nArg; i++)
                                                    {
                                                    ...
                                                    {
                                                        Debug.Assert(nArg > 1 && apArg[0] != null &&
                                                                     (apArg[0].flags & MEM_Null) != 0);
                                                    ...
                                                    if (rc == SQLITE_CONSTRAINT && pOp.p4.pVtab.bConstraint != 0)
                                                    {
                                                        if (pOp.p5 == OE_Ignore)
                                                            ...
                                                            p.errorAction = (byte)(pOp.p5 == OE_Replace
                                                                ? OE_Abort
                                                                : pOp.p5);
                                                    }
                                                    else
                                  ...
                                  */
                                            case OP_MaxPgcnt:
                                            {
                                                ...
                                                pOut.u.i = sqlite3BtreeMaxPageCount(pBt, (int)newMax);
                                                break;
                                            }
#endif
