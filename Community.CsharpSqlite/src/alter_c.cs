...
*/
        ...
        )
        {
            ...
            var zOutput = "";
            string zResult;
            var zInput = sqlite3_value_text(argv[0]);
            var zOld = sqlite3_value_text(argv[1]);
            var zNew = sqlite3_value_text(argv[2]);

            int zIdx; /* Pointer to token */
            var zLeft = 0; /* Pointer to remainder of String */
            var n = 0; /* Length of token z */
            var token = 0; /* Type of token */
            ...
            for (zIdx = 0; zIdx < zInput.Length; zIdx += n) //z=zInput; *z; z=z+n)
            {
                ...
                if (token == TK_REFERENCES)
                {
                    ...
                    {
                        var zOut = sqlite3MPrintf(db, "%s%.*s\"%w\"",
...
*/
  ...
  FuncDef[] aFunc = GLOBAL(FuncDef, aAlterTableFuncs);
            ...
            var aFunc = aAlterTableFuncs;
#endif

            for (i = 0; i < ArraySize(aAlterTableFuncs); i++) sqlite3FuncDefInsert(pHash, aFunc[i]);
        }
...
*/
        private static string whereTempTriggers(Parse pParse, Table pTab)
        {
            ...
            */
            ...
            if (!string.IsNullOrEmpty(zWhere)) zWhere = sqlite3MPrintf(pParse.db, "type='trigger' AND (%s)", zWhere);
...
*/
        private static void reloadTableSchema(Parse pParse, Table pTab, string zName)
        {
          ...
          */
            if ((zWhere = whereTempTriggers(pParse, pTab)) != "") sqlite3VdbeAddParseSchemaOp(v, 1, zWhere);
#endif
        }
...
*/
        ...
        )
        {
            ...
            */
            if (SQLITE_OK != isSystemTable(pParse, pTab.zName)) goto exit_rename_table;
...
goto exit_rename_table;
            ...
            if (IsVirtual(pTab))
            {
                ...
                if (pVTab.pVtab.pModule.xRename == null) pVTab = null;
            }
#endif
 ...
 "sql = sqlite_rename_table(sql, %Q), " +
          ...
          */
            ...
            if ((db.flags & SQLITE_ForeignKeys) != 0)
            {
                ...
                {
                    var pFrom = p.pFrom;
                    if (pFrom != pTab) reloadTableSchema(pParse, p.pFrom, pFrom.zName);
                }
            }
#endif
...
*/
        private static void sqlite3AlterBeginAddColumn(Parse pParse, SrcList pSrc)
        {
            ...
            if (SQLITE_OK != isSystemTable(pParse, pTab.zName)) goto exit_begin_add_column;
