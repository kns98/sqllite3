...
namespace Community.CsharpSqlite
{
    ...
    */
        public class sqlite3_mutex
        {
            ...
            public sqlite3_mutex(Mutex mutex, int id, int nRef, int owner
#if SQLITE_DEBUG
                , int trace
#endif
            )
        ...
        private static readonly Mutex SQLITE_W32_MUTEX_INITIALIZER = null;
#if SQLITE_DEBUG
...
return osType==2;
    ...
    */
        ...
        private static readonly sqlite3_mutex[] winMutex_staticMutexes =
        {
            new sqlite3_mutex(SQLITE_W32_MUTEX_INITIALIZER, 0, 0, 0
#if SQLITE_DEBUG
                , 0
#endif
        ...
        private static int winMutexInit()
        {
                ...
                //if ( Interlocked.CompareExchange(ref winMutex_lock, 1, 0 ) == 0 )
            {
                ...
                for (i = 0; i < ArraySize(winMutex_staticMutexes); i++)
                    if (winMutex_staticMutexes[i].mutex == null)
                        winMutex_staticMutexes[i].mutex = new Mutex();
        ...
        */
        private static void winMutexEnter(sqlite3_mutex p)
        {
            ...
            p.nRef++;
#if SQLITE_DEBUG
            if (p.trace != 0) printf("enter mutex {0} ({1}) with nRef={2}\n", p.GetHashCode(), p.owner, p.nRef);
#endif
        }
...
#if SQLITE_DEBUG
            if (p.trace != 0) printf("leave mutex {0} ({1}) with nRef={2}\n", p.GetHashCode(), p.owner, p.nRef);
#endif
        }

        private static sqlite3_mutex_methods sqlite3DefaultMutex()
        {
            var sMutex = new sqlite3_mutex_methods(
                winMutexInit,
                winMutexEnd,
                winMutexAlloc,
                winMutexFree,
