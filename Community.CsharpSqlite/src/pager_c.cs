...
return (pPager->pWal!=0);
        ...
        private static int pagerWalFrames(Pager v, PgHdr w, uint x, int y, int z)
        {
            return 0;
        }
...
0==MEMDB
 ...
 0 == pPager.memDb
    ...
    */
        ...
        {
            var zRet = new StringBuilder(1024);
    ...
    */
        ...
        */
        ...
        private static int read32bits(sqlite3_file fd, long offset, ref uint pRes)
        {
            var rc = read32bits(fd, (int)offset, ref pRes);
            return rc;
        }

        private static int read32bits(sqlite3_file fd, int offset, ref uint pRes)
        {
            var ac = new byte[4];
        ...
        */
        ...
        {
            sqlite3Put4byte(ac, offset, (uint)val);
        }

        private static void put32bits(byte[] ac, uint val)
        {
            sqlite3Put4byte(ac, 0U, val);
        }

        private static void put32bits(byte[] ac, int offset, uint val)
        {
        ...
        */
        private static int write32bits(sqlite3_file fd, long offset, uint val)
        {
            var ac = new byte[4];
            put32bits(ac, val);
        ...
        */
        private static int pagerUnlockDb(Pager pPager, int eLock)
        {
            ...
            if (isOpen(pPager.fd))
            {
                ...
                if (pPager.eLock != UNKNOWN_LOCK) pPager.eLock = (byte)eLock;
                IOTRACE("UNLOCK %p %d\n", pPager, eLock);
            }
...
assert( (pPg->flags&PGHDR_DIRTY) || pPg->pageHash==pager_pagehash(pPg) );
    ...
    */
        private static int readMasterJournal(sqlite3_file pJrnl, byte[] zMaster, uint nMaster)
        {
            int rc; /* Return code */
        ...
        private static long journalHdrOffset(Pager pPager)
        {
            ...
            if (c != 0)
                offset = (int)(((c - 1) / pPager.sectorSize + 1) *
                               pPager
                                   .sectorSize); //offset = ((c-1)/JOURNAL_HDR_SZ(pPager) + 1) * JOURNAL_HDR_SZ(pPager);
            ...
            if (pPager.journalOff != 0)
            {
                ...
                */
                if (rc == SQLITE_OK && iLimit > 0)
                {
                    ...
                    if (rc == SQLITE_OK && sz > iLimit) rc = sqlite3OsTruncate(pPager.jfd, iLimit);
                }
            }
        ...
        */
        private static int writeJournalHdr(Pager pPager)
        {
            ...
            uint nWrite; /* Bytes of header sector written */
            int ii; /* Loop counter */
            ...
            */
            ...
            {
                Array.Clear(zHeader, 0,
                    aJournalMagic.Length + 4); //memset(zHeader, 0, sizeof(aJournalMagic)+4);
            }

            /* The random check-hash initialiser */
            long i64Temp = 0;
            sqlite3_randomness(sizeof(long), ref i64Temp);
            pPager.cksumInit = (uint)i64Temp;
        ...
        */
        private static int readJournalHdr(
            ...
            int isHot,
            long journalSize, /* Size of the open journal file in bytes */
            out uint pNRec, /* OUT: Value read from the nRec field */
            out uint pDbSize /* OUT: Value of original database size field */
        )
        {
            int rc; /* Return code */
            var aMagic = new byte[8]; /* A buffer to hold the magic header */
            long iHdrOff; /* Offset of journal header being read */
            ...
            */
            ...
            if (pPager.journalOff == 0)
            {
                ...
                */
                if (iPageSize == 0) iPageSize = (uint)pPager.pageSize;
                  ...
                  */
                    ...
                    */
                    private static int writeMasterJournal(Pager pPager, string zMaster)
                    {
                        ...
                        long iHdrOff; /* Offset of header in journal file */
                        ...
                        */
                        if (0 != (rc = write32bits(pPager.jfd, iHdrOff, PAGER_MJ_PGNO(pPager)))
                            || 0 != (rc = sqlite3OsWrite(pPager.jfd, Encoding.UTF8.GetBytes(zMaster), nMaster,
                                iHdrOff + 4))
                            || 0 != (rc = write32bits(pPager.jfd, iHdrOff + 4 + nMaster, (uint)nMaster))
                    ...
                    ** already in memory.
                    */
                    private static PgHdr pager_lookup(Pager pPager, uint pgno)
                    {
                    ...
                    */
                    private static int addToSavepointBitvecs(Pager pPager, uint pgno)
                    {
                        int ii; /* Loop counter */
                    ...
                    */
                    private static void pager_unlock(Pager pPager)
                    {
                        ...
                        else if (!pPager.exclusiveMode)
                        {
                            ...
                            var iDc = isOpen(pPager.fd) ? sqlite3OsDeviceCharacteristics(pPager.fd) : 0;
                            ...
                            */
                            ...
                            if (rc != SQLITE_OK && pPager.eState == PAGER_ERROR) pPager.eLock = UNKNOWN_LOCK;
...
sqlite3PagerUnref(p);
                    ...
                    */
                    private static uint pager_cksum(Pager pPager, byte[] aData)
                    {
...
static void pagerReportSize(Pager X){}
                ...
                */
                    private static int pager_playback_one_page(
                        ...
                        ref long pOffset, /* Offset of record to playback */
                    ...
                    )
                    {
                        ...
                        uint pgno = 0; /* The page number of a page in journal */
...
0==MEMDB
                        ...
                        if (pPg != null)
                        {
                            ...
                            */
                            var pData = pPg.pData;
                            Buffer.BlockCopy(aData, 0, pData, 0,
                                pPager.pageSize); // memcpy(pData, (u8[])aData, pPager.pageSize);
                    ...
                    */
                    private static int pager_truncate(Pager pPager, uint nPage)
                    {
                        var rc = SQLITE_OK;
                        Debug.Assert(pPager.eState != PAGER_ERROR);
                        Debug.Assert(pPager.eState != PAGER_READER);
                    ...
                    */
                    private static void setSectorSize(Pager pPager)
                    {
                        ...
                        if (!pPager.tempFile)
                          ...
                          ** call will segfault.
                          */
                                pPager.sectorSize = (uint)sqlite3OsSectorSize(pPager.fd);
                            ...
                            */
                            private static int pager_playback(Pager pPager, int isHot)
                            {
                                ...
                                uint u; /* Unsigned loop counter */
                                uint mxPg = 0; /* Size of the original file in pages */
                                ...
                                */
                                ...
                                if (rc == SQLITE_OK && zMaster[0] != 0)
                                    rc = sqlite3OsAccess(pVfs,
                                        Encoding.UTF8.GetString(zMaster, 0, zMaster.Length),
                                        SQLITE_ACCESS_EXISTS, ref res);
                                zMaster = null;
...
/*NOTREACHED*/
                                ...
                                */
                                ...
                                Debug.Assert(
                                    ...
                                    sqlite3OsFileControl(pPager.fd, SQLITE_FCNTL_DB_UNCHANGED, ref iDummy) >=
                                    SQLITE_OK
                                );
...
0 == MEMDB
                                ...
                                if (NEVER(!isOpen(pPager.fd)))
                                {
                                    ...
                                    Array.Clear(pPg.pData, 0,
                                        pPager.pageSize); // memset(pPg.pData, 0, pPager.pageSize);
                                    return SQLITE_OK;
                                }
                                ...
                                {
                                    var iOffset = (pgno - 1) * pPager.pageSize;
                                ...
                                if (pgno == 1)
                                {
                                    if (rc != 0)
                                      ...
                                      */
                                            for (var i = 0;
                                                 i < pPager.dbFileVers.Length;
                                                 pPager.dbFileVers[i++] = 0xff)
                                                ; // memset(pPager.dbFileVers, 0xff, sizeof(pPager.dbFileVers));
                                        else
                                            //u8[] dbFileVers = pPg.pData[24];
                                            Buffer.BlockCopy(pPg.pData, 24, pPager.dbFileVers, 0,
                                                pPager.dbFileVers
                                                    .Length); //memcpy(&pPager.dbFileVers, dbFileVers, sizeof(pPager.dbFileVers));
                                        }

                                        if (CODEC1(pPager, pPg.pData, pgno, SQLITE_DECRYPT))
                                            rc =
                                                SQLITE_NOMEM; //CODEC1(pPager, pPg.pData, pgno, 3, rc = SQLITE_NOMEM);
...
return rc;
                                        ...
                                        */
                                        private static int pagerPagecount(Pager pPager, ref uint pnPage)
                                        {
                                            uint nPage; /* Value to return via *pnPage */
                                            ...
                                            */
                                            if (nPage == 0)
                                            {
                                                long n = 0; /* Size of db file in bytes */
                                                Debug.Assert(isOpen(pPager.fd) || pPager.tempFile);
                                                if (isOpen(pPager.fd))
                                                {
                                                ...
                                                nPage = (uint)(n / pPager.pageSize);
                                                if (nPage == 0 && n > 0) nPage = 1;
                                            }
                                            ...
                                            */
                                            if (nPage > pPager.mxPgno) pPager.mxPgno = nPage;
...
return rc;
                                    ...
                                    */
                                        private static int pagerPlaybackSavepoint(Pager pPager,
                                            PagerSavepoint pSavepoint)
                                        {
                                            ...
                                            var rc = SQLITE_OK; /* Return code */
                                            Bitvec pDone =
                                                null; /* Bitvec to ensure pages played back only once */
                                            ...
                                            */
                                            if (pSavepoint != null && !pagerUseWal(pPager))
                                            {
                                                ...
                                                while (rc == SQLITE_OK && pPager.journalOff < iHdrOff)
                                                    rc = pager_playback_one_page(pPager, ref pPager.journalOff,
                                                        pDone, 1, 1);
                                            ...
                                            */
                                            while (rc == SQLITE_OK && pPager.journalOff < szJ)
                                            {
                                                ...
                                                */
                                                if (nJRec == 0
                                                    && pPager.journalHdr + JOURNAL_HDR_SZ(pPager) >=
                                                    pPager.journalOff
                                                   )
                                                    nJRec = (uint)((szJ - pPager.journalOff) /
                                                                   JOURNAL_PG_SZ(pPager));
                                                for (ii = 0;
                                                     rc == SQLITE_OK && ii < nJRec && pPager.journalOff < szJ;
                                                     ii++)
                                                    rc = pager_playback_one_page(pPager, ref pPager.journalOff,
                                                        pDone, 1, 1);
                                                Debug.Assert(rc != SQLITE_DONE);
                                            ...
                                            */
                                            if (pSavepoint != null)
                                            {
                                                ...
                                                if (pagerUseWal(pPager))
                                                    rc = sqlite3WalSavepointUndo(pPager.pWal,
                                                        pSavepoint.aWalData);
                                                for (ii = pSavepoint.iSubRec;
                                                     rc == SQLITE_OK && ii < pPager.nSubRec;
                                                     ii++)
                                                {
                                                    Debug.Assert(offset == ii * (4 + pPager.pageSize));
                                                    rc = pager_playback_one_page(pPager, ref offset, pDone, 0,
                                                        1);
                                                }
...
*/
 ...
 new tcl.lang.Var.SQLITE3_GETSET( "sqlite3_opentemp_count" );
      ...
      sqlite3_opentemp_count.iValue++;  /* Used for testing and analysis only */
                                        ...
                                        */
                                        private static int sqlite3PagerSetPagesize(Pager pPager,
                                            ref uint pPageSize, int nReserve)
                                        {
                                            var rc = SQLITE_OK;
                                            ...
                                            */
                                            ...
                                            Debug.Assert(pageSize == 0 ||
                                                         (pageSize >= 512 && pageSize <= SQLITE_MAX_PAGE_SIZE));
                                            if ((pPager.memDb == 0 || pPager.dbSize == 0)
                                                && sqlite3PcacheRefCount(pPager.pPCache) == 0
                                                && pageSize != 0 && pageSize != (uint)pPager.pageSize
                                               )
                                            {
                                                ...
                                                {
                                                    pager_reset(pPager);
                                                    pPager.dbSize = (uint)(nByte / pageSize);
                                        ...
                                        */
                                        private static uint sqlite3PagerMaxPageCount(Pager pPager, int mxPage)
                                        {
                                            if (mxPage > 0) pPager.mxPgno = (uint)mxPage;
                                            Debug.Assert(pPager.eState !=
                                                         PAGER_OPEN); /* Called only by OP_MaxPgcnt */
                                            Debug.Assert(pPager.mxPgno >=
                                                         pPager.dbSize); /* OP_MaxPgcnt enforces this */
...
//# define enable_simulated_io_errors()
                                    ...
                                    */
                                        private static int sqlite3PagerReadFileheader(Pager pPager, int N,
                                            byte[] pDest)
                                        {
                                            var rc = SQLITE_OK;
                                        ...
                                        */
                                        private static void sqlite3PagerPagecount(Pager pPager, out uint pnPage)
                                        {
                                            Debug.Assert(pPager.eState >= PAGER_READER);
                                        ...
                                        */
                                        ...
                                        private static void assertTruncateConstraintCb(PgHdr pPg)
                                        {
                                            ...
                                            Debug.Assert(
                                                !subjRequiresPage(pPg) || pPg.pgno <= pPg.pPager.dbSize);
                                        }

                                        private static void assertTruncateConstraint(Pager pPager)
                                        {
                                            sqlite3PcacheIterateDirty(pPager.pPCache,
...
static void assertTruncateConstraint(Pager pPager) { }
                                    ...
                                    */
                                        private static void sqlite3PagerTruncateImage(Pager pPager, uint nPage)
                                        {
...
1==MEMDB
      ...
      enable_simulated_io_errors();
                                    ...
                                    */
                                        private static uint sqlite3PagerPagenumber(DbPage pPg)
                                        {
                                            return pPg.pgno;
                                        }
#else
                                                    ...
                                                    pPager.journalMode != PAGER_JOURNALMODE_MEMORY)
                                                {
                                                    ...
                                                    if (0 == (iDc & SQLITE_IOCAP_SAFE_APPEND))
                                                    {
                                                        ...
                                                        */
                                                        ...
                                                        aJournalMagic.CopyTo(zHeader,
                                                            0); // memcpy(zHeader, aJournalMagic, sizeof(aJournalMagic));
                                                        put32bits(zHeader, aJournalMagic.Length, pPager.nRec);
                                                        iNextHdrOffset = journalHdrOffset(pPager);
                                                        rc = sqlite3OsRead(pPager.jfd, aMagic, 8,
                                                            iNextHdrOffset);
                                                        if (rc == SQLITE_OK &&
                                                        ...
                                                        */
                                                        ...
                                                        rc = sqlite3OsWrite(
                                                            pPager.jfd, zHeader, zHeader.Length,
                                                            pPager.journalHdr
                                                        );
                                                        if (rc != SQLITE_OK)
                                                            return rc;
                                        ...
                                        */
                                        private static int pager_write_pagelist(Pager pPager, PgHdr pList)
                                        {
                                            ...
                                            */
                                            ...
                                            {
                                                var szFile = pPager.pageSize * pPager.dbSize;
                                                sqlite3OsFileControl(pPager.fd, SQLITE_FCNTL_SIZE_HINT,
                                                    ref szFile);
                                            ...
                                            while (rc == SQLITE_OK && pList)
                                            {
                                                ...
                                                */
                                                if (pList.pgno <= pPager.dbSize &&
                                                    0 == (pList.flags & PGHDR_DONT_WRITE))
                                                {
                                                    var offset =
                                                        (pList.pgno - 1) *
                                                        pPager.pageSize; /* Offset to write */
                                                    ...
                                                    /* Encode the database */
                                                    if (CODEC2(pPager, pList.pData, pgno,
                                                            SQLITE_ENCRYPT_WRITE_CTX, ref pData))
                                                        return
                                                            SQLITE_NOMEM; //     CODEC2(pPager, pList.pData, pgno, 6, return SQLITE_NOMEM, pData);
          ...
          PAGER_INCR( ref pPager.nWrite );
                                        ...
                                        private static int openSubJournal(Pager pPager)
                                        {
                                            ...
                                            if (!isOpen(pPager.sjfd))
                                            {
                                                if (pPager.journalMode == PAGER_JOURNALMODE_MEMORY ||
                                                    ...
                                                    rc = pagerOpentemp(pPager, ref pPager.sjfd,
                                            ...
                                            if (pPager.journalMode != PAGER_JOURNALMODE_OFF)
                                            {
                                                ...
                                                ** write the journal record into the file.  */
                                                if (rc == SQLITE_OK)
                                                {
                                                    ...
                                                    if (CODEC2(pPager, pData, pPg.pgno, SQLITE_ENCRYPT_READ_CTX,
                                                            ref pData2))
                                                        return
                                                            SQLITE_NOMEM; //CODEC2(pPager, pData, pPg.pgno, 7, return SQLITE_NOMEM, pData2);
                                                    PAGERTRACE("STMT-JOURNAL %d page %d\n", PAGERID(pPager),
                                                        pPg.pgno);
                                                    rc = write32bits(pPager.sjfd, offset, pPg.pgno);
                                                    if (rc == SQLITE_OK)
                                                        rc = sqlite3OsWrite(pPager.sjfd, pData2,
                                                            pPager.pageSize, offset + 4);
                                                }
                                            }
                                        ...
                                        private static int pagerStress(object p, PgHdr pPg)
                                        {
                                            ...
                                            */
                                            ...
                                            if (pPager.doNotSpill != 0)
                                                return SQLITE_OK;
                                            if (pPager.doNotSyncSpill != 0 &&
                                                (pPg.flags & PGHDR_NEED_SYNC) != 0) return SQLITE_OK;
                                            pPg.pDirty = null;
                                            if (pagerUseWal(pPager))
                                            {
                                                /* Write a single frame for this page to the log. */
                                                if (subjRequiresPage(pPg)) rc = subjournalPage(pPg);
                                                if (rc == SQLITE_OK) rc = pagerWalFrames(pPager, pPg, 0, 0, 0);
                                            }
                                            else
                                            {
                                                ...
                                                */
                                                if (NEVER(
                                                        rc == SQLITE_OK && pPg.pgno > pPager.dbSize &&
                                        ...
                                        */
                                        ...
                                        )
                                        {
                                            ...
                                            var rc = SQLITE_OK; /* Return code */
                                            byte tempFile =
                                                0; /* True for temp files (incl. in-memory files) */ // Needs to be u8 for later tests
                                            ...
                                            var noReadlock =
                                                (flags & PAGER_NO_READLOCK) != 0; /* True to omit read-lock */
                                            var pcacheSize =
                                            ...
                                            var nUri = 0; /* Number of bytes of URI args at *zUri */
                                            ...
                                            */
                                            if (!string.IsNullOrEmpty(zFilename))
                                            {
                                                ...
                                                zPathname = new StringBuilder(nPathname *
                                                                              2); // sqlite3Malloc( nPathname * 2 );
                                                //if ( zPathname == null )
                                                //{
                                                //  return SQLITE_NOMEM;
                                                //}
                                                //zPathname[0] = 0; /* Make sure initialized even if FullPathname() fails */
                                                rc = sqlite3OsFullPathname(pVfs, zFilename, nPathname,
                                                    zPathname);

                                                nPathname = sqlite3Strlen30(zPathname);
                                                z = zUri =
                                                    zFilename; //.Substring(sqlite3Strlen30( zFilename ) );
                                                //while ( *z )
                                                //{
                                                ...
                                                if (rc == SQLITE_OK && nPathname + 8 > pVfs.mxPathname)
                                                  ...
                                                  */
                                                    ...
                                                    */
                                                    ...
                                                    pPager.pPCache =
                                                        new PCache(); //(PCache*)(pPtr += ROUND8(sizeof(*pPager)));
                                                    pPager.fd =
                                                        new sqlite3_file(); //(sqlite3_file*)(pPtr += ROUND8(pcacheSize));
                                                    pPager.sjfd =
                                                        new sqlite3_file(); //(sqlite3_file*)(pPtr += ROUND8(pVfs.szOsFile));
                                                    pPager.jfd =
                                                        new sqlite3_file(); //(sqlite3_file*)(pPtr += journalFileSize);
                                                    ...
                                                    if (zPathname != null)
                                                    {
                                                        ...
                                                        zUri = pPager
                                                            .zFilename; //.Substring( nPathname + 1 );//memcpy( &pPager.zFilename[nPathname + 1], zUri, nUri );
                                                    ...
                                                    */
                                                    if (!string.IsNullOrEmpty(zFilename))
                                                    {
                                                        ...
                                                        */
                                                        ...
                                                        {
                                                            setSectorSize(pPager);
                                                            Debug.Assert(SQLITE_DEFAULT_PAGE_SIZE <=
                                                                         SQLITE_MAX_DEFAULT_PAGE_SIZE);
                                                            if (szPageDflt < pPager.sectorSize)
                                                            {
                                                                if (pPager.sectorSize >
                                                                    SQLITE_MAX_DEFAULT_PAGE_SIZE)
...
szPageDflt = ii;
                                                    ...
                                                    */
                                                    if (rc == SQLITE_OK)
                                                    {
                                                        ...
                                                        rc = sqlite3PagerSetPagesize(pPager, ref szPageDflt,
                                                            -1);
                                                        testcase(rc != SQLITE_OK);
                                                    }
                                                    ...
                                                    */
                                                    ...
                                                    sqlite3PcacheOpen((int)szPageDflt, nExtra, 0 == memDb,
                                                        0 == memDb ? (dxStress)pagerStress : null, pPager,
                                                        pPager.pPCache);
...
Debug.Assert(pPager.state == (tempFile != 0 ? PAGER_EXCLUSIVE : PAGER_UNLOCK));
                                                    ...
                                                    pPager.syncFlags =
                                                        (byte)(pPager.noSync ? 0 : SQLITE_SYNC_NORMAL);
                                                    ...
                                                    */
                                                    private static int hasHotJournal(Pager pPager,
                                                        ref int pExists)
                                                    {
                                                        var pVfs = pPager.pVfs;
                                                        var rc = SQLITE_OK; /* Return code */
                                                        var exists = 1; /* True if a journal file is present */
                                                        var jrnlOpen = isOpen(pPager.jfd) ? 1 : 0;
                                                        Debug.Assert(pPager.useJournal != 0);
                                                        Debug.Assert(isOpen(pPager.fd));
                                                        Debug.Assert(pPager.eState == PAGER_OPEN);

                                                        Debug.Assert(jrnlOpen == 0 ||
                                                        ...
                                                        if (0 == jrnlOpen)
                                                            rc = sqlite3OsAccess(pVfs, pPager.zJournal,
                                                                SQLITE_ACCESS_EXISTS, ref exists);
                                                        if (rc == SQLITE_OK && exists != 0)
                                                        {
                                                            ...
                                                            */
                                                            rc = sqlite3OsCheckReservedLock(pPager.fd,
                                                                ref locked);
                                                            if (rc == SQLITE_OK && locked == 0)
                                                            {
                                                                uint nPage =
                                                                    0; /* Number of pages in database file */
                                                                ...
                                                                */
                                                                ...
                                                                if (rc == SQLITE_OK)
                                                                {
                                                                    ...
                                                                    {
                                                                        sqlite3BeginBenignMalloc();
                                                                        if (pagerLockDb(pPager,
                                                                                RESERVED_LOCK) == SQLITE_OK)
                                                                        {
                                                                    ...
                                                                    else
                                                                    {
                                                                        ...
                                                                        */
                                                                        if (0 == jrnlOpen)
                                                                        {
                                                                            var f = SQLITE_OPEN_READONLY |
                                                                                SQLITE_OPEN_MAIN_JOURNAL;
                                                                            rc = sqlite3OsOpen(pVfs,
                                                                                pPager.zJournal, pPager.jfd, f,
                                                                                ref f);
                                                                        }
                                                    ...
                                                    */
                                                    private static int sqlite3PagerSharedLock(Pager pPager)
                                                    {
                                                        ...
                                                        */
                                                        Debug.Assert(sqlite3PcacheRefCount(pPager.pPCache) ==
                                                                     0);
                                                        Debug.Assert(assert_pager_state(pPager));
                                                        Debug.Assert(pPager.eState == PAGER_OPEN ||
                                                                     pPager.eState == PAGER_READER);
...
0==MEMDB
                                                            ...
                                                            Debug.Assert(pPager.noReadlock == 0 ||
                                                                         pPager.readOnly);
                                                            ...
                                                            if (pPager.noReadlock == 0)
                                                            {
                                                                ...
                                                                if (rc != SQLITE_OK)
                                                                {
                                                                    Debug.Assert(pPager.eLock == NO_LOCK ||
                                                            ...
                                                            */
                                                            ...
                                                            if (bHotJournal != 0)
                                                            {
                                                                ...
                                                                */
                                                                if (!isOpen(pPager.jfd))
                                                                {
                                                                    var pVfs = pPager.pVfs;
                                                                    var bExists =
                                                                        0; /* True if journal file exists */
                                                                    rc = sqlite3OsAccess(
                                                                        pVfs, pPager.zJournal,
                                                                        SQLITE_ACCESS_EXISTS, ref bExists);
                                                                    if (rc == SQLITE_OK && bExists != 0)
                                                                    {
                                                                        var fout = 0;
                                                                        var f = SQLITE_OPEN_READWRITE |
                                                                            SQLITE_OPEN_MAIN_JOURNAL;
                                                                        Debug.Assert(!pPager.tempFile);
                                                                        rc = sqlite3OsOpen(pVfs,
                                                                            pPager.zJournal, pPager.jfd, f,
                                                               ...
                                                               )
                                                            {
                                                                ...
                                                                */
                                                                uint nPage = 0;
                                                                var dbFileVers =
                                                                    new byte[pPager.dbFileVers.Length];
                                                                ...
                                                                if (nPage > 0)
                                                                {
                                                                    ...
                                                                    rc = sqlite3OsRead(pPager.fd, dbFileVers,
                                                                        dbFileVers.Length, 24);
                                                                    if (rc != SQLITE_OK) goto failed;
                                                                }
                                                                else
                                                                {
                                                                    Array.Clear(dbFileVers, 0,
                                                                        dbFileVers
                                                                            .Length); // memset( dbFileVers, 0, sizeof( dbFileVers ) );
                                                                }

                                                                if (memcmp(pPager.dbFileVers, dbFileVers,
                                                                        dbFileVers.Length) !=
...
0==MEMDB
                                                    ...
                                                    */
                                                    ...
                                                    private static int sqlite3PagerGet(
                                                        ...
                                                        uint pgno, /* Page number to fetch */
                                                    ...
                                                    private static int sqlite3PagerAcquire(
                                                        ...
                                                        uint pgno, /* Page number to fetch */
                                                        ref DbPage
                                                            ppPage, /* Write a pointer to the page here */
                                                        byte noContent /* Do not bother reading content from disk if true */
                                                    )
                                                    {
                                                        int rc;
                                                        PgHdr pPg = null;
                                                        ...
                                                        ** Otherwise, request the page from the PCache layer. */
                                                        if (pPager.errCode != SQLITE_OK)
                                                            ...
                                                            rc = sqlite3PcacheFetch(pPager.pPCache, pgno, 1,
                                                                ref ppPage);
        ...
        PAGER_INCR( ref pPager.nMiss );
                                                        ...
                                                        pPg.pPager = pPager;
                                                        pPg.pExtra =
                                                            new MemPage(); //memset(pPg.pExtra, 0, pPager.nExtra);
...
1==MEMDB
                                                            ...
                                                            pPager.memDb != 0
#endif
                                                            || pPager.dbSize < pgno || noContent != 0 ||
                                                            !isOpen(pPager.fd))
                                                        {
...
addToSavepointBitvecs(pPager, pgno);
                                                    ...
                                                    ** has ever happened.
                                                    */
                                                    private static DbPage sqlite3PagerLookup(Pager pPager,
                                                        uint pgno)
                                                    {
                                                        PgHdr pPg = null;
                                                    ...
                                                    */
                                                    private static int pager_open_journal(Pager pPager)
                                                    {
                                                        ...
                                                        ** an error state. */
                                                        ...
                                                        if (!pagerUseWal(pPager) && pPager.journalMode !=
                                                            PAGER_JOURNALMODE_OFF)
                                                        {
                                                            pPager.pInJournal =
                                                                sqlite3BitvecCreate(pPager.dbSize);
                                                            ...
                                                            if (!isOpen(pPager.jfd))
                                                            {
                                                                ...
                                                                {
                                                                    var
                                                                        flags = /* VFS flags to open journal file */
                                                                            SQLITE_OPEN_READWRITE |
                                                                            SQLITE_OPEN_CREATE |
                                                                            (pPager.tempFile
                                                                                ? SQLITE_OPEN_DELETEONCLOSE |
...
pVfs, pPager.zJournal, pPager.jfd, flags, jrnlBufferSize(pPager)
                                                    ...
                                                    */
                                                    private static int sqlite3PagerBegin(Pager pPager,
                                                        bool exFlag, int subjInMemory)
                                                    {
                                                        var rc = SQLITE_OK;
                                                        if (pPager.errCode != 0)
                                                            return pPager.errCode;
                                                        ...
                                                        if (ALWAYS(pPager.eState == PAGER_READER))
                                                        {
                                                            ...
                                                            if (pagerUseWal(pPager))
                                                            {
                                                                ...
                                                                */
                                                                rc = sqlite3WalBeginWriteTransaction(
                                                                    pPager.pWal);
                                                            }
                                                            else
                                                            {
                                                            ...
                                                            */
                                                            if (!pageInJournal(pPg) && !pagerUseWal(pPager))
                                                            {
                                                                    ...
                                                                    isOpen(pPager.jfd))
                                                                {
                                                                    ...
                                                                    ** that we do not. */
                                                                    Debug.Assert(pPg.pgno !=
                                                                        PENDING_BYTE / pPager.pageSize +
                                                                        1); //PAGER_MJ_PGNO(pPager) );

                                                                    Debug.Assert(pPager.journalHdr <=
                                                                        pPager.journalOff);
                                                                    if (CODEC2(pPager, pData, pPg.pgno,
                                                                            SQLITE_ENCRYPT_READ_CTX,
                                                                            ref pData2))
                                                                        return
                                                                            SQLITE_NOMEM; // CODEC2(pPager, pData, pPg.pgno, 7, return SQLITE_NOMEM, pData2);
                                                                    ...
                                                                    */
                                                                    ...
                                                                    rc = write32bits(pPager.jfd, iOff,
                                                                        pPg.pgno);
                                                                    if (rc != SQLITE_OK)
                                                                        return rc;
                                                                    rc = sqlite3OsWrite(pPager.jfd, pData2,
                                                                        pPager.pageSize, iOff + 4);
                                                                    if (rc != SQLITE_OK)
                                                                        return rc;
                                                                    rc = write32bits(pPager.jfd,
                                                                        iOff + pPager.pageSize + 4, cksum);
                                                                    if (rc != SQLITE_OK)
                                                                        return rc;
            ...
            sqlite3_pager_writej_count.iValue = iValue;
                                                    ...
                                                    */
                                                    private static int sqlite3PagerWrite(DbPage pDbPage)
                                                    {
                                                        ...
                                                        var pPager = pPg.pPager;
                                                        var nPagePerSector =
                                                            (uint)(pPager.sectorSize / pPager.pageSize);
                                                        ...
                                                        {
                                                            uint nPageCount =
                                                                0; /* Total number of pages in database file */
                                                            uint
                                                                pg1; /* First page of the sector pPg is located on. */
                                                            uint nPage =
                                                                0; /* Number of pages starting at pg1 to journal */
                                                            int ii; /* Loop counter */
                                                            var needSync =
                                                                false; /* True if any page has PGHDR_NEED_SYNC */
...
static bool sqlite3PagerIswriteable( DbPage pPg ) { return true; }
                                                ...
                                                */
                                                    ...
                                                    */
                                                    private static int pager_incr_changecounter(Pager pPager,
                                                        bool isDirectMode)
                                                    {
                                                        var rc = SQLITE_OK;
                                                            ...
                                                            */
                                                            ...
                                                            if (rc == SQLITE_OK)
                                                            {
                                                                ...
                                                                if (DIRECT_MODE)
                                                                {
                                                                    ...
                                                                    if (CODEC2(pPager, pPgHdr.pData, 1,
                                                                            SQLITE_ENCRYPT_WRITE_CTX, ref zBuf))
                                                                        return
                                                                            rc =
                                                                                SQLITE_NOMEM; //CODEC2(pPager, pPgHdr.pData, 1, 6, rc=SQLITE_NOMEM, zBuf);
                                                                    if (rc == SQLITE_OK)
                                                                        rc = sqlite3OsWrite(pPager.fd, zBuf,
                                                                            pPager.pageSize, 0);
...
0 == MEMDB
                                                            ...
                                                            sqlite3OsFileControl(pPager.fd,
                                                                SQLITE_FCNTL_SYNC_OMITTED, ref rc);
                                                        }
...
0 != MEMDB
                                                        ...
                                                        else
                                                        {
                                                            if (pagerUseWal(pPager))
                                                            {
                                                                var pList = sqlite3PcacheDirtyList(
                                                                    pPager.pPCache);
                                                                PgHdr pPageOne = null;
                                                                if (pList == null)
                                                                {
...
rc = pager_incr_changecounter(pPager, 0);
                                                                ...
                                                                */
                                                                   ...
                                                                   )
                                                                {
                                                                    ...
                                                                    var iSkip = PAGER_MJ_PGNO(
                                                                        pPager); /* Pending lock page */
                                                                    var dbSize =
                                                                        pPager.dbSize; /* Database image size */
                                                                    pPager.dbSize = pPager.dbOrigSize;
                                                                    for (i = dbSize + 1;
                                                                         i <= pPager.dbOrigSize;
                                                                         i++)
                                                                        if (0 == sqlite3BitvecTest(
                                                                                pPager.pInJournal, i) &&
                                                                            i != iSkip)
                                                                        {
                                                                            PgHdr pPage =
                                                                                null; /* Page to journal */
                                                      ...
                                                      */
                                                                ...
                                                                */
                                                                ...
                                                                rc = pager_write_pagelist(pPager,
                                                                ...
                                                                */
                                                                ...
                                                                {
                                                                    var nNew = (uint)(pPager.dbSize -
                                                                        (pPager.dbSize == PAGER_MJ_PGNO(pPager)
                                                                            ? 1
                                                                            : 0));
                                                                    Debug.Assert(pPager.eState >=
                                                                        PAGER_WRITER_DBMOD);
                                                                ...
                                                                if (!noSync) rc = sqlite3PagerSync(pPager);
                                                                IOTRACE("DBSYNC %p\n", pPager);
                                                            }
                                                        }
                                                    ...
                                                    private static int sqlite3PagerRollback(Pager pPager)
                                                    {
                                                        ...
                                                        */
                                                        ...
                                                        if (pagerUseWal(pPager))
                                                        {
                                                            ...
                                                            rc = sqlite3PagerSavepoint(pPager,
...
0==MEMDB
                                                        ...
                                                        Debug.Assert(pPager.eState == PAGER_READER ||
                                                                     rc != SQLITE_OK);
                                                        Debug.Assert(rc == SQLITE_OK || rc == SQLITE_FULL ||
                                                    ...
                                                    */
                                                    ...
                                                    {
                                                        var perPageSize = pPager.pageSize + pPager.nExtra +
                                                                          20; //+ sizeof(PgHdr) + 5*sizeof(void*);
                                                        return perPageSize *
                                                               sqlite3PcachePagecount(pPager.pPCache)
...
return MEMDB != 0;
                                                    ...
                                                    */
                                                    private static int sqlite3PagerOpenSavepoint(Pager pPager,
                                                        int nSavepoint)
                                                    {
                                                        var rc = SQLITE_OK; /* Return code */
                                                        var nCurrent =
                                                            pPager
                                                                .nSavepoint; /* Current number of savepoints */
                                                        ...
                                                        if (nSavepoint > nCurrent && pPager.useJournal != 0)
                                                        {
                                                            ...
                                                            PagerSavepoint[]
                                                                aNew; /* New Pager.aSavepoint array */
                                                            ...
                                                            */
                                                            ...
                                                            for (ii = nCurrent; ii < nSavepoint; ii++)
                                                            {
                                                                ...
                                                                //}
                                                                if (pagerUseWal(pPager))
                                                                    sqlite3WalSavepoint(pPager.pWal,
                                                                        aNew[ii].aWalData);
                                                    ...
                                                    */
                                                    private static int sqlite3PagerSavepoint(Pager pPager,
                                                        int op, int iSavepoint)
                                                    {
                                                        var rc = pPager.errCode; /* Return code */
                                                        ...
                                                        if (rc == SQLITE_OK && iSavepoint < pPager.nSavepoint)
                                                        {
                                                            int ii; /* Iterator variable */
                                                            int
                                                                nNew; /* Number of remaining savepoints after this op. */
                                                            ...
                                                            */
                                                            nNew = iSavepoint +
                                                                   (op == SAVEPOINT_RELEASE ? 0 : 1);
                                                            for (ii = nNew; ii < pPager.nSavepoint; ii++)
                                                                sqlite3BitvecDestroy(ref pPager.aSavepoint[ii]
                                                                    .pInSavepoint);
                                                ...
                                                */
                                                    private static void sqlite3PagerSetCodec(
                                                        ...
                                                        dxCodecSizeChng
                                                            xCodecSizeChng, //void (*xCodecSizeChng)(void*,int,int),
                                                ...
                                                */
                                                    private static int sqlite3PagerMovepage(Pager pPager,
                                                        DbPage pPg, uint pgno, int isCommit)
                                                    {
                                                        PgHdr pPgOld; /* The page being overwritten. */
                                                        uint needSyncPgno =
                                                            0; /* Old value of pPg.pgno, if sync is required */
                                                        int rc; /* Return code */
                                                        uint origPgno; /* The original page number */
...
1==MEMDB
                                                        ...
                                                        */
                                                        ...
                                                        PAGERTRACE(
                                                            ...
                                                            PAGERID(pPager), pPg.pgno,
                                                            (pPg.flags & PGHDR_NEED_SYNC) != 0 ? 1 : 0, pgno);
...
0!=MEMDB
                                                                ...
                                                                if (needSyncPgno != 0)
                                                                {
                                                                    ...
                                                                    */
                                                                    PgHdr pPgHdr = null;
                                                                    rc = sqlite3PagerGet(pPager, needSyncPgno,
                                                                        ref pPgHdr);
                                                                    if (rc != SQLITE_OK)
                                                                    {
                                                                        if (needSyncPgno <= pPager.dbOrigSize)
                                                                        {
                                                                            Debug.Assert(pPager.pTmpSpace !=
                                                                                null);
                                                                            var pTemp = new uint[pPager
                                                                                .pTmpSpace.Length];
                                                                            sqlite3BitvecClear(
                                                                                pPager.pInJournal, needSyncPgno,
                                                                                pTemp); //pPager.pTmpSpace );
                                                                        }
                                                            ...
                                                            */
                                                                private static byte[] sqlite3PagerGetData(
                                                                    DbPage pPg)
                                                                {
                                                                    Debug.Assert(pPg.nRef > 0 ||
                                                                        pPg.pPager.memDb != 0);
                                                                ...
                                                                */
                                                                private static MemPage sqlite3PagerGetExtra(
                                                                    DbPage pPg)
                                                                {
                                                                    return pPg.pExtra;
                                                                }
                                                                ...
                                                                ** locking-mode.
                                                                */
                                                                private static bool sqlite3PagerLockingMode(
                                                                ...
                                                                */
                                                                private static int sqlite3PagerSetJournalMode(
                                                                    Pager pPager, int eMode)
                                                                {
                                                                    var eOld = pPager
                                                                        .journalMode; /* Prior journalmode */
...
1==MEMDB
                                                                    ...
                                                                    {
                                                                        Debug.Assert(
                                                                            eOld == PAGER_JOURNALMODE_MEMORY ||
                                                                            eOld == PAGER_JOURNALMODE_OFF);
                                                                        if (eMode != PAGER_JOURNALMODE_MEMORY &&
                                                                            eMode != PAGER_JOURNALMODE_OFF)
                                                                            eMode = eOld;
                                                                    }
                                                                    ...
                                                                    if (eMode != eOld)
                                                                    {
                                                                        ...
                                                                        */
                                                                        Debug.Assert(
                                                                            (PAGER_JOURNALMODE_TRUNCATE & 5) ==
                                                                            1);
                                                                        Debug.Assert(
                                                                            (PAGER_JOURNALMODE_PERSIST & 5) ==
                                                                            1);
                                                                        Debug.Assert((PAGER_JOURNALMODE_DELETE &
                                                                            5) == 0);
                                                                        Debug.Assert((PAGER_JOURNALMODE_MEMORY &
                                                                            5) == 4);
                                                                        Debug.Assert(
                                                                            (PAGER_JOURNALMODE_OFF & 5) == 0);
                                                                        Debug.Assert(
                                                                            (PAGER_JOURNALMODE_WAL & 5) == 5);
                                                                ...
                                                                */
                                                                private static int sqlite3PagerGetJournalMode(
                                                                    Pager pPager)
                                                                {
                                                                    return pPager.journalMode;
                                                                }
                                                                ...
                                                                */
                                                                private static sqlite3_backup
                                                                    sqlite3PagerBackupPtr(Pager pPager)
                                                                {
