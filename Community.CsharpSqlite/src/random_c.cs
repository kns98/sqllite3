...
//# define wsdPrng p[0]
      ...
      */
            if (!wsdPrng.isInit)
            {
                ...
                for (i = 0; i < 255; i++) wsdPrng.s[i] = (byte)i;
                for (i = 0; i < 255; i++)
                {
        ...
        */
        private static void sqlite3_randomness(int N, ref long pBuf)
        {
            ...
            var mutex = sqlite3MutexAlloc(SQLITE_MUTEX_STATIC_PRNG);
#endif
            sqlite3_mutex_enter(mutex);
            while (N-- > 0) pBuf = (uint)((pBuf << 8) + randomu8()); //  zBuf[N] = randomu8();
            sqlite3_mutex_leave(mutex);
        }

        private static void sqlite3_randomness(byte[] pBuf, int Offset, int N)
        {
            var iBuf = System.DateTime.Now.Ticks;
#if SQLITE_THREADSAFE
            var mutex = sqlite3MutexAlloc(SQLITE_MUTEX_STATIC_PRNG);
#endif
            sqlite3_mutex_enter(mutex);
            while (N-- > 0)
            {
                iBuf = (uint)((iBuf << 8) + randomu8()); //  zBuf[N] = randomu8();
