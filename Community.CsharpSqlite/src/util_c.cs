    ...
    */
        ...
        */
        ...
        private static int sqlite3StrNICmp(string zLeft, string zRight, int N)
        {
            ...
            //b = (unsigned char )zRight;
            int a = 0, b = 0;
            while (N-- > 0 && a < zLeft.Length && b < zRight.Length && (zLeft[a] == zRight[b] ||
                                                                        (zLeft[a] != 0 && zLeft[a] < 256 &&
                                                                         zRight[b] < 256 && UpperToLower[zLeft[a]] ==
                                                                         UpperToLower[zRight[b]])))
            {
                a++;
                b++;
            }
        ...
        */
        private static bool sqlite3AtoF(string z, ref double pResult, int length, byte enc)
        {
            ...
            var sign = 1; /* sign of significand */
            long s = 0; /* significand */
            var d = 0; /* adjust exponent for shifting decimal point */
            var esign = 1; /* sign of exponent */
            var e = 0; /* exponent */
            var eValid = 1; /* True exponent is either not used or is well-formed */
            double result = 0;
            var nDigits = 0;
...
return !sqlite3Atoi64(z, pResult, length, enc);
        ...
        */
        private static int compare2pow63(string zNum, int incr)
        {
            ...
            int i;
            /* 012345678901234567 */
            var pow63 = "922337203685477580";
            for (i = 0; c == 0 && i < 18; i++) c = (zNum[i * incr] - pow63[i]) * 10;
        ...
        */
        private static int sqlite3Atoi64(string zNum, ref long pNum, int length, byte enc)
        {
            ...
            var incr = enc == SQLITE_UTF8 ? 1 : 2;
            ulong u = 0;
            var neg = 0; /* assume positive */
            int i;
            var c = 0;
            var zDx = 0; //  string zStart;
            ...
            for (i = zDx; i < length && (c = zNum[i]) >= '0' && c <= '9'; i += incr) u = u * 10 + (ulong)(c - '0');
            if (u > LARGEST_INT64)
                pNum = SMALLEST_INT64;
            else if (neg != 0)
                pNum = -(long)u;
        ...
        */
        ...
        private static int sqlite3PutVarint(byte[] p, int offset, ulong v)
        {
            ...
            bufByte10[0] &= 0x7f;
            Debug.Assert(n <= 9);
            for (i = 0, j = n - 1; j >= 0; j--, i++) p[offset + i] = bufByte10[j];
            return n;
        }
        ...
        */
        ...
        {
            uint u32_v = 0;
            var result = sqlite3GetVarint32(p, 0, out u32_v);
            v = (int)u32_v;
            return result;
        }
        ...
        {
            uint u32_v = 0;
            var result = sqlite3GetVarint32(p, offset, out u32_v);
            v = (int)u32_v;
            return result;
        }

        private static byte sqlite3GetVarint32(byte[] p, out uint v)
        {
            return sqlite3GetVarint32(p, 0, out v);
        }
...
return n;
        ...
        */
        ...
        private static uint sqlite3Get4byte(byte[] p, int offset)
        {
            return offset + 3 > p.Length
                ? 0
                : (uint)((p[0 + offset] << 24) | (p[1 + offset] << 16) | (p[2 + offset] << 8) | p[3 + offset]);
        ...
        private static uint sqlite3Get4byte(byte[] p, uint offset)
        {
            return offset + 3 > p.Length
                ? 0
                : (uint)((p[0 + offset] << 24) | (p[1 + offset] << 16) | (p[2 + offset] << 8) | p[3 + offset]);
        ...
        private static uint sqlite3Get4byte(byte[] p)
        {
...
//#endif
    ...
    */
        ...
        */
        ...
        private const long TWOPOWER32 = (long)1 << 32;

        //#define TWOPOWER31 (((i64)1)<<31)
        private const long TWOPOWER31 = (long)1 << 31;

        private static int sqlite3MulInt64(ref long pA, long iB)
        {
            var iA = pA;
            long iA1, iA0, iB1, iB0, r;
