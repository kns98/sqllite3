        ...
        private static PgHdr pcache1Alloc(int nByte)
        {
            ...
            if (p == null)
            {
                ...
                */
                ...
                //if ( p != null )
                {
                    var sz = nByte; //sqlite3MallocSize( p );
                    sqlite3_mutex_enter(pcache1.mutex);
  ...
  if( p>=pcache1.pStart && p<pcache1.pEnd ){
    ...
    */
        private static PgHdr1 pcache1AllocPage(PCache1 pCache)
        {
            ...
            var pPg = pcache1Alloc(pCache.szPage); //nByte );
            PgHdr1 p = null;
            //if ( pPg !=null)
            {
                ...
                p.pCache = pCache;
                p.pPgHdr = pPg;
                if (pCache.bPurgeable) pCache.pGroup.nCurrentPage++;
            }
            //else
            //{
            //  p = 0;
        ...
        */
        private static void pcache1PinPage(PgHdr1 pPage)
        {
            ...
            if (pPage.pLruNext != null || pPage == pGroup.pLruTail)
            {
                ...
                if (pGroup.pLruHead == pPage) pGroup.pLruHead = pPage.pLruNext;
                if (pGroup.pLruTail == pPage) pGroup.pLruTail = pPage.pLruPrev;
                pPage.pLruNext = null;
                pPage.pLruPrev = null;
