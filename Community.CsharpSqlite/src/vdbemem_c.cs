        ...
        */
        private static int sqlite3VdbeMemGrow(Mem pMem, int n, int preserve)
        {
            ...
            if (pMem.z == null)
                ...
                pMem.flags = (ushort)(pMem.flags & ~(MEM_Ephem | MEM_Static));
            pMem.xDel = null;
...
return r;
      ...
      */
            const long maxInt = LARGEST_INT64;
            const long minInt = SMALLEST_INT64;
        ...
        */
        private static void sqlite3VdbeIntegerAffinity(Mem pMem)
        {
            ...
            */
            if (pMem.r == pMem.u.i && pMem.u.i > SMALLEST_INT64
        ...
        */
        private static int sqlite3VdbeMemNumerify(Mem pMem)
        {
            if ((pMem.flags & (MEM_Int | MEM_Real | MEM_Null)) == 0)
            {
                ...
                {
                    if (0 == sqlite3Atoi64(Encoding.UTF8.GetString(pMem.zBLOB, 0, pMem.zBLOB.Length),
                            ref pMem.u.i, pMem.n, pMem.enc))
                    {
                        MemSetTypeFlag(pMem, MEM_Int);
                    }
                    else
    ...
    */
        ...
        */
        private static void sqlite3VdbeMemShallowCopy(Mem pTo, Mem pFrom, int srcType)
        {
            ...
            {
                pTo.flags = (ushort)(pFrom.flags & ~(MEM_Dyn | MEM_Static | MEM_Ephem));
        ...
        */
        private static int sqlite3VdbeMemCopy(Mem pTo, Mem pFrom)
        {
            ...
            sqlite3VdbeMemReleaseExternal(pTo);
            pFrom.CopyTo(ref pTo); // memcpy(pTo, pFrom, MEMCELLSIZE);
            pTo.flags = (ushort)(pTo.flags & ~MEM_Dyn);
        ...
        */
        ...
        )
        {
            var nByte = n; /* New value for pMem->n */
            int iLimit; /* Maximum allowed string or blob size */
            ushort flags = 0; /* New value for pMem->flags */
            ...
            if (pMem.db != null)
                ...
                iLimit = SQLITE_MAX_LENGTH;
            flags = (ushort)(enc == 0 ? MEM_Blob : MEM_Str);
            if (nByte < 0)
            {
                Debug.Assert(enc != 0);
                if (enc == SQLITE_UTF8)
                    for (nByte = 0;
                         nByte <= iLimit && nByte < z.Length - offset && z[offset + nByte] != 0;
                         nByte++)
                    {
                    }
                else
                    for (nByte = 0;
                         (nByte <= iLimit && z[nByte + offset] != 0) || z[offset + nByte + 1] != 0;
                         nByte += 2)
                    {
                    }
            ...
            */
            ...
            {
                var nAlloc = (uint)nByte;
                if ((flags & MEM_Term) != 0) nAlloc += (uint)(enc == SQLITE_UTF8 ? 1 : 2);
                if (nByte > iLimit) return SQLITE_TOOBIG;
                ...
                if (enc == 0)
                {
                    pMem.z = null;
                    pMem.zBLOB = sqlite3Malloc(n);
                    for (var i = 0; i < n && i < z.Length - offset; i++)
            ...
            else
            {
                ...
                pMem.xDel = xDel;
                flags |= (ushort)(xDel == SQLITE_STATIC ? MEM_Static : MEM_Dyn);
            }
                    ...
                    ** comparison function directly */
                        ...
                        )
                        {
                            ...
                            ** that both the BtShared and database handle mutexes are held. */
                            ...
                            else if (SQLITE_OK == (rc = sqlite3VdbeMemGrow(pMem, amt + 2, 0)))
                            {
                                ...
                                if (key)
                                    rc = sqlite3BtreeKey(pCur, (uint)offset, (uint)amt, pMem.zBLOB);
                                else
                                    rc = sqlite3BtreeData(pCur, (uint)offset, (uint)amt,
                                        pMem.zBLOB); //pMem.z =  pMem_z ;
                                //pMem.z[amt] = 0;
                                //pMem.z[amt+1] = 0;
                        ...
                        */
                        private static string sqlite3ValueText(sqlite3_value pVal, int enc)
                        {
                            ...
                            pVal.flags |= (ushort)((pVal.flags & MEM_Blob) >> 3);
                            ...
                            if ((pVal.flags & MEM_Str) != 0)
                            {
                                ...
                                if ((enc & SQLITE_UTF16_ALIGNED) != 0 &&
                                    1 == (1 & pVal.z[0])) //1==(1&SQLITE_PTR_TO_INT(pVal.z))
                                {
        ...
        op = pExpr.op2;
                      ...
                      */
                            ...
                            if (op == TK_STRING || op == TK_FLOAT || op == TK_INTEGER)
                            {
                                ...
                                {
                                    sqlite3VdbeMemSetInt64(pVal, (long)pExpr.u.iValue * negInt);
                                }
                                else
                                {
                            ...
                            else if (op == TK_BLOB)
                            {
                                ...
                                var blob = sqlite3HexToBlob(db, zVal, nVal);
                                sqlite3VdbeMemSetStr(pVal, Encoding.UTF8.GetString(blob, 0, blob.Length),
                                    nVal / 2, 0, SQLITE_DYNAMIC);
                            }
#endif

                            if (pVal != null) sqlite3VdbeMemStoreType(pVal);
