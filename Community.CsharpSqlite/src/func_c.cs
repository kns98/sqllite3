        ...
        */
        ...
        )
        {
            ...
            if (z2 != null)
                ...
                //  }
                sqlite3_result_text(context, z2.Length == 0 ? "" : z2.Substring(0, n).ToLower(), -1,
                    null); //sqlite3_free );
            //}
        }
        ...
        */
        ...
        )
        {
            ...
            if (p != null)
            {
                ...
                for (var i = 0; i < n; i++)
                {
                    sqlite3_randomness(sizeof(byte), ref _p);
                    p[i] = (char)(_p & 0x7F);
                }
            ...
            while ((c = sqlite3Utf8Read(zPattern, ref zPattern)) != 0)
                if (!prevEscape && c == matchAll)
                {
                    ...
                    else if (c == matchSet)
                    {
                        ...
                        while (len < zString.Length &&
                               patternCompare(inPattern.Substring(inPattern.Length - zPattern.Length - 1),
                                   zString.Substring(len), pInfo, esc) ==
                               false) SQLITE_SKIP_UTF8(zString, ref len);
                    ...
                    while ((c2 = sqlite3Utf8Read(zString, ref zString)) != 0)
                    {
                        ...
                        {
                            if (0 == (c2 & ~0x7f))
                                c2 = (uint)sqlite3UpperToLower[c2]; //GlogUpperToLower(c2);
                            if (0 == (c & ~0x7f))
                                c = (uint)sqlite3UpperToLower[c]; //GlogUpperToLower(c);
                            while (c2 != 0 && c2 != c)
                            {
    ...
    ** digits. */
        ...
        */
        ...
        )
        {
            ...
            {
                int[] lenOne = { 1 };
                byte[] azOne = { (byte)' ' }; //static unsigned char * const azOne[] = { (u8*)" " };
                nChar = 1;
                aLen = lenOne;
                azChar = new byte[1][];
                azChar[0] = azOne;
            ...
            if (nChar > 0)
            {
                flags = (int)sqlite3_user_data(
                    context); // flags = SQLITE_PTR_TO_INT(sqlite3_user_data(context));
                if ((flags & 1) != 0)
                    while (nIn > 0)
                    {
                        var len = 0;
        ...
        )
        {
            ...
            if (p != null && type != SQLITE_NULL)
            {
                ...
                if (type == SQLITE_INTEGER)
                {
                    ...
                    p.rSum += v;
                    if (!(p.approx | (p.overflow != 0)) && 0 != sqlite3AddInt64(ref p.iSum, v)) p.overflow = 1;
                }
                else
                {
...
|| p.n == sqlite3_aggregate_count( context ) );
        ...
        */
        ...
        public struct sAggs
        {
            ...
            // Constructor
            public sAggs(string zName, sbyte nArg, byte argType, byte needCollSeq, dxStep xStep,
                dxFinal xFinalize)
            {
                this.zName = zName;
                this.nArg = nArg;
        ...
        */
        private static void sqlite3RegisterLikeFunctions(sqlite3 db, int caseSensitive)
        {
            ...
            if (caseSensitive != 0)
                ...
                pInfo = likeInfoNorm;
            sqlite3CreateFunc(db, "like", 2, SQLITE_UTF8, pInfo, likeFunc, null, null, null);
            sqlite3CreateFunc(db, "like", 3, SQLITE_UTF8, pInfo, likeFunc, null, null, null);
            sqlite3CreateFunc(db, "glob", 2, SQLITE_UTF8,
                globInfo, likeFunc, null, null, null);
...
// use versionFunc here just for a dummy placeholder
                new FuncDef(-1, SQLITE_UTF8, SQLITE_FUNC_COALESCE, null, null, versionFunc, null, null,
                    "coalesce", null, null),
                FUNCTION("hex", 1, 0, 0, hexFunc),
/*  FUNCTION(ifnull,             2, 0, 0, ifnullFunc       ), */
// use versionFunc here just for a dummy placeholder
                new FuncDef(2, SQLITE_UTF8, SQLITE_FUNC_COALESCE, null, null, versionFunc, null, null, "ifnull",
                    null, null),
...
/* AGGREGATE(count,             0, 0, 0, countStep,       countFinalize  ), */
                new FuncDef(0, SQLITE_UTF8, SQLITE_FUNC_COUNT, null, null, null, countStep, countFinalize,
                    "count", null, null),
