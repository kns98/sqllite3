...
namespace Community.CsharpSqlite
{
    ...
    public partial class Sqlite3
    {
        ...
        */
#if SQLITE_DEBUG
        private static readonly bool sqlite3VdbeAddopTrace = false;
#endif
...
pOp.cnt = 0;
        ...
        */
        ...
        {
            int j;
            var addr = sqlite3VdbeAddOp3(p, OP_ParseSchema, iDb, 0, 0);
        ...
        */
        private static int sqlite3VdbeMakeLabel(Vdbe p)
        {
            ...
            if (i >= p.nLabelAlloc)
            {
                var n = p.nLabelAlloc == 0 ? 15 : p.nLabelAlloc * 2 + 5;
                if (p.aLabel == null)
    ...
    */
        private static void resolveP2Values(Vdbe p, ref int pMaxFuncArgs)
        {
            ...
            {
                pOp = p.aOp[i];
                var opcode = pOp.opcode;

                pOp.opflags = (byte)sqlite3OpcodeProperty[opcode];
...
#if SQLITE_DEBUG
                    pOut.zComment = null;
                    if (sqlite3VdbeAddopTrace) sqlite3VdbePrintOp(null, i + addr, p.aOp[i + addr]);
#endif
                }
...
if( p.nOp==0 ) return dummy;
    ...
    */
        ...
        */
        ...
        {
            Debug.Assert(i >= 0 && i < p.db.nDb && i < sizeof(int) * 8);
            Debug.Assert(i < sizeof(int) * 8);
            p.btreeMask |= 1 << i;
            if (i != 1 && sqlite3BtreeSharable(p.db.aDb[i].pBt)) p.lockMask |= 1 << i;
        }
  ...
  for(i = 0, mask = 1; i<nDb; i++, mask += mask){
    ...
    */
        private static void sqlite3VdbePrintOp(FILE pOut, int pc, Op pOp)
        {
            string zP4;
            string zPtr = null;
            var zFormat1 = "%4d %-13s %4d %4d %4d %-4s %.2X %s\n";
            if (pOut == null)
                pOut = Console.Out;
            zP4 = displayP4(pOp, zPtr, 50);
            var zOut = new StringBuilder(10);
    ...
    */
        ...
        private static void releaseMemArray(Mem[] p, int starting, int N)
        {
            if (p != null && p.Length > starting && p[starting] != null && N != 0)
            {
                ...
                //  return;
                //}
                for (var i = starting; i < N; i++) //pEnd =  p[N] ; p < pEnd ; p++ )
                {
                    pEnd = p[i];
        ...
        */
        private static void sqlite3VdbeFrameDelete(VdbeFrame p)
        {
            ...
            var apCsr = p.aChildCsr; // (VdbeCursor)aMem[p.nChildMem];
            for (i = 0; i < p.nChildCsr; i++) sqlite3VdbeFreeCursor(p.v, apCsr[i]);
            ...
            */
            ...
            if (p.explain == 1)
            {
                ...
                ** cells.  */
                ...
                for (i = 0; i < nSub; i++) nRow += apSub[i].nOp;
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    ** pick up the appropriate opcode. */
                    int j;
                    i -= p.nOp;
                    for (j = 0; i >= apSub[j].nOp; j++) i -= apSub[j].nOp;
                    pOp = apSub[j].aOp[i];
                }
                ...
                if (p.explain == 1)
                {
                    ...
                    if (p.pResultSet[i_pMem] == null) p.pResultSet[i_pMem] = sqlite3Malloc(p.pResultSet[i_pMem]);
...
#if SQLITE_DEBUG
            for (i = 1; i < p.nMem; i++) Debug.Assert(p.aMem[i].db == p.db);
#endif
            p.pc = -1;
            p.rc = SQLITE_OK;
      ...
      for(i = 0; i<p.nOp; i++){
        ...
        */
        ...
        )
        {
            ...
            */
            ...
            if (pParse.explain != 0 && nMem < 10) nMem = 10;
            ...
            */
            ...
            for (n = 0; n < p.nzVar; n++) p.azVar[n] = pParse.azVar[n];
            //
            ...
            for (n = 0; n < nVar; n++) p.aVar[n] = sqlite3Malloc(p.aVar[n]);
            p.nVar = (short)nVar;
            //
        ...
        */
        private static void sqlite3VdbeFreeCursor(Vdbe p, VdbeCursor pCx)
        {
            ...
            {
                var pVtabCursor = pCx.pVtabCursor;
                var pModule = pCx.pModule;
                p.inVtabMethod = 1;
        ...
        */
        private static void closeAllCursors(Vdbe p)
        {
            ...
            if (p.aMem != null) releaseMemArray(p.aMem, 1, p.nMem);
            while (p.pDelFrame != null)
            {
                var pDel = p.pDelFrame;
...
*/
      ...
      */
            ...
            */
                ...
                || nTrans <= 1)
            {
                ...
                */
                ...
                {
                    var pBt = db.aDb[i].pBt;
                    if (pBt != null) rc = sqlite3BtreeCommitPhaseTwo(pBt, 0);
                }
            ...
            */
            ...
            else
            {
                ...
                do
                {
                    ...
                    sqlite3_randomness(sizeof(uint), ref iRandom); //random.Length
                ...
                */
                for (i = 0; i < db.nDb; i++)
                {
                    ...
                    if (sqlite3BtreeIsInTrans(pBt))
                    {
                        ...
                        if (zFile == null) continue; /* Ignore TEMP and :memory: databases */
                        Debug.Assert(zFile != "");
        ...
        disable_simulated_io_errors();
                ...
                {
                    var pBt = db.aDb[i].pBt;
                    if (pBt != null) sqlite3BtreeCommitPhaseTwo(pBt, 0);
                }
        ...
        private static int sqlite3VdbeCloseStatement(Vdbe p, int eOp)
        {
            ...
            */
            if (db.nStatement != 0 && p.iStatement != 0)
            {
                ...
                if (rc == SQLITE_OK)
                {
                    if (eOp == SAVEPOINT_ROLLBACK) rc = sqlite3VtabSavepoint(db, SAVEPOINT_ROLLBACK, iSavepoint);
                    if (rc == SQLITE_OK) rc = sqlite3VtabSavepoint(db, SAVEPOINT_RELEASE, iSavepoint);
                }

                /* If the statement transaction is being rolled back, also restore the 
                ** database handles deferred constraint counter to the value it had when 
                ** the statement transaction was opened.  */
                if (eOp == SAVEPOINT_ROLLBACK) db.nDeferredCons = p.nStmtDefCons;
            }
        ...
        private static int sqlite3VdbeHalt(Vdbe p)
        {
            ...
            */
            ...
            if (p.pc >= 0)
            {
                ...
                if (p.rc == SQLITE_OK) sqlite3VdbeCheckFk(p, 0);
        ...
        private static int sqlite3VdbeReset(Vdbe p)
        {
            ...
            */
            if (p.pc >= 0)
            {
                ...
                {
                    sqlite3BeginBenignMalloc();
                    sqlite3ValueSetStr(db.pErr, -1, p.zErrMsg == null ? "" : p.zErrMsg, SQLITE_UTF8,
                        SQLITE_TRANSIENT);
                    sqlite3EndBenignMalloc();
                    db.errCode = p.rc;
                    sqlite3DbFree(db, ref p.zErrMsg);
...
fclose(out);
        ...
        */
        private static void sqlite3VdbeDeleteAuxData(VdbeFunc pVdbeFunc, int mask)
        {
            ...
            for (i = 0; i < pVdbeFunc.nAux; i++)
            {
                ...
                {
                    if (pAux.pAux != null && pAux.pAux is IDisposable) (pAux.pAux as IDisposable).Dispose();
                    pAux.pAux = null;
                }
            }
        }
        ...
        */
        private static uint sqlite3VdbeSerialType(Mem pMem, int file_format)
        {
            ...
            if ((flags & MEM_Int) != 0)
            {
                ...
                var i = pMem.u.i;
                ulong u;
                if (file_format >= 4 && (i & 1) == i) return 8 + (uint)i;
                if (i < 0)
                {
                    if (i < -MAX_6BYTE)
                        return 6;
            ...
            {
                if (pMem.z != null)
                    n = Encoding.UTF8.GetByteCount(pMem.n < pMem.z.Length
                        ? pMem.z.Substring(0, pMem.n)
                        : pMem.z);
                else
                    n = pMem.zBLOB.Length;
                pMem.n = n;
            ...
            Debug.Assert(n >= 0);
            return (uint)(n * 2 + 12 + ((flags & MEM_Str) != 0 ? 1 : 0));
        }

        /*
        ** Return the length of the data corresponding to the supplied serial-type.
        */
        private static readonly uint[] aSize = { 0, 1, 2, 3, 4, 6, 8, 8, 0, 0, 0, 0 };

        private static uint sqlite3VdbeSerialTypeLen(uint serial_type)
        {
            if (serial_type >= 12)
...
//# define swapMixedEndianFloat(X)  X = floatSwap(X)
    ...
    */
        private static uint sqlite3VdbeSerialPut(byte[] buf, int offset, int nBuf, Mem pMem,
            int file_format)
        {
...
swapMixedEndianFloat( v );
            ...
            if (serial_type >= 12)
            {
                ...
                if ((len = (uint)pMem.n) != 0)
                    ...
                    {
                        Buffer.BlockCopy(pMem.zBLOB, 0, buf, offset,
                ...
                if ((pMem.flags & MEM_Zero) != 0)
                {
                    ...
                    Debug.Assert(nBuf >= 0);
                    if (len > (uint)nBuf) len = (uint)nBuf;
                    Array.Clear(buf, offset + pMem.n,
                        (int)(len - pMem.n)); // memset( &buf[pMem.n], 0, len - pMem.n );
                }
        ...
        )
        {
            switch (serial_type)
            {
                ...
                case 4:
                {
                    ...
                    pMem.u.i = ((sbyte)buf[offset + 0] << 24) | (buf[offset + 1] << 16) |
                ...
                case 5:
                {
                    ...
                    var y = (uint)((buf[offset + 2] << 24) | (buf[offset + 3] << 16) |
...
#endif
                    Debug.Assert(sizeof(double) == sizeof(ulong) && memcmp(BitConverter.GetBytes(r1),
                            BitConverter.GetBytes(t2), sizeof(double)) ==
                        0); //Debug.Assert( sizeof(r1)==sizeof(t2) && memcmp(&r1, t2, sizeof(r1))==0 );
#endif

                    x = (ulong)((buf[offset + 0] << 24) | (buf[offset + 1] << 16) | (buf[offset + 2] << 8) |
                                buf[offset + 3]);
                    y = (uint)((buf[offset + 4] << 24) | (buf[offset + 5] << 16) | (buf[offset + 6] << 8) |
                               buf[offset + 7]);
                    x = (x << 32) | y;
                    if (serial_type == 6)
                    {
              ...
              pMem.r = BitConverter.ToDouble(BitConverter.GetBytes((long)x), 0);
...
#endif
                        pMem.flags = (ushort)(sqlite3IsNaN(pMem.r) ? MEM_Null : MEM_Real);
                    }
                ...
                default:
                {
                    ...
                    if ((serial_type & 0x01) != 0)
                    {
                        ...
                        {
                            pMem.z = Encoding.UTF8.GetString(buf, offset,
                                (int)len); //memcpy( buf, pMem.z, len );
                    ...
                    else
                    {
                        ...
                        if (len <= buf.Length - offset)
                            Buffer.BlockCopy(buf, offset, pMem.zBLOB, 0,
                                (int)len); //memcpy( buf, pMem.z, len );
                        else
...
swapMixedEndianFloat(x);
        ...
        */
        ...
        )
        {
            ...
            */
            ...
            idx = (uint)getVarint32(aKey, 0, out szHdr); // GetVarint( aKey, szHdr );
            d = szHdr;
            u = 0;
            while (idx < szHdr && u < p.nField && d <= nKey)
            {
                ...
                idx += (uint)getVarint32(aKey, idx,
                    out serial_type); // GetVarint( aKey + idx, serial_type );
        ...
        */
        ...
        )
        {
            ...
            */
            ...
            idx1 = (uint)((szHdr1 = pKey1[offset]) <= 0x7f
                ? 1
            ...
            while (idx1 < szHdr1 && i < pPKey2.nField)
            {
                ...
                idx1 += (uint)((serial_type1 = pKey1[offset + idx1]) <= 0x7f
                    ? 1
                    : getVarint32(pKey1, (uint)(offset + idx1),
                        out serial_type1)); //GetVarint( aKey1 + idx1, serial_type1 );
                ...
                */
                ...
                if (rc != 0)
                {
                    ...
                    if (pKeyInfo.aSortOrder != null && i < nField && pKeyInfo.aSortOrder[i] != 0) rc = -rc;
        ...
        */
        private static int sqlite3VdbeIdxRowid(sqlite3 db, BtCursor pCur, ref long rowid)
        {
            long nCellKey = 0;
            int rc;
            ...
            */
            ...
            Debug.Assert(((uint)nCellKey & SQLITE_MAX_U32) == (ulong)nCellKey);
            ...
            ** Verify that the last entry really is an integer. */
            ...
            testcase((uint)m.n == szHdr + lenRowid);
            if (unlikely((uint)m.n < szHdr + lenRowid)) goto idx_rowid_corruption;
        ...
        */
        private static sqlite3_value sqlite3VdbeGetValue(Vdbe v, int iVar, byte aff)
        {
            Debug.Assert(iVar > 0);
            if (v != null)
            {
                var pMem = v.aVar[iVar - 1];
                if (0 == (pMem.flags & MEM_Null))
                {
                    var pRet = sqlite3ValueNew(v.db);
                    if (pRet != null)
                    {
