...
//#define winLogError(a,b,c)     winLogErrorAtLine(a,b,c,__LINE__)
        ...
        )
        {
            ...
            int i; /* Loop counter */
            ulong iErrno; // = GetLastError();  /* Error code */
#if SQLITE_SILVERLIGHT
      iErrno = (int)ERROR_NOT_SUPPORTED;
#else
            iErrno = (uint)Marshal.GetLastWin32Error();
#endif
...
*****************************************************************************/
    ...
    */
        private static int seekWinFile(sqlite3_file id, long iOffset)
        {
            ...
            ulong dwRet; /* Value returned by SetFilePointer() */
            var pFile = id;
...
free(pFile.zDeleteOnClose);
      ...
      OpenCounter( -1 );
        ...
        */
        private static int winRead(
            ...
            long offset /* Begin reading at this offset */
        )
        {
            long rc;
            var pFile = id;
      ...
      if ( SimulateIOError() )
        return SQLITE_IOERR_READ;
            ...
            if (!id.fs.CanRead)
                return SQLITE_IOERR_READ;
            if (seekWinFile(pFile, offset) != 0) return SQLITE_FULL;
...
pFile.lastErrno = 1;
            ...
            if (nRead < amt)
            {
                ...
                Array.Clear(pBuf, nRead, amt - nRead); // memset(&((char)pBuf)[nRead], 0, amt-nRead);
        ...
        */
        private static int winWrite(
            ...
            long offset /* Offset into the file to begin writing at */
        )
        {
            int rc; /* True if error has occured, else false */
            var pFile = id; /* File handle */
...
id.lastErrno = 1;
#else
                id.lastErrno = (uint)Marshal.GetLastWin32Error();
#endif
        ...
        */
        ...
        {
            var pFile = id; /* File handle object */
            var rc = SQLITE_OK; /* Return code for this function */
      ...
      */
            ...
            if (pFile.szChunk != 0) nByte = (nByte + pFile.szChunk - 1) / pFile.szChunk * pFile.szChunk;
...
error = 1;
        ...
        */
        private static int winUnlock(sqlite3_file id, int locktype)
        {
            ...
            if (type >= RESERVED_LOCK)
                try
                {
                    lockingStrategy.UnlockFile(pFile, RESERVED_BYTE,
                        1); // UnlockFile(pFile.h, RESERVED_BYTE, 0, 1, 0);
                }
                catch (Exception e)
                {
                }
        ...
        */
        private static int winFileControl(sqlite3_file id, int op, ref long pArg)
        {
            switch (op)
            {
...
int isTemp = 0;
            ...
            */
            ...
            var rc = SQLITE_OK; /* Function Return Code */
            var eType = (int)(flags & 0xFFFFFF00); /* Type of file to open */
            var isExclusive = (flags & SQLITE_OPEN_EXCLUSIVE) != 0;
            var isDelete = (flags & SQLITE_OPEN_DELETEONCLOSE) != 0;
            var isCreate = (flags & SQLITE_OPEN_CREATE) != 0;
            var isReadonly = (flags & SQLITE_OPEN_READONLY) != 0;
            var isReadWrite = (flags & SQLITE_OPEN_READWRITE) != 0;
            var isOpenJournal = isCreate && (
      ...
      */
            ...
            ** automatically deleted. Nor are they ever temporary files.  */
            ...
            Debug.Assert((!isDelete && !string.IsNullOrEmpty(zName)) ||
                         eType != SQLITE_OPEN_MASTER_JOURNAL);
...
dwFlagsAndAttributes |= FileOptions.RandomAccess; // FILE_FLAG_RANDOM_ACCESS;
 ...
 new IsolatedStorageFileStream(zConverted, dwCreationDisposition, dwDesiredAccess, dwShareMode, IsolatedStorageFile.GetUserStoreForApplication());
                        ...
                        fs = new FileStream(zConverted, dwCreationDisposition, dwDesiredAccess, dwShareMode,
                            4096, dwFlagsAndAttributes);
#else
            ...
            */
            ...
            switch (flags)
            {
                ...
                case SQLITE_ACCESS_READWRITE:
                    rc = attr == 0 ? 0 :
                        (int)(attr & FileAttributes.ReadOnly) != 0 ? 0 :
                        1; //FILE_ATTRIBUTE_READONLY ) == 0;
                    break;
                default:
...
//DWORD cnt = GetTickCount();
                ...
                put32bits(zBuf, n, (uint)dt.Ticks); // memcpy(&zBuf[n], cnt, sizeof(cnt));
                n += 4; // cnt.Length;
            }
            ...
            if (sizeof(long) <= nBuf - n)
            {
                ...
                put32bits(zBuf, n, (uint)(i & 0xFFFFFFFF)); //memcpy(&zBuf[n], i, sizeof(i));
                put32bits(zBuf, n, (uint)(i >> 32));
                n += sizeof(long);
            }
#endif
            return n;
    ...
    */
        private static int winCurrentTimeInt64(sqlite3_vfs pVfs, ref long piNow)
        {
      ...
      const sqlite3_int64 unixEpoch = 24405875 * (sqlite3_int64)8640000;
            ...
            piNow = winFiletimeEpoch + System.DateTime.UtcNow.ToFileTimeUtc() / 10000;
#if SQLITE_TEST
#if !TCLSH
