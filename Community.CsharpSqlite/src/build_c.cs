...
p.zName, P4_STATIC );
    ...
    */
        ...
        */
        ...
        private static readonly object nestingLock = new object();
        ...
        */
        private static Table sqlite3FindTable(sqlite3 db, string zName, string zDatabase)
        {
            ...
            for (i = OMIT_TEMPDB; i < db.nDb; i++)
            {
                ...
                if (zDatabase != null &&
                    !zDatabase.Equals(db.aDb[j].zName, StringComparison.InvariantCultureIgnoreCase))
                    continue;
        ...
        */
        private static void sqlite3ResetInternalSchema(sqlite3 db, int iDb)
        {
            ...
            ** databases. */
            ...
            {
                var pDb = db.aDb[i];
                if (pDb.pSchema != null) sqlite3SchemaClear(pDb.pSchema);
            }
        ...
        */
        private static int sqlite3CheckObjectName(Parse pParse, string zName)
        {
            if (0 == pParse.db.init.busy && pParse.nested == 0
                                         ...
                                         && zName.StartsWith("sqlite_", StringComparison.InvariantCultureIgnoreCase))
            {
        ...
        */
        ...
        )
        {
                              ...
                              && sortOrder == SQLITE_SO_ASC)
            {
                ...
                pTab.tabFlags |= (byte)(autoInc * TF_Autoincrement);
            }
            else if (autoInc != 0)
            {
#if !SQLITE_OMIT_AUTOINCREMENT
...
#if !SQLITE_OMIT_VIRTUALTABLE
            if (sqlite3VtabCallConnect(pParse, pTable) != 0) return SQLITE_ERROR;
#endif
            if (IsVirtual(pTable))
                return 0;
...
return;
        ...
        */
        // OVERLOADS, so I don't need to rewrite parse.c
        private static Index sqlite3CreateIndex(Parse pParse, int null_2, int null_3, int null_4, int null_5,
        ...
        private static Index sqlite3CreateIndex(Parse pParse, int null_2, int null_3, int null_4, ExprList pList,
            int onError, int null_7, int null_8, int sortOrder, int ifNotExist)
        {
        ...
        */
            ...
            if (pTab.zName.StartsWith("sqlite_", StringComparison.InvariantCultureIgnoreCase)
                && !pTab.zName.StartsWith("sqlite_altertab_", StringComparison.InvariantCultureIgnoreCase))
            {
...
goto exit_drop_index;
        ...
        */
        private static void sqlite3IdListDelete(sqlite3 db, ref IdList pList)
        {
            ...
            if (pList == null)
                return;
            for (i = 0; i < pList.nId; i++) sqlite3DbFree(db, ref pList.a[i].zName);
        ...
        */
        ...
        private static SrcList sqlite3SrcListAppendFromTerm(Parse pParse, SrcList p, Token pTable,
            Token pDatabase, Token pAlias, int null_6, Expr pOn, IdList pUsing)
        {
...
|| sqlite3AuthCheck(pParse, SQLITE_SAVEPOINT, az[op], zName, 0)
                    ...
                    private static void sqlite3CodeVerifySchema(Parse pParse, int iDb)
                    {
                        ...
                        if (iDb >= 0)
                        {
                            ...
                            if ((pToplevel.cookieMask & mask) == 0)
                            {
                                ...
                                if (0 == OMIT_TEMPDB && iDb == 1) sqlite3OpenTempDatabase(pToplevel);
                            }
                        }
                    }
                    ...
                    */
                    private static void sqlite3HaltConstraint(Parse pParse, int onError, string p4,
                        int p4type)
                    {
                        var v = sqlite3GetVdbe(pParse);
                    ...
                    private static void sqlite3HaltConstraint(Parse pParse, int onError, byte[] p4,
                        int p4type)
                    {
                        var v = sqlite3GetVdbe(pParse);
                        if (onError == OE_Abort) sqlite3MayAbort(pParse);
                    ...
                    */
                    ...
                    private static bool collationMatch(string zColl, Index pIndex)
                    {
                        ...
                        for (i = 0; i < pIndex.nColumn; i++)
                        {
                            ...
                            Debug.Assert(z != null);
                            if (z.Equals(zColl, StringComparison.InvariantCultureIgnoreCase)) return true;
                        }
                    ...
                    private static void reindexDatabases(Parse pParse, string zColl)
                    {
                        ...
                        for (iDb = 0; iDb < db.nDb; iDb++) //, pDb++ )
                        {
                            ...
                            for (k = pDb.pSchema.tblHash.first;
                                 k != null;
                                 k = k
                                     .next) //for ( k = sqliteHashFirst( pDb.pSchema.tblHash ) ; k != null ; k = sqliteHashNext( k ) )
                            {
