    ...
    */
        ...
        */
        private static Trigger sqlite3TriggerList(Parse pParse, Table pTab)
        {
            ...
            if (pParse.disableTriggers != 0) return null;
      ...
      */
        ...
        */
        ...
        private static TriggerStep sqlite3TriggerInsertStep(sqlite3 db, Token pTableName, IdList pColumn,
            int null_4, int null_5, byte orconf)
        {
        ...
        private static TriggerStep sqlite3TriggerInsertStep(sqlite3 db, Token pTableName, IdList pColumn,
            int null_4, Select pSelect, byte orconf)
        {
        ...
        private static TriggerStep sqlite3TriggerInsertStep(
            ...
            byte orconf /* The conflict algorithm (OE_Abort, OE_Replace, etc.) */
        )
        {
            TriggerStep pTriggerStep;
        ...
        */
        private static TriggerStep sqlite3TriggerUpdateStep(
            ...
            byte orconf /* The conflict algorithm. (OE_Abort, OE_Ignore, etc) */
        )
        {
            TriggerStep pTriggerStep;
        ...
        **/
        private static void sqlite3DropTrigger(Parse pParse, SrcList pName, int noErr)
        {
            ...
            for (i = OMIT_TEMPDB; i < db.nDb; i++)
            {
                ...
                if (zDb != null &&
                    !db.aDb[j].zName.Equals(zDb, StringComparison.InvariantCultureIgnoreCase))
                    continue;
...
return;
      ...
      */
        ...
        */
        ...
        )
        {
            ...
            if ((pParse.db.flags & SQLITE_EnableTrigger) != 0) pList = sqlite3TriggerList(pParse, pTab);
        ...
        */
        ...
        )
        {
            ...
            if (iDb == 0 || iDb >= 2)
            {
                ...
                pSrc.a[pSrc.nSrc - 1].zDatabase =
                    db.aDb[iDb].zName; // sqlite3DbStrDup( db, db.aDb[iDb].zName );
            }
        ...
        */
        ...
        )
        {
            ...
            for (pStep = pStepList; pStep != null; pStep = pStep.pNext)
            {
                ...
                */
                pParse.eOrconf = orconf == OE_Default ? pStep.orconf : (byte)orconf;
                ...
                switch (pStep.op)
                {
                        ...
                        Debug.Assert(pStep.op == TK_SELECT);
                    {
                        ...
                        var pSelect = sqlite3SelectDup(db, pStep.pSelect, 0);
                ...
                if (pStep.op != TK_SELECT) sqlite3VdbeAddOp0(v, OP_ResetCount);
            }
    ...
    */
        ...
        */
        ...
        {
            var pTop = sqlite3ParseToplevel(pParse);
            ...
            var iEndTrigger = 0; /* Label to jump to if WHEN is false */
            ...
            ** OP_Halt inserted at the end of the program.  */
                ...
                if (iEndTrigger != 0) sqlite3VdbeResolveLabel(v, iEndTrigger);
        ...
        */
        ...
        {
            var pRoot = sqlite3ParseToplevel(pParse);
            TriggerPrg pPrg;
            ...
            ** in the Parse.pTriggerPrg list. Search for such an entry.  */
            ...
            if (null == pPrg) pPrg = codeRowTrigger(pParse, pTrigger, pTab, orconf);
        ...
        */
        ...
        {
            var v = sqlite3GetVdbe(pParse); /* Main VM */
            TriggerPrg pPrg;
            ...
            ** is a pointer to the sub-vdbe containing the trigger program.  */
            if (pPrg != null)
            {
                var bRecursive = !string.IsNullOrEmpty(p.zName) &&
                                 0 == (pParse.db.flags & SQLITE_RecTriggers);
                sqlite3VdbeAddOp3(v, OP_Program, reg, ignoreJump, ++pParse.nMem);
                sqlite3VdbeChangeP4(v, -1, pPrg.pProgram, P4_SUBPROGRAM);
#if SQLITE_DEBUG
                VdbeComment
                (v, "Call: %s.%s", !string.IsNullOrEmpty(p.zName) ? p.zName : "fkey",
                    onErrorText(orconf));
#endif
            ...
            ** is clear.  */
                sqlite3VdbeChangeP5(v, (byte)(bRecursive ? 1 : 0));
            }
        }
        ...
        */
        ...
        {
            var op = pChanges != null ? TK_UPDATE : TK_DELETE;
            uint mask = 0;
            Trigger p;
            ...
            for (p = pTrigger; p != null; p = p.pNext)
                   ...
                   )
                {
                    TriggerPrg pPrg;
                    pPrg = getRowTrigger(pParse, p, pTab, orconf);
                    if (pPrg != null) mask |= pPrg.aColmask[isNew];
                }
