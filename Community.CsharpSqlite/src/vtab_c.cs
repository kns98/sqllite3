    ...
    public partial class Sqlite3
    {
        ...
        */
        ...
        )
        {
            ...
            if (pMod != null)
            {
                ...
                pDel = sqlite3HashInsert(ref db.aModule, zCopy, nName, pMod);
        ...
        */
        private static void sqlite3VtabUnlock(VTable pVTab)
        {
            ...
            if (pVTab.nRef == 0)
            {
                ...
                if (p != null) ((sqlite3_vtab)p).pModule.xDisconnect(ref p);
        ...
        */
        private static void sqlite3VtabClear(sqlite3 db, Table p)
        {
            ...
            {
                int i;
                for (i = 0; i < p.nModuleArg; i++) sqlite3DbFree(db, ref p.azModuleArg[i]);
    ...
    sqlite3AuthCheck(pParse, SQLITE_CREATE_VTABLE, pTable->zName, 
        ...
        */
        private static void addArgumentToVtab(Parse pParse)
        {
            ...
            {
                var z = pParse.sArg.z.Substring(0, pParse.sArg.n);
                var n = pParse.sArg.n;
                var db = pParse.db;
        ...
        */
        ...
        {
            var pTab = pParse.pNewTable; /* The table being constructed */
            ...
            ** the required virtual table implementations are registered.  */
            ...
            {
                Table pOld;
                var pSchema = pTab.pSchema;
                var zName = pTab.zName;
        ...
        */
        ...
        {
            var sCtx = new VtabCtx();
            VTable pVTable;
            int rc;
            var azArg = pTab.azModuleArg;
            var nArg = pTab.nModuleArg;
            string zErr = null;
            var zModuleName = sqlite3MPrintf(db, "%s", pTab.zName);
            ...
            else if (ALWAYS(pVTable.pVtab))
            {
                ...
                ** the sqlite3_vtab object if successful.  */
                ...
                if (sCtx.pTab != null)
                {
                    var zFormat = "vtable constructor did not declare schema: %s";
                    ...
                    ** the type string.  */
                    ...
                    for (iCol = 0; iCol < pTab.nCol; iCol++)
                    {
                        if (string.IsNullOrEmpty(pTab.aCol[iCol].zType))
                            continue;
                        var zType = new StringBuilder(pTab.aCol[iCol].zType);
                        int nType;
                        var i = 0;
                        //if ( zType )
                        //  continue;
                        ...
                        {
                            int j;
                            var nDel = 6 + (zType.Length > i + 6 ? 1 : 0);
        ...
        */
        private static int sqlite3VtabCallConnect(Parse pParse, Table pTab)
        {
            ...
            if ((pTab.tabFlags & TF_Virtual) == 0 || sqlite3GetVTable(db, pTab) != null) return SQLITE_OK;

            /* Locate the required virtual table module */
            zMod = pTab.azModuleArg[0];
            pMod = sqlite3HashFind(db.aModule, zMod, sqlite3Strlen30(zMod), (Module)null);

            if (null == pMod)
            {
                var zModule = pTab.azModuleArg[0];
            ...
            else
            {
                ...
                if (rc != SQLITE_OK) sqlite3ErrorMsg(pParse, "%s", zErr);
        ...
        */
        private static int sqlite3VtabCallCreate(sqlite3 db, int iDb, string zTab, ref string pzErr)
        {
            ...
            ** create a valid sqlite3_vtab if it returns SQLITE_OK. */
            ...
            {
                rc = growVTrans(db);
                if (rc == SQLITE_OK) addToVTrans(db, sqlite3GetVTable(db, pTab));
            }
        ...
        */
        private static int sqlite3_declare_vtab(sqlite3 db, string zCreateTable)
        {
            ...
            //else
            {
                ...
                if (pParse.pVdbe != null) sqlite3VdbeFinalize(ref pParse.pVdbe);
        ...
        */
        private static int sqlite3VtabCallDestroy(sqlite3 db, int iDb, string zTab)
        {
            ...
            {
                var p = vtabDisconnectAll(db, pTab);
        ...
        */
        private static int sqlite3VtabSync(sqlite3 db, ref string pzErrmsg)
        {
            int i;
            var rc = SQLITE_OK;
            var aVTrans = db.aVTrans;

            db.aVTrans = null;
            for (i = 0; rc == SQLITE_OK && i < db.nVTrans; i++)
            {
                smdxFunction x; //int (*x)(sqlite3_vtab );
                var pVtab = aVTrans[i].pVtab;
        ...
        */
        private static int sqlite3VtabBegin(sqlite3 db, VTable pVTab)
        {
            ...
            */
            if (sqlite3VtabInSync(db)) return SQLITE_LOCKED;
            if (null == pVTab) return SQLITE_OK;
            ...
            if (pModule.xBegin != null)
            {
                ...
                ** sqlite3.aVTrans[] array. */
                ...
                if (rc == SQLITE_OK)
                {
                    ...
                    if (rc == SQLITE_OK) addToVTrans(db, pVTab);
                }
            }
        ...
        */
        ...
        private static readonly int[] aMap =
        {
            ...
            switch (op)
            {
                case SQLITE_VTAB_CONSTRAINT_SUPPORT:
                {
                    ...
                    else
                    {
                        ...
                        p.pVTable.bConstraint = (byte)va_arg(ap, 0);
                    }
