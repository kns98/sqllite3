...
** SQLite database.
*/
        private static readonly byte[] zMagicHeader = Encoding.UTF8.GetBytes(SQLITE_FILE_HEADER);
...
//#define get2byteNotZero(X)  (((((int)get2byte(X))-1)&0xffff)+1)
        ...
        {
            return ((get2byte(X, offset) - 1) & 0xffff) + 1;
        }
...
*/
        private static int saveCursorPosition(BtCursor pCur)
        {
            ...
            */
            ...
            {
                var pKey = sqlite3Malloc((int)pCur.nKey);
                //if( pKey !=null){
...
*/
        private static int ptrmapGet(BtShared pBt, uint key, ref byte pEType, ref uint pPgno)
        {
            var pDbPage = new PgHdr(); /* The pointer map page */
...
//OVERLOADS
        ...
        )
        {
            ...
            {
                if (pPage.hasData != 0)
                    n += (ushort)getVarint32(pCell, iCell + n, out nPayload);
                else
                    nPayload = 0;
                n += (ushort)getVarint(pCell, iCell + n, out pInfo.nKey);
...
CellInfo debuginfo = new CellInfo();
            ...
            if (nSize > pPage.maxLocal)
            {
                ...
                testcase(nSize == pPage.maxLocal);
                testcase(nSize == pPage.maxLocal + 1);
                if (nSize > pPage.maxLocal) nSize = (uint)minLocal;
                nSize += 4;
            }
...
*/
        private static int defragmentPage(MemPage pPage)
        {
            ...
            */
                if (pc < iCellFirst || pc > iCellLast) return SQLITE_CORRUPT_BKPT();
#endif
    ...
    if ( cbrk < iCellFirst || pc + size > usableSize )
    {
      return SQLITE_CORRUPT_BKPT();
...
#else
                if (cbrk < iCellFirst || pc + size > usableSize) return SQLITE_CORRUPT_BKPT();
#endif
            ...
            Debug.Assert(sqlite3PagerIswriteable(pPage.pDbPage));
            if (cbrk - iCellFirst != pPage.nFree) return SQLITE_CORRUPT_BKPT();
            return SQLITE_OK;
        }
...
*/
        private static int allocateSpace(MemPage pPage, int nByte, ref int pIdx)
        {
            ...
            uint usableSize; /* Usable size of the page */
            ...
            else if (gap + 2 <= top)
            {
                ...
                */
                ...
                for (addr = hdr + 1; (pc = get2byte(data, addr)) > 0; addr = pc)
                {
                    ...
                    if (pc > usableSize - 4 || pc < addr + 4) return SQLITE_CORRUPT_BKPT();
...
*/
        private static int freeSpace(MemPage pPage, uint start, int size)
        {
        ...
        private static int freeSpace(MemPage pPage, int start, int size)
        {
            ...
            */
            ...
            {
                if (pbegin < addr + 4) return SQLITE_CORRUPT_BKPT();
                addr = pbegin;
            }

            if (pbegin > iLast) return SQLITE_CORRUPT_BKPT();
            ...
            while ((pbegin = get2byte(data, addr)) > 0)
            {
                ...
                if (pbegin + psize + 3 >= pnext && pnext > 0)
                {
                    ...
                    if (frag < 0 || frag > data[hdr + 7]) return SQLITE_CORRUPT_BKPT();
                ...
                while (pc > 0)
                {
                    ...
                    if ((next > 0 && next <= pc + size + 3) || pc + size > usableSize)
                      ...
                      ** the free-block must lie on the database page.  */
                        ...
                        */
                        if (nFree > usableSize) return SQLITE_CORRUPT_BKPT();
                        ...
                        private static void zeroPage(MemPage pPage, int flags)
                        {
                            ...
                            if (pBt.secureDelete)
                                Array.Clear(data, hdr,
                                    (int)(pBt.usableSize -
                                          hdr)); //memset(&data[hdr], 0, pBt->usableSize - hdr);

                            data[hdr] = (byte)flags;
                            first = (ushort)(hdr + 8 + 4 * ((flags & PTF_LEAF) == 0 ? 1 : 0));
...
*/
                        private static uint btreePagecount(BtShared pBt)
                        {
                            return pBt.nPage;
                        }

                        private static uint sqlite3BtreeLastPage(Btree p)
                        {
                            Debug.Assert(sqlite3BtreeHoldsMutex(p));
...
*/
                                    ...
                                    )
                                    {
                                        ...
                                        sqlite3_mutex
                                            mutexOpen = null; /* Prevents a race condition. Ticket #3537 */
                                        ...
                                        var isTempDb =
                                            string.IsNullOrEmpty(zFilename); //zFilename==0 || zFilename[0]==0;
...
bool isMemdb = false;
                                        ...
                                        Debug.Assert((flags & BTREE_UNORDERED) == 0 ||
                                                     (flags & BTREE_SINGLE) != 0);

                                        /* A BTREE_SINGLE database is always a temporary and/or ephemeral */
                                        Debug.Assert((flags & BTREE_SINGLE) == 0 || isTempDb);

                                        if ((db.flags & SQLITE_NoReadlock) != 0) flags |= BTREE_NO_READLOCK;
...
p.sharable = 1;
                                        ...
                                        if (pBt == null)
                                        {
                                            ...
                                            */
                                            ...
                                            if (rc == SQLITE_OK)
                                                rc = sqlite3PagerReadFileheader(pBt.pPager, zDbHeader.Length,
                                                    zDbHeader);
...
pBt.secureDelete = true;
                                          ...
                                          */
                                            ...
                                            Debug.Assert((pBt.pageSize & 7) ==
                                                         0); /* 8-byte alignment of pageSize */
...
*/
                                    ...
                                    {
                                        if (null == pBt.pTmpSpace) pBt.pTmpSpace = sqlite3Malloc(pBt.pageSize);
                                    }
...
*/
                                    private static int sqlite3BtreeSetPageSize(Btree p, int pageSize,
                                        int nReserve, int iFix)
                                    {
                                        var rc = SQLITE_OK;
                                        var pBt = p.pBt;
                                        ...
                                        if (nReserve < 0) nReserve = (int)(pBt.pageSize - pBt.usableSize);
...
*/
                                    private static uint sqlite3BtreeMaxPageCount(Btree p, int mxPage)
                                    {
                                        uint n;
                                        sqlite3BtreeEnter(p);
...
*/
                                    private static int sqlite3BtreeSecureDelete(Btree p, int newFlag)
                                    {
                                        ...
                                        if (p == null)
                                            return 0;
                                        sqlite3BtreeEnter(p);
                                        if (newFlag >= 0) p.pBt.secureDelete = newFlag != 0;
...
*/
                                    private static int lockBtree(BtShared pBt)
                                    {
                                        ...
                                        uint nPage; /* Number of pages in the database */
                                        uint nPageFile = 0; /* Number of pages in the database file */
                                        uint nPageHeader; /* Number of pages in the database according to hdr */
                                        ...
                                        */
                                        nPage = nPageHeader =
                                            sqlite3Get4byte(pPage1.aData,
                                                28); //get4byte(28+(u8*)pPage1->aData);
                                        sqlite3PagerPagecount(pBt.pPager, out nPageFile);
                                        if (nPage == 0 ||
                                            memcmp(pPage1.aData, 24, pPage1.aData, 92, 4) !=
                                            0) //memcmp(24 + (u8*)pPage1.aData, 92 + (u8*)pPage1.aData, 4) != 0)
                                            nPage = nPageFile;
                                        if (nPage > 0)
                                        {
                                            uint pageSize;
...
rc = SQLITE_NOTADB;
                                        ...
                                        */
                                            if (memcmp(page1, 21, "\x0040\x0020\x0020", 3) !=
                                                0) //   "\100\040\040"
                                                goto page1_init_failed;
...
*/
                                    private static int newDatabase(BtShared pBt)
                                    {
                                        ...
                                        Debug.Assert(sqlite3_mutex_held(pBt.mutex));
                                        if (pBt.nPage > 0) return SQLITE_OK;
                                        pP1 = pBt.pPage1;
                                        ...
                                        if (rc != 0)
                                            return rc;
                                        Buffer.BlockCopy(zMagicHeader, 0, data, 0,
                                    ...
                                    private static int sqlite3BtreeBeginTrans(Btree p, int wrflag)
                                    {
                                        ...
                                        */
                                        if (p.inTrans == TRANS_WRITE ||
                                            (p.inTrans == TRANS_READ && 0 == wrflag)) goto trans_begun;
...
pBt.isExclusive = (u8)(wrflag>1);
                                          ...
                                          */
                                                if (pBt.nPage != sqlite3Get4byte(pPage1.aData, 28))
                                                {
                                                    ...
                                                    if (rc == SQLITE_OK)
                                                        sqlite3Put4byte(pPage1.aData, (uint)28, pBt.nPage);
                                                }
                                            }
                                        }
...
*/
                                            private static int setChildPtrmaps(MemPage pPage)
                                            {
                                                ...
                                                if (0 == pPage.leaf)
                                                {
                                                    var childPgno = sqlite3Get4byte(pPage.aData,
                                                        pPage.hdrOffset + 8);
                                                    ptrmapPut(pBt, childPgno, PTRMAP_BTREE, pgno, ref rc);
                                                }
...
*/
                                            private static int modifyPagePointer(MemPage pPage, uint iFrom,
                                                uint iTo, byte eType)
                                            {
                                                Debug.Assert(sqlite3_mutex_held(pPage.pBt.mutex));
                                                Debug.Assert(sqlite3PagerIswriteable(pPage.pDbPage));
                                                ...
                                                else
                                                {
                                                    ...
                                                    for (i = 0; i < nCell; i++)
                                                    {
                                                        ...
                                                        if (eType == PTRMAP_OVERFLOW1)
                                                        {
                                                            ...
                                                            btreeParseCellPtr(pPage, pCell, ref info);
                                                            if (info.iOverflow != 0)
                                                                if (iFrom == sqlite3Get4byte(pPage.aData, pCell,
                                                                        info.iOverflow))
                                                                {
                                                                    sqlite3Put4byte(pPage.aData,
                                                                        pCell + info.iOverflow, (int)iTo);
                                                                    break;
                                                                }
                                                        }
                                                        else
                                                    ...
                                                    if (i == nCell)
                                                    {
                                                        if (eType != PTRMAP_BTREE ||
                                                            sqlite3Get4byte(pPage.aData, pPage.hdrOffset + 8) !=
                                                            iFrom)
                                                            return SQLITE_CORRUPT_BKPT();
...
*/
                                            ...
                                            {
                                                var pPtrPage =
                                                    new MemPage(); /* The page that contains a pointer to pDbPage */
                                                ...
                                                */
                                                ...
                                                else
                                                {
                                                    ...
                                                    if (nextOvfl != 0)
                                                    {
                                                        ptrmapPut(pBt, nextOvfl, PTRMAP_OVERFLOW2, iFreePage,
                                                            ref rc);
                                                        if (rc != SQLITE_OK) return rc;
                                                    }
                                                }
...
*/
                                            private static int incrVacuumStep(BtShared pBt, uint nFin,
                                                uint iLastPg)
                                            {
                                                    ...
                                                    iLastPg != PENDING_BYTE_PAGE(pBt))
                                                {
                                                    ...
                                                    if (rc != SQLITE_OK) return rc;
                                                    if (eType == PTRMAP_ROOTPAGE) return SQLITE_CORRUPT_BKPT();
                                                    ...
                                                    if (eType == PTRMAP_FREEPAGE)
                                                    {
                                                        if (nFin == 0)
                                                        {
                                                            ...
                                                            */
                                                            ...
                                                            rc = allocateBtreePage(pBt, ref pFreePg,
                                                                ref iFreePg, iLastPg, 1);
                                                            if (rc != SQLITE_OK) return rc;
                                                    ...
                                                    {
                                                        uint iFreePg =
                                                            0; /* Index of free page to move pLastPg to */
                                                        ...
                                                        */
                                                        ...
                                                        if (rc == SQLITE_OK)
                                                            rc = relocatePage(pBt, pLastPg, eType, iPtrPage,
                                                                iFreePg, nFin != 0 ? 1 : 0);
                                                        releasePage(pLastPg);
                                                        if (rc != SQLITE_OK) return rc;
                                                    }
                                            ...
                                            private static int sqlite3BtreeIncrVacuum(Btree p)
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    if (rc == SQLITE_OK)
                                                    {
                                                        ...
                                                        sqlite3Put4byte(pBt.pPage1.aData, (uint)28,
                                                            pBt
                                                                .nPage); //put4byte(&pBt->pPage1->aData[28], pBt->nPage);
                                                    }
                                                }
...
#if !NDEBUG || DEBUG
                                                var nRef = sqlite3PagerRefcount(pPager);
#else
int nRef = 0;
#endif
                                                ...
                                                {
                                                    uint
                                                        nFin; /* Number of pages in database after autovacuuming */
                                                    ...
                                                    if (PTRMAP_ISPAGE(pBt, nOrig) ||
                                                      ...
                                                      */
                                                        ...
                                                        nPtrmap = (uint)((nFree - nOrig +
                                                                          PTRMAP_PAGENO(pBt, nOrig) + (uint)nEntry) /
                                                        nEntry);
                                                        nFin = nOrig - nFree - nPtrmap;
                                                        if (nOrig > PENDING_BYTE_PAGE(pBt) &&
                                                            nFin < PENDING_BYTE_PAGE(pBt)) nFin--;
                                                        ...
                                                        for (iFree = nOrig;
                                                             iFree > nFin && rc == SQLITE_OK;
                                                             iFree--) rc = incrVacuumStep(pBt, nFin, iFree);
                                                        if ((rc == SQLITE_DONE || rc == SQLITE_OK) && nFree > 0)
                                                        {
                                                            rc = sqlite3PagerWrite(pBt.pPage1.pDbPage);
                                                            sqlite3Put4byte(pBt.pPage1.aData, 32, 0);
                                                            sqlite3Put4byte(pBt.pPage1.aData, 36, 0);
                                                            sqlite3Put4byte(pBt.pPage1.aData, (uint)28, nFin);
...
*/
                                                        private static int sqlite3BtreeCommitPhaseOne(Btree p,
                                                            string zMaster)
                                                        {
                                                            var rc = SQLITE_OK;
                                                            if (p.inTrans == TRANS_WRITE)
...
** invalidated.
*/
                                                        private static void sqlite3BtreeTripAllCursors(
...
sqlite3BtreeTripAllCursors(p, rc);
                                                            ...
                                                            if (p.inTrans == TRANS_WRITE)
                                                            {
                                                                ...
                                                                ** sure pPage1.aData is set correctly. */
                                                                if (btreeGetPage(pBt, 1, ref pPage1, 0) ==
                                                                    SQLITE_OK)
                                                                {
                                                                    var nPage = sqlite3Get4byte(pPage1.aData,
                                                                        28);
                                                                    testcase(nPage == 0);
                                                                    if (nPage == 0)
...
*/
                                                        private static int sqlite3BtreeBeginStmt(Btree p,
                                                            int iStatement)
                                                        {
                                                            int rc;
                                                            var pBt = p.pBt;
                                                            ...
                                                            */
                                                            rc = sqlite3PagerOpenSavepoint(pBt.pPager,
                                                                iStatement);
                                                            sqlite3BtreeLeave(p);
                                                            return rc;
                                                        }
...
*/
                                                        private static int sqlite3BtreeSavepoint(Btree p,
                                                            int op, int iSavepoint)
                                                        {
                                                            var rc = SQLITE_OK;
                                                            if (p != null && p.inTrans == TRANS_WRITE)
                                                            {
                                                                ...
                                                                Debug.Assert(iSavepoint >= 0 ||
                                                                             (iSavepoint == -1 &&
                                                                              op == SAVEPOINT_ROLLBACK));
                                                                sqlite3BtreeEnter(p);
                                                                rc = sqlite3PagerSavepoint(pBt.pPager, op,
                                                                    iSavepoint);
                                                                if (rc == SQLITE_OK)
                                                                {
...
*/
                                                        private static int btreeCursor(
                                                            ...
                                                            KeyInfo
                                                                pKeyInfo, /* First arg to comparison function */
                                                        ...
                                                        )
                                                        {
                                                            ...
                                                            ** this lock.  */
                                                            Debug.Assert(hasSharedCacheTableLock(p,
                                                                (uint)iTable, pKeyInfo != null ? 1 : 0,
                                                                wrFlag + 1));
                                                            Debug.Assert(wrFlag == 0 ||
                                                                         !hasReadConflicts(p, (uint)iTable));
...
*/
                                                        ...
                                                        {
                                                            return
                                                                -1; // Not Used --  return ROUND8(sizeof(BtCursor));
                                                        }
...
*/
                                                        private static void sqlite3BtreeSetCachedRowid(
                                                            BtCursor pCur, long iRowid)
                                                        {
                                                            BtCursor p;
...
** zero is returned.
*/
                                                        private static long sqlite3BtreeGetCachedRowid(
                                                            BtCursor pCur)
                                                        {
...
*/
                                                        private static int sqlite3BtreeCloseCursor(
                                                            BtCursor pCur)
                                                        {
...
*/
                                                        ...
                                                        private static void assertCellInfo(BtCursor pCur)
                                                        {
                                                            ...
                                                            info =
                                                                new CellInfo(); //memset(info, 0, sizeof(info));
                                                            btreeParseCell(pCur.apPage[iPage],
                                                                pCur.aiIdx[iPage], ref info);
                                                            Debug.Assert(
                                                                info.GetHashCode() == pCur.info.GetHashCode() ||
                                                                info.Equals(
                                                                    pCur
                                                                        .info)); //memcmp(info, pCur.info, sizeof(info))==0 );
                                                        }
#else
...
/* Use a real function in MSVC to work around bugs in that compiler. */
                                                        private static void getCellInfo(BtCursor pCur)
                                                        {
                                                            if (pCur.info.nSize == 0)
                                                            {
                                                                int iPage = pCur.iPage;
                                                                btreeParseCell(pCur.apPage[iPage],
                                                                    pCur.aiIdx[iPage], ref pCur.info);
                                                                pCur.validNKey = true;
                                                            }
                                                            else
                                                            {
...
*/
                                                        private static bool sqlite3BtreeCursorIsValid(
                                                            BtCursor pCur)
                                                        {
...
*/
                                                        private static int sqlite3BtreeKeySize(BtCursor pCur,
                                                            ref long pSize)
                                                        {
                                                            Debug.Assert(cursorHoldsMutex(pCur));
...
*/
                                                        private static int sqlite3BtreeDataSize(BtCursor pCur,
                                                            ref uint pSize)
                                                        {
                                                            Debug.Assert(cursorHoldsMutex(pCur));
...
*/
                                                        private static int getOverflowPage(
                                                            ...
                                                            out MemPage
                                                                ppPage, /* OUT: MemPage handle (may be NULL) */
                                                            out uint
                                                        ...
                                                        )
                                                        {
                                                          ...
                                                          */
                                                            if (pBt.autoVacuum)
                                                            {
                                                                ...
                                                                {
                                                                    rc = ptrmapGet(pBt, iGuess, ref eType,
                                                                        ref pgno);
                                                                    if (rc == SQLITE_OK &&
...
*/
                                                        ...
                                                        )
                                                        {
                                                            if (eOp != 0)
                                                            {
                                                                ...
                                                                if (rc != SQLITE_OK) return rc;
                                                                Buffer.BlockCopy(pBuf, (int)pBufOffset,
                                                                    pPayload, (int)payloadOffset,
                                                                    (int)
                                                                    nByte); // memcpy( pPayload, pBuf, nByte );
                                                            }
                                                            else
                                                            {
                                                                /* Copy data from page to buffer (a read operation) */
                                                                Buffer.BlockCopy(pPayload, (int)payloadOffset,
                                                                    pBuf, (int)pBufOffset,
                                                                    (int)
                                                                    nByte); //memcpy(pBuf, pPayload, nByte);
                                                            }
...
*/
                                                        private static int accessPayload(
                                                            BtCursor
                                                                pCur, /* Cursor pointing to entry to read from */
                                                        ...
                                                        )
                                                        {
                                                            ...
                                                            uint nKey;
                                                            var iIdx = 0;
                                                            var pPage = pCur.apPage[
                                                                pCur.iPage]; /* Btree page of current entry */
                                                            var pBt = pCur
                                                                .pBt; /* Btree this cursor belongs to */
                                                            ...
                                                            getCellInfo(pCur);
                                                            aPayload = pCur.info
                                                                .pCell; //pCur.info.pCell + pCur.info.nHeader;
                                                            nKey = (uint)(pPage.intKey != 0
                                                                ? 0
                                                                : (int)pCur.info.nKey);
                                                            ...
                                                            if (NEVER(offset + amt > nKey + pCur.info.nData)
                                                                || pCur.info.nLocal >
                                                                pBt
                                                                    .usableSize //&aPayload[pCur.info.nLocal] > &pPage.aData[pBt.usableSize]
                                                               )
                                                            ...
                                                            if (offset < pCur.info.nLocal)
                                                            {
                                                                ...
                                                                rc = copyPayload(aPayload,
                                                                    (uint)(offset + pCur.info.iCell +
                                                                           pCur.info.nHeader), pBuf, pBufOffset,
                                                            ...
                                                            if (rc == SQLITE_OK && amt > 0)
                                                            {
                                                                ...
                                                                nextPage = sqlite3Get4byte(aPayload,
                                                                    pCur.info.nLocal + pCur.info.iCell +
                                                                    pCur.info.nHeader);
...
#endif
                                                                        rc = getOverflowPage(pBt, nextPage,
                                                                            out MemPageDummy, out nextPage);
                                                                        offset -= ovflSize;
                                                                    }
                                                                    else
                                                                    {
                                                                        ...
                                                                        */
                                                                        ...
                                                                        rc = sqlite3PagerGet(pBt.pPager,
                                                                            nextPage, ref pDbPage);
                                                                        if (rc == SQLITE_OK)
                                                                        {
                                                                            aPayload = sqlite3PagerGetData(
                                                                                pDbPage);
...
*/
                                                        private static int sqlite3BtreeKey(BtCursor pCur,
                                                            uint offset, uint amt, byte[] pBuf)
                                                        {
                                                            Debug.Assert(cursorHoldsMutex(pCur));
                                                            Debug.Assert(pCur.eState == CURSOR_VALID);
                                                            Debug.Assert(pCur.iPage >= 0 &&
                                                                         pCur.apPage[pCur.iPage] != null);
                                                            Debug.Assert(pCur.aiIdx[pCur.iPage] <
...
** the available payload.
*/
                                                        private static int sqlite3BtreeData(BtCursor pCur,
                                                            uint offset, uint amt, byte[] pBuf)
                                                        {
                                                            int rc;
...
*/
                                                        private static byte[] fetchPayload(
                                                            BtCursor
                                                                pCur, /* Cursor pointing to entry to read from */
                                                            ref int
                                                                pAmt, /* Write the number of available bytes here */
                                                        ...
                                                        )
                                                        {
                                                            ...
                                                            Debug.Assert(pCur != null && pCur.iPage >= 0 &&
                                                                         pCur.apPage[pCur.iPage] != null);
                                                            Debug.Assert(pCur.eState == CURSOR_VALID);
                                                            Debug.Assert(cursorHoldsMutex(pCur));
                                                            outOffset = -1;
                                                            ...
                                                            if (NEVER(pCur.info.nSize == 0))
                                                                btreeParseCell(pCur.apPage[pCur.iPage],
                                                                    pCur.aiIdx[pCur.iPage],
                                                                    ref pCur.info);
                                                            //aPayload = pCur.info.pCell;
                                                            //aPayload += pCur.info.nHeader;
                                                            aPayload = sqlite3Malloc(pCur.info.nSize -
                                                                pCur.info.nHeader);
                                                            if (pPage.intKey != 0)
                                                                nKey = 0;
                                                            else
                                                            ...
                                                            if (skipKey)
                                                            {
                                                                ...
                                                                outOffset = (int)(pCur.info.iCell +
                                                                    pCur.info.nHeader + nKey);
                                                                Buffer.BlockCopy(pCur.info.pCell, outOffset,
                                                                    aPayload, 0,
                                                                    (int)(pCur.info.nSize - pCur.info.nHeader -
                                                                          nKey));
...
*/
                                                            ...
                                                            BtCursor pCur, ref int pAmt, ref int outOffset)
                                                        {
                                                            ...
                                                            if (ALWAYS(pCur.eState == CURSOR_VALID))
                                                                p = fetchPayload(pCur, ref pAmt, ref outOffset,
                                                                    false);
                                                            return p;
                                                        }
...
** vice-versa).
*/
                                                        private static int moveToChild(BtCursor pCur,
                                                            uint newPgno)
                                                        {
                                                            int rc;
                                                            int i = pCur.iPage;
...
** the page.
*/
                                                        private static void assertParentIndex(MemPage pParent,
                                                            int iIdx, uint iChild)
                                                        {
                                                            Debug.Assert(iIdx <= pParent.nCell);
                                                            if (iIdx == pParent.nCell)
                                                                Debug.Assert(sqlite3Get4byte(pParent.aData,
                                                                    pParent.hdrOffset + 8) == iChild);
                                                            else
                                                                Debug.Assert(sqlite3Get4byte(pParent.aData,
                                                                    findCell(pParent, iIdx)) == iChild);
                                                        }
#else
...
*/
                                                        private static int moveToRoot(BtCursor pCur)
                                                        {
                                                            ...
                                                            {
                                                                rc = getAndInitPage(pBt, pCur.pgnoRoot,
                                                                    ref pCur.apPage[0]);
                                                                if (rc != SQLITE_OK)
                                                                {
                                                                    pCur.eState = CURSOR_INVALID;
                                                            ...
                                                            ** to the page.  */
                                                            ...
                                                            if (pRoot.nCell == 0 && 0 == pRoot.leaf)
                                                            {
                                                                ...
                                                                if (pRoot.pgno != 1)
                                                                    return SQLITE_CORRUPT_BKPT();
                                                                subpage = sqlite3Get4byte(pRoot.aData,
                                                                    pRoot.hdrOffset + 8);
                                                                pCur.eState = CURSOR_VALID;
...
*/
                                                        private static int moveToLeftmost(BtCursor pCur)
                                                        {
                                                                   ...
                                                                   0 == (pPage = pCur.apPage[pCur.iPage]).leaf)
                                                            {
                                                                Debug.Assert(pCur.aiIdx[pCur.iPage] <
                                                                             pPage.nCell);
                                                                pgno = sqlite3Get4byte(pPage.aData,
                                                                    findCell(pPage, pCur.aiIdx[pCur.iPage]));
...
*/
                                                        private static int sqlite3BtreeFirst(BtCursor pCur,
                                                            ref int pRes)
                                                        {
                                                            int rc;
                                                            ...
                                                            if (rc == SQLITE_OK)
                                                            {
                                                                ...
                                                                {
                                                                    Debug.Assert(pCur.apPage[pCur.iPage].nCell >
                                                                        0);
                                                                    pRes = 0;
...
** or set pRes to 1 if the table is empty.
*/
                                                        private static int sqlite3BtreeLast(BtCursor pCur,
                                                            ref int pRes)
                                                        {
                                                            int rc;
                                                            ...
                                                            ** to the last entry in the b-tree. */
                                                                ...
                                                                for (ii = 0; ii < pCur.iPage; ii++)
                                                                    Debug.Assert(pCur.aiIdx[ii] ==
                                                                        pCur.apPage[ii].nCell);
                                                                Debug.Assert(pCur.aiIdx[pCur.iPage] ==
                                                            ...
                                                            for (;;)
                                                            {
                                                                ...
                                                                ** a moveToChild() or moveToRoot() call would have detected corruption.  */
                                                                ...
                                                                if (biasRight != 0)
                                                                    ...
                                                                    pCur.aiIdx[pCur.iPage] =
                                                                        (ushort)(idx = (upr + lwr) / 2);
                                                                for (;;)
                                                                {
                                                                    int
                                                                        pCell; /* Pointer to current cell in pPage */
                                                                    ...
                                                                    if (pPage.intKey != 0)
                                                                    {
                                                                        ...
                                                                        {
                                                                            uint Dummy0 = 0;
                                                                            pCell += getVarint32(pPage.aData,
                                                                                pCell, out Dummy0);
                                                                        }

                                                                        getVarint(pPage.aData, pCell,
                                                                            out nCellKey);
                                                                        if (nCellKey == intKey)
                                                                        {
                                                                            c = 0;
                                                                    ...
                                                                    else
                                                                    {
                                                                        ...
                                                                        */
                                                                        ...
                                                                        else if (0 == (pPage.aData[pCell + 1] &
                                                                                 0x80) //!(pCell[1] & 0x80)
                                                                             && (nCell = ((nCell & 0x7f) << 7) +
                                                                                 pPage.aData[pCell + 1]) <=
                                                                             pPage
                                                                                 .maxLocal //pCell[1])<=pPage.maxLocal
                                                                            )
                                                                        {
                                                                        ...
                                                                        else
                                                                        {
                                                                            ...
                                                                            ** buffer before VdbeRecordCompare() can be called. */
                                                                            byte[] pCellKey;
                                                                            var pCellBody =
                                                                                new byte[pPage.aData.Length -
                                                                                    pCell + pPage.childPtrSize];
                                                                            Buffer.BlockCopy(pPage.aData,
                                                                                pCell - pPage.childPtrSize,
                                                                                pCellBody, 0,
                                                                                pCellBody
                                                                                    .Length); //          u8 * const pCellBody = pCell - pPage->childPtrSize;
                                                                            btreeParseCellPtr(pPage, pCellBody,
                                                                                ref pCur.info);
                                                                            ...
                                                                            //  goto moveto_finish;
                                                                            //}
                                                                            rc = accessPayload(pCur, 0,
                                                                                (uint)nCell, pCellKey, 0);
                                                                            if (rc != 0)
                                                                            {
                                                                                pCellKey =
                                                                                    null; // sqlite3_free(ref pCellKey );
                                                                                goto moveto_finish;
                                                                            }
                                                                ...
                                                                if (pPage.leaf != 0)
                                                                    ...
                                                                    chldPg = sqlite3Get4byte(pPage.aData,
                                                                        pPage.hdrOffset + 8);
                                                                else
                                                                    chldPg = sqlite3Get4byte(pPage.aData,
                                                                        findCell(pPage, lwr));
...
*/
                                                        private static int sqlite3BtreeNext(BtCursor pCur,
                                                            ref int pRes)
                                                        {
                                                            int rc;
                                                            int idx;
...
** this routine was called, then set pRes=1.
*/
                                                        private static int sqlite3BtreePrevious(BtCursor pCur,
                                                            ref int pRes)
                                                        {
                                                            int rc;
                                                            MemPage pPage;
...
*/
                                                        ...
                                                        )
                                                        {
                                                            ...
                                                            uint
                                                                k; /* Number of leaves on the trunk of the freelist */
                                                            MemPage pTrunk = null;
                                                            ...
                                                            if (n > 0)
                                                            {
                                                                ...
                                                                */
                                                                ...
                                                                if (exact != 0 && nearby <= mxPage)
                                                                {
                                                                    ...
                                                                    Debug.Assert(pBt.autoVacuum);
                                                                    uint Dummy0 = 0;
                                                                    rc = ptrmapGet(pBt, nearby, ref eType,
                                                                        ref Dummy0);
                                                                    if (rc != 0)
                                                                        return rc;
                                                                    if (eType == PTRMAP_FREEPAGE)
                                                            ...
                                                            */
                                                                ...
                                                                if (rc != 0)
                                                                    return rc;
                                                                sqlite3Put4byte(pPage1.aData, (uint)36, n - 1);
                                                                ...
                                                                */
                                                                do
                                                                {
                                                                    ...
                                                                    if (pPrevTrunk != null)
                                                                        ...
                                                                        iTrunk = sqlite3Get4byte(pPage1.aData,
                                                                            32);
                                                                    testcase(iTrunk == mxPage);
                                                                    if (iTrunk > mxPage)
                                                                        rc = SQLITE_CORRUPT_BKPT();
                                                                    else
                                                                        rc = btreeGetPage(pBt, iTrunk,
                                                                            ref pTrunk, 0);
                                                                    if (rc != 0)
                                                                    {
                                                                        pTrunk = null;
                                                                    ...
                                                                    k = sqlite3Get4byte(pTrunk.aData,
                                                                        4); /* # of leaves on this trunk page */
                                                                    if (k == 0 && 0 == searchList)
                                                                    {
                                                                        ...
                                                                        ** allocated page */
                                                                        ...
                                                                        pPgno = iTrunk;
                                                                        Buffer.BlockCopy(pTrunk.aData, 0,
                                                                            pPage1.aData, 32,
                                                                        ...
                                                                        */
                                                                        ...
                                                                        if (k == 0)
                                                                        {
                                                                            ...
                                                                            else
                                                                            {
                                                                                ...
                                                                                pPrevTrunk.aData[0 + 0] =
                                                                                    pTrunk.aData[0 + 0];
                                                                                pPrevTrunk.aData[0 + 1] =
                                                                                    pTrunk.aData[0 + 1];
                                                                                pPrevTrunk.aData[0 + 2] =
                                                                                    pTrunk.aData[0 + 2];
                                                                                pPrevTrunk.aData[0 + 3] =
                                                                                    pTrunk.aData[0 + 3];
                                                                            }
                                                                        }
                                                                        else
                                                                        {
                                                                            ...
                                                                            */
                                                                            ...
                                                                            pNewTrunk.aData[0 + 3] =
                                                                                pTrunk.aData[0 + 3];
                                                                            sqlite3Put4byte(pNewTrunk.aData,
                                                                                (uint)4, k - 1);
                                                                            Buffer.BlockCopy(pTrunk.aData, 12,
                                                                                pNewTrunk.aData, 8,
                                                                                (int)(k - 1) *
                                                                                4); //memcpy( pNewTrunk.aData[8], ref pTrunk.aData[12], ( k - 1 ) * 4 );
                                                                            releasePage(pNewTrunk);
                                                                            if (null == pPrevTrunk)
                                                                            {
                                                                                Debug.Assert(
                                                                    ...
                                                                    else if (k > 0)
                                                                    {
                                                                        ...
                                                                        if (nearby > 0)
                                                                        {
                                                                            ...
                                                                            dist = sqlite3AbsInt32(
                                                                                (int)(sqlite3Get4byte(aData,
                                                                                    8) - nearby));
                                                                            for (i = 1; i < k; i++)
                                                                            {
                                                                                var d2 = sqlite3AbsInt32(
                                                                                    (int)(sqlite3Get4byte(aData,
                                                                                        8 + i * 4) - nearby));
                                                                                if (d2 < dist)
                                                                                {
                                                                        ...
                                                                        if (0 == searchList || iPage == nearby)
                                                                        {
                                                                            ...
                                                                            pPgno = iPage;
                                                                            TRACE(
                                                                                "ALLOCATE: %d was leaf %d of %d on trunk %d" +
                                                                                ": %d more free pages\n",
                                                                                pPgno, closest + 1, k,
                                                                                pTrunk.pgno, n - 1);
                                                                            rc = sqlite3PagerWrite(
                                                                            ...
                                                                            if (rc != 0)
                                                                                goto end_allocate_page;
                                                                            if (closest < k - 1)
                                                                                Buffer.BlockCopy(aData,
                                                                                    (int)(4 + k * 4), aData,
                                                                                    8 + (int)closest * 4,
                                                                                    4); //memcpy( aData[8 + closest * 4], ref aData[4 + k * 4], 4 );
                                                                            sqlite3Put4byte(aData, (uint)4,
                                                                                k -
                                                                                1); // sqlite3Put4byte( aData, 4, k - 1 );
                                                                            noContent = !btreeGetHasContent(pBt,
                                                                                pPgno)
                                                                                ? 1
                                                                                : 0;
                                                                            rc = btreeGetPage(pBt, pPgno,
                                                                    ...
                                                                    PTRMAP_ISPAGE(pBt, pBt.nPage))
                                                                {
                                                                    ...
                                                                    */
                                                                    ...
                                                                    TRACE(
                                                                        ...
                                                                        pPgno);
                                                                    Debug.Assert(pBt.nPage !=
                                                                        PENDING_BYTE_PAGE(pBt));
                                                                    rc = btreeGetPage(pBt, pBt.nPage, ref pPg,
                                                                        1);
                                                                    if (rc == SQLITE_OK)
                                                                    {
                                                                        rc = sqlite3PagerWrite(pPg.pDbPage);
                                                            ...
                                                            if (rc == SQLITE_OK)
                                                            {
                                                                if (sqlite3PagerPageRefcount(ppPage.pDbPage) >
                                                                    1)
                                                                {
                                                                    releasePage(ppPage);
                                                                    return SQLITE_CORRUPT_BKPT();
                                                            ...
                                                            Debug.Assert(rc != SQLITE_OK ||
                                                                         sqlite3PagerIswriteable(ppPage.pDbPage));
                                                            return rc;
                                                        }
...
*/
                                                        private static int freePage2(BtShared pBt,
                                                            MemPage pMemPage, uint iPage)
                                                        {
                                                            MemPage pTrunk = null; /* Free-list trunk page */
                                                            uint iTrunk =
                                                                0; /* Page number of free-list trunk page */
                                                            var pPage1 =
                                                                pBt.pPage1; /* Local reference to page 1 */
                                                            MemPage pPage; /* Page being freed. May be NULL. */
                                                            int rc; /* Return Code */
                                                            int
                                                                nFree; /* Initial number of pages on free-list */
                                                            ...
                                                            if (pBt.secureDelete)
                                                            {
                                                                ...
                                                                */
                                                                if ((null == pPage &&
                                                                     (rc = btreeGetPage(pBt, iPage, ref pPage,
                                                                         0)) != 0)
                                                                    || (rc = sqlite3PagerWrite(
                                                                        pPage.pDbPage)) != 0
                                                                   )
                                                                    goto freepage_out;
                                                                Array.Clear(pPage.aData, 0,
                                                                    (int)pPage.pBt
                                                                        .pageSize); //memset(pPage->aData, 0, pPage->pBt->pageSize);
                                                            }
...
if (false)
#endif
                                                            {
                                                                ptrmapPut(pBt, iPage, PTRMAP_FREEPAGE, 0,
                                                                    ref rc);
                                                                if (rc != 0)
                                                                    goto freepage_out;
                                                            }
                                                            ...
                                                            */
                                                            ...
                                                            {
                                                                uint
                                                                    nLeaf; /* Initial number of leaf cells on trunk page */
                                                                ...
                                                                if (nLeaf < pBt.usableSize / 4 - 8)
                                                                {
                                                                    ...
                                                                    */
                                                                    ...
                                                                    if (rc == SQLITE_OK)
                                                                    {
                                                                        sqlite3Put4byte(pTrunk.aData, (uint)4,
                                                                            nLeaf + 1);
                                                                        sqlite3Put4byte(pTrunk.aData,
                                                                            8 + nLeaf * 4, iPage);
                                                            ...
                                                            */
                                                            ...
                                                            sqlite3Put4byte(pPage1.aData, (uint)32, iPage);
...
*/
                                                        private static int clearCell(MemPage pPage, int pCell)
                                                        {
                                                            ...
                                                            if (info.iOverflow == 0)
                                                                return
                                                                    SQLITE_OK; /* No overflow pages. Return without doing anything */
                                                            ovflPgno = sqlite3Get4byte(pPage.aData, pCell,
                                                                info.iOverflow);
...
*/
                                                                        private static int fillInCell(
                                                                            MemPage
                                                                                pPage, /* The page that contains the cell */
                                                                            byte[]
                                                                        ...
                                                                        )
                                                                        {
                                                                            ...
                                                                            ** buffer space that is separate from the pPage buffer area */
                                                                            ...
                                                                            if (pPage.hasData != 0)
                                                                                nHeader += putVarint(pCell,
                                                                                    nHeader,
                                                                                    nData +
                                                                                    nZero); //putVarint( pCell[nHeader], nData + nZero );
                                                                            else
                                                                                nData = nZero = 0;
                                                                            nHeader += putVarint(pCell, nHeader,
                                                                                (ulong)
                                                                                nKey); //putVarint( pCell[nHeader], *(u64*)&nKey );
                                                                            btreeParseCellPtr(pPage, pCell,
                                                                                ref info);
                                                                            Debug.Assert(
                                                                            ...
                                                                            while (nPayload > 0)
                                                                            {
...
#if !SQLITE_OMIT_AUTOVACUUM
                                                                                    var pgnoPtrmap =
                                                                                        pgnoOvfl; /* Overflow page pointer-map entry page */
                                                                                    if (pBt.autoVacuum)
                                                                                        do
                                                                                        {
                                                                                            pgnoOvfl++;
...
#endif
                                                                                    rc = allocateBtreePage(pBt,
                                                                                        ref pOvfl, ref pgnoOvfl,
                                                                                        pgnoOvfl, 0);
#if !SQLITE_OMIT_AUTOVACUUM
                                                                              ...
                                                                              */
                                                                                    ...
                                                                                    ** is still writeable */
                                                                                    ...
                                                                                    sqlite3Put4byte(pPrior,
                                                                                        pPriorIndex, pgnoOvfl);
                                                                                    releasePage(pToRelease);
                                                                                    pToRelease = pOvfl;
                                                                                    pPrior = pOvfl.aData;
                                                                                    pPriorIndex = 0;
                                                                                ...
                                                                                ** is still writeable */
                                                                                ...
                                                                                if (nSrc > 0)
                                                                                {
                                                                                    ...
                                                                                    Buffer.BlockCopy(pSrc,
                                                                                        pSrcIndex, pPayload,
                                                                                        pPayloadIndex,
                                                                                        n); //memcpy(pPayload, pSrc, n);
                                                                                }
                                                                                else
                                                                                {
                                                                                    var pZeroBlob =
                                                                                        sqlite3Malloc(
                                                                                            n); // memset(pPayload, 0, n);
                                                                                    Buffer.BlockCopy(pZeroBlob,
                                                                                        0, pPayload,
                                                                                        pPayloadIndex, n);
                                                                                }
...
*/
                                                                        private static void dropCell(
                                                                            MemPage pPage, int idx, int sz,
                                                                            ref int pRC)
                                                                        {
                                                                            uint
                                                                                pc; /* Offset to cell content of cell being deleted */
                                                                            byte[] data; /* pPage.aData */
                                                                            int
                                                                                ptr; /* Used to move bytes around within data[] */
                                                                            int endPtr; /* End of loop */
                                                                            int rc; /* The return code */
                                                                            int
                                                                                hdr; /* Beginning of the header.  0 most pages.  100 page 1 */
                                                                            ...
                                                                            Debug.Assert(idx >= 0 &&
                                                                                idx < pPage.nCell);
#if SQLITE_DEBUG
                                                                            Debug.Assert(sz == cellSize(pPage,
                                                                                idx));
#endif
                                                                            Debug.Assert(
                                                                                sqlite3PagerIswriteable(
                                                                            ...
                                                                            ptr = pPage.cellOffset +
                                                                                2 *
                                                                                idx; //ptr = &data[pPage.cellOffset + 2 * idx];
                                                                            pc = (uint)get2byte(data, ptr);
                                                                            hdr = pPage.hdrOffset;
                                                                            testcase(pc == get2byte(data,
                                                                                hdr + 5));
                                                                            testcase(pc + sz ==
                                                                                pPage.pBt.usableSize);
                                                                            if (pc < (uint)get2byte(data,
                                                                                    hdr + 5) || pc + sz >
                                                                                pPage.pBt.usableSize)
                                                                            {
                                                                                pRC = SQLITE_CORRUPT_BKPT();
                                                                            ...
                                                                            data[pPage.hdrOffset + 4] =
                                                                                (byte)pPage
                                                                                    .nCell; //put2byte( data, hdr + 3, pPage.nCell );
                                                                            pPage.nFree += 2;
                                                                        }
...
*/
                                                                        private static void insertCell(
                                                                            MemPage
                                                                                pPage, /* Page into which we are copying */
                                                                            ...
                                                                            uint
                                                                                iChild, /* If non-zero, replace first 4 bytes with this value */
                                                                            ref int
                                                                                pRC /* Read and write return code from here */
                                                                        )
                                                                        {
                                                                            var idx =
                                                                            ...
                                                                            int j; /* Loop counter */
                                                                            int
                                                                                end; /* First byte past the last cell pointer in data[] */
                                                                            int
                                                                                ins; /* Index in data[] where new cell pointer is inserted */
                                                                            int
                                                                                cellOffset; /* Address of first cell pointer in data[] */
                                                                            byte[]
                                                                                data; /* The content of the whole page */
                                                                            byte
                                                                                ptr; /* Used for moving information around in data[] */
                                                                            ...
                                                                            Debug.Assert(pPage.nOverflow <=
                                                                                ArraySize(pPage.aOvfl));
                                                                            Debug.Assert(
                                                                                sqlite3_mutex_held(
                                                                                    pPage.pBt.mutex));
                                                                            ...
                                                                            ** the term after the || in the following assert(). */
                                                                                ...
                                                                                sz + 2 > pPage.nFree)
                                                                            {
                                                                                if (pTemp != null)
                                                                                {
                                                                                    Buffer.BlockCopy(pCell,
                                                                                        nSkip, pTemp, nSkip,
                                                                                        sz -
                                                                                        nSkip); //memcpy(pTemp+nSkip, pCell+nSkip, sz-nSkip);
                                                                                    pCell = pTemp;
                                                                                }

                                                                                if (iChild != 0)
                                                                                    sqlite3Put4byte(pCell,
                                                                                        iChild);
                                                                                j = pPage.nOverflow++;
                                                                                Debug.Assert(
                                                                                    j < pPage.aOvfl
                                                                                        .Length); //(int)(sizeof(pPage.aOvfl)/sizeof(pPage.aOvfl[0])) );
                                                                                pPage.aOvfl[j].pCell = pCell;
                                                                                pPage.aOvfl[j].idx = (ushort)i;
                                                                            }
                                                                            else
                                                                            {
                                                                                var rc = sqlite3PagerWrite(
                                                                                    pPage.pDbPage);
                                                                                if (rc != SQLITE_OK)
                                                                                {
                                                                                    pRC = rc;
                                                                                ...
                                                                                ** if it returns success */
                                                                                ...
                                                                                Debug.Assert(idx + sz <=
                                                                                    (int)pPage.pBt.usableSize);
                                                                                pPage.nCell++;
                                                                                pPage.nFree -= (ushort)(2 + sz);
                                                                                Buffer.BlockCopy(pCell, nSkip,
                                                                                    data, idx + nSkip,
                                                                                    sz -
                                                                                    nSkip); //memcpy( data[idx + nSkip], pCell + nSkip, sz - nSkip );
                                                                                if (iChild != 0)
                                                                                    sqlite3Put4byte(data, idx,
                                                                                        iChild);
                                                                                //ptr = &data[end];
                                                                                //endPtr = &data[ins];
                                                                                //assert( ( SQLITE_PTR_TO_INT( ptr ) & 1 ) == 0 );  /* ptr is always 2-byte aligned */
...
*/
                                                                                    private static void
                                                                                        assemblePage(
                                                                                            MemPage
                                                                                                pPage, /* The page to be assemblied */
                                                                                            int
                                                                                                nCell, /* The number of cells to add to this page */
                                                                                            byte[]
                                                                                                apCell, /* Pointer to a single the cell bodies */
                                                                                            int[]
                                                                                                aSize /* Sizes of the cells bodie*/
                                                                                        )
                                                                                    {
                                                                                        int
                                                                                            i; /* Loop counter */
                                                                                        int
                                                                                            pCellptr; /* Address of next cell pointer */
                                                                                        int
                                                                                            cellbody; /* Address of next cell body */
                                                                                        var data = pPage
                                                                                            .aData; /* Pointer to data for pPage */
                                                                                        int hdr = pPage
                                                                                            .hdrOffset; /* Offset of header on pPage */
                                                                                        var nUsable =
                                                                                            (int)pPage.pBt
                                                                                                .usableSize; /* Usable size of page */
                                                                                        ...
                                                                                        pCellptr = pPage
                                                                                                .cellOffset +
                                                                                            nCell *
                                                                                            2; //data[pPage.cellOffset + nCell * 2];
                                                                                        cellbody = nUsable;
                                                                                        for (i = nCell - 1;
                                                                                         i >= 0;
                                                                                         i--)
                                                                                        {
                                                                                            ...
                                                                                            Buffer.BlockCopy(
                                                                                                apCell, 0, data,
                                                                                                cellbody,
                                                                                                sz); // memcpy(&data[cellbody], apCell[i], sz);
                                                                                        }
                                                                                    ...
                                                                                    private static void
                                                                                        assemblePage(
                                                                                            ...
                                                                                            int
                                                                                                offset /* Offset into the cell bodies, for c#  */
                                                                                        )
                                                                                    {
                                                                                        int
                                                                                            i; /* Loop counter */
                                                                                         ...
                                                                                         i--)
                                                                                        {
                                                                                            ...
                                                                                            Buffer.BlockCopy(
                                                                                                ...
                                                                                                aSize[
                                                                                                    i +
                                                                                                    offset]); //          memcpy(&data[cellbody], apCell[i], aSize[i]);
                                                                                        }
                                                                                        ...
                                                                                        )
                                                                                    {
                                                                                         ...
                                                                                         i--)
                                                                                        {
                                                                                            ...
                                                                                            Buffer.BlockCopy(
                                                                                                ...
                                                                                                cellbody,
                                                                                                aSize[
                                                                                                    i]); //memcpy( data[cellbody], apCell[i], aSize[i] );
                                                                                        }
...
*/
                                                                                    private static readonly int
                                                                                        NN =
                                                                                            1; /* Number of neighbors on either side of pPage */

                                                                                    private static readonly int
                                                                                        NB = NN * 2 +
                                                                                            1; /* Total pages involved in the balance */
...
*/
                                                                                            ...
                                                                                            byte[] pSpace)
                                                                                    {
                                                                                        ...
                                                                                        var pNew =
                                                                                            new
                                                                                                MemPage(); /* Newly allocated page */
                                                                                        int
                                                                                            rc; /* Return Code */
                                                                                        uint pgnoNew =
                                                                                            0; /* Page number of pNew */
                                                                                            ...
                                                                                            */
                                                                                            var iCell =
                                                                                                findCell(pPage,
                                                                                                    pPage
                                                                                                        .nCell -
                                                                                                    1); //pCell = findCell( pPage, pPage.nCell - 1 );
                                                                                            pCell = pPage.aData;
                                                                                            var _pCell = iCell;
                                                                                            pStop = _pCell +
                                                                                                9; //pStop = &pCell[9];
                                                                                            ...
                                                                                            ref int pRC)
                                                                                    {
                                                                                        if (pRC == SQLITE_OK)
                                                                                        {
                                                                                            ...
                                                                                            Buffer.BlockCopy(
                                                                                                ...
                                                                                                (int)pBt
                                                                                                    .usableSize -
                                                                                                iData); //memcpy(aTo[iData], ref aFrom[iData], pBt.usableSize-iData);
                                                                                            Buffer.BlockCopy(
                                                                                                aFrom, iFromHdr,
                                                                                                aTo, iToHdr,
                                                                                                pFrom
                                                                                                    .cellOffset +
                                                                                                2 * pFrom
                                                                                                    .nCell); //memcpy(aTo[iToHdr], ref aFrom[iFromHdr], pFrom.cellOffset + 2*pFrom.nCell);
...
// under C#; Try to reuse Memory
                                                                                    ...
                                                                                    private static int
                                                                                        balance_nonroot(
                                                                                            MemPage
                                                                                                pParent, /* Parent page of siblings being balanced */
                                                                                            int
                                                                                                iParentIdx, /* Index of "the page" in pParent */
                                                                                            byte[]
                                                                                                aOvflSpace, /* page-size bytes of space for parent ovfl */
                                                                                            int
                                                                                                isRoot /* True if pParent is a root-page */
                                                                                        )
                                                                                    {
                                                                                        var apOld =
                                                                                            new MemPage
                                                                                                [NB]; /* pPage and up to two siblings */
                                                                                        var apCopy =
                                                                                            new MemPage
                                                                                                [NB]; /* Private copies of apOld[] pages */
                                                                                        var apNew =
                                                                                            new MemPage[NB +
                                                                                                2]; /* pPage and up to NB siblings after balancing */
                                                                                        var apDiv = new int[NB -
                                                                                            1]; /* Divider cells in pParent */
                                                                                        var cntNew =
                                                                                            new int[NB +
                                                                                                2]; /* Index in aCell[] of cell after i-th page */
                                                                                        var szNew = new int[NB +
                                                                                            2]; /* Combined size of cells place on i-th page */
                                                                                        var szCell =
                                                                                            new ushort
                                                                                                [1]; /* Local size of all cells in apCell[] */
                                                                                        BtShared
                                                                                            pBt; /* The whole database */
                                                                                        var nCell =
                                                                                            0; /* Number of cells in apCell[] */
                                                                                        var nMaxCells =
                                                                                            0; /* Allocated size of apCell, szCell, aFrom. */
                                                                                        var nNew =
                                                                                            0; /* Number of pages in apNew[] */
                                                                                        int
                                                                                            nOld; /* Number of pages in apOld[] */
                                                                                        int i,
                                                                                            j,
                                                                                            k; /* Loop counters */
                                                                                        int
                                                                                            nxDiv; /* Next divider slot in pParent.aCell[] */
                                                                                        var rc =
                                                                                            SQLITE_OK; /* The return code */
                                                                                        ushort
                                                                                            leafCorrection; /* 4 if pPage is a leaf.  0 if not */
                                                                                        int
                                                                                            leafData; /* True if pPage is a leaf of a LEAFDATA tree */
                                                                                        int
                                                                                            usableSpace; /* Bytes in pPage beyond the header */
                                                                                        int
                                                                                            pageFlags; /* Value of pPage.aData[0] */
                                                                                        int
                                                                                            subtotal; /* Subtotal of bytes in cells on one page */
                                                                                        //int iSpace1 = 0;             /* First unused byte of aSpace1[] */
                                                                                        var iOvflSpace =
                                                                                            0; /* First unused byte of aOvflSpace[] */
                                                                                        int
                                                                                            szScratch; /* Size of scratch memory requested */
                                                                                        int
                                                                                            pRight; /* Location in parent of right-sibling pointer */
                                                                                        byte[][] apCell =
                                                                                            null; /* All cells begin balanced */
                                                                                        //u16[] szCell;                         /* Local size of all cells in apCell[] */
                                                                                        //u8[] aSpace1;                         /* Space for copies of dividers cells */
                                                                                        uint
                                                                                            pgno; /* Temp var to store a page number in */
                                                                                      ...
                                                                                      */
                                                                                        ...
                                                                                        */
                                                                                        ...
                                                                                        if (i + nxDiv -
                                                                                         ...
                                                                                         pParent.nCell)
                                                                                            pRight = pParent
                                                                                                    .hdrOffset +
                                                                                                8; //&pParent.aData[pParent.hdrOffset + 8];
                                                                                        else
                                                                                            pRight = findCell(
                                                                                                pParent,
                                                                                                i + nxDiv -
                                                                                        ...
                                                                                        */
                                                                                        ...
                                                                                        if (szCell.Length <
                                                                                         nMaxCells)
                                                                                            Array.Resize(
                                                                                                ref szCell,
                                                                                                nMaxCells); //(u16*)&apCell[nMaxCells];
                                                                                        ...
                                                                                        */
                                                                                         ...
                                                                                         i--)
                                                                                        {
                                                                                            var szRight =
                                                                                                szNew[
                                                                                                    i]; /* Size of sibling on the right */
                                                                                            var szLeft =
                                                                                                szNew[
                                                                                                    i -
                                                                                                    1]; /* Size of sibling on the left */
                                                                                            int
                                                                                                r; /* Index of right-most cell in left sibling */
                                                                                            int
                                                                                                d; /* Index of first cell to the left of right sibling */
                                                                                            ...
                                                                                            ** actually moved between pages.  */
                                                                                            ...
                                                                                            var iOverflow =
                                                                                                nOverflow != 0
                                                                                                    ...
                                                                                                    : -1;
                                                                                            j =
                                                                                                0; /* Current 'old' sibling page */
                                                                                            k =
                                                                                                0; /* Current 'new' sibling page */
                                                                                            for (i = 0;
                                                                                             i < nCell;
                                                                                             i++)
                                                                                            {
...
*/
                                                                                    ...
                                                                                    {
                                                                                        int
                                                                                            rc; /* Return value from subprocedures */
                                                                                        MemPage pChild =
                                                                                            null; /* Pointer to a new child page */
                                                                                        uint pgnoChild =
...
if (false)
                                                                                        ...
                                                                                        Array.Copy(pRoot.aOvfl,
                                                                                            pChild.aOvfl,
                                                                                            pRoot
                                                                                                .nOverflow); //memcpy(pChild.aOvfl, pRoot.aOvfl, pRoot.nOverflow*sizeof(pRoot.aOvfl[0]));
                                                                                        pChild.nOverflow =
                                                                                            pRoot.nOverflow;
                                                                                        ...
                                                                                        BtCursor pCur)
                                                                                    {
                                                                                        ...
                                                                                        var
                                                                                            balance_quick_called =
                                                                                                0; //TESTONLY( int balance_quick_called = 0 );
                                                                                        var
                                                                                            balance_deeper_called =
                                                                                                0; //TESTONLY( int balance_deeper_called = 0 );
#else
                                                                                                        ...
                                                                                                        var
                                                                                                            pSpace =
                                                                                                                new
                                                                                                                    ...
                                                                                                                    [pCur
                                                                                                                        .pBt
                                                                                                                        .pageSize]; // u8 pSpace = sqlite3PageMalloc( pCur.pBt.pageSize );
                                                                                                        rc =
                                                                                                            balance_nonroot(
                                                                                                                pParent,
                                                                                                                iIdx,
...
*/
                                                                                    private static int
                                                                                        sqlite3BtreeInsert(
                                                                                            BtCursor
                                                                                                pCur, /* Insert data into the table of this cursor */
                                                                                            byte[] pKey,
                                                                                            long
                                                                                            ...
                                                                                            int
                                                                                                appendBias, /* True if this is likely an append */
                                                                                            int
                                                                                                seekResult /* Result of prior MovetoUnpacked() call */
                                                                                        )
                                                                                    {
                                                                                        int rc;
                                                                                        var loc =
                                                                                            seekResult; /* -1: before desired location  +1: after */
                                                                                        var szNew = 0;
                                                                                        int idx;
                                                                                        MemPage pPage;
                                                                                        var p = pCur.pBtree;
...
*/
                                                                                            ...
                                                                                            BtCursor pCur)
                                                                                    {
                                                                                        ...
                                                                                        int
                                                                                            rc; /* Return code */
                                                                                        MemPage
                                                                                            pPage; /* Page to delete cell from */
                                                                                        int
                                                                                            pCell; /* Pointer to cell to delete */
                                                                                        int
                                                                                            iCellIdx; /* Index of cell to delete */
                                                                                        int
                                                                                            iCellDepth; /* Depth of node containing pCell */
                                                                                        ...
                                                                                        if (NEVER(pCur.aiIdx[
                                                                                            ...
                                                                                            return
                                                                                                SQLITE_ERROR; /* Something has gone awry. */
                                                                                        ...
                                                                                        ** node to replace the deleted cell.  */
                                                                                        if (0 == pPage.leaf)
                                                                                        {
                                                                                            ...
                                                                                            insertCell(pPage,
                                                                                                ...
                                                                                                n,
                                                                                                ref
                                                                                                rc); //insertCell( pPage, iCellIdx, pCell - 4, nCell + 4, pTmp, n, ref rc );
                                                                                            dropCell(pLeaf,
                                                                                                pLeaf.nCell - 1,
                                                                                                nCell, ref rc);
                                                                                            if (rc != 0)
...
*/
                                                                                            ...
                                                                                            int createTabFlags)
                                                                                    {
                                                                                        ...
                                                                                        int rc;
                                                                                        int
                                                                                            ptfFlags; /* Page-type flage for the root page of new table */
...
return rc;
                                                                                        ...
                                                                                        {
                                                                                            uint pgnoMove =
                                                                                                0; /* Move a page here to make room for the root-page */
                                                                                            var pPageMove =
                                                                                                new
                                                                                                    MemPage(); /* The page to move to. */
...
*/
                                                                                    private static int
                                                                                        clearDatabasePage(
                                                                                            BtShared
                                                                                                pBt, /* The BTree that contains the table */
                                                                                            uint
                                                                                                pgno, /* Page number to clear */
                                                                                            int
                                                                                                freePageFlag, /* Deallocate page if true */
                                                                                            ref int
                                                                                                pnChange /* Add number of Cells freed to this counter */
                                                                                        )
                                                                                    {
                                                                                        var pPage =
                                                                                            new MemPage();
...
*/
                                                                                            ...
                                                                                            ref long pnEntry)
                                                                                    {
                                                                                        ...
                                                                                        */
                                                                                        ...
                                                                                        {
                                                                                            int
                                                                                                iIdx; /* Index of child node in parent */
                                                                                            MemPage
                                                                                                pPage; /* Current page of the b-tree */
...
*/
                                                                                    private static void
                                                                                        checkPtrmap(
                                                                                            IntegrityCk
                                                                                                pCheck, /* Integrity check context */
                                                                                            uint
                                                                                                iChild, /* Child page number */
                                                                                            byte
                                                                                                eType, /* Expected pointer map type */
                                                                                            uint
                                                                                                iParent, /* Expected pointer map parent page number */
                                                                                            string
                                                                                                zContext /* Context description (used for error msg) */
                                                                                        )
                                                                                    {
                                                                                        int rc;
                                                                                        byte ePtrmapType = 0;
...
*/
                                                                                    private static void
                                                                                        checkList(
                                                                                            IntegrityCk
                                                                                                pCheck, /* Integrity checking context */
                                                                                            int
                                                                                                isFreeList, /* True for a freelist.  False for overflow page list */
                                                                                            int
                                                                                                iPage, /* Page number for first page in the list */
                                                                                            int N, /* Expected number of pages in the list */
                                                                                            string
                                                                                                zContext /* Context for error messages */
                                                                                        )
                                                                                    {
                                                                                        int i;
                                                                                        var expected = N;
...
*/
                                                                                    ...
                                                                                    private static long
                                                                                        refNULL; //Dummy for C# ref NULL

                                                                                    private static int
                                                                                        checkTreePage(
                                                                                            IntegrityCk
                                                                                                pCheck, /* Context for the sanity check */
                                                                                            int
                                                                                                iPage, /* Page number of the page to check */
                                                                                            string
                                                                                                zParentContext, /* Parent context */
                                                                                            ref long
                                                                                                pnParentMinKey,
                                                                                            ref long
                                                                                                pnParentMaxKey,
                                                                                            object
                                                                                                _pnParentMinKey, /* C# Needed to determine if content passed*/
                                                                                            object
                                                                                                _pnParentMaxKey /* C# Needed to determine if content passed*/
                                                                                        )
                                                                                    {
                                                                                        var pPage =
                                                                                            new MemPage();
                                                                                        ...
                                                                                        ** btreeInitPage() is executed.  */
                                                                                             ...
                                                                                             pPage)) != 0)
                                                                                        {
                                                                                            Debug.Assert(
                                                                                                rc ==
                                                                                                SQLITE_CORRUPT); /* The only possible error from InitPage */
                                                                                            checkAppendMsg(
                                                                                                pCheck,
                                                                                                zContext
                                                                                                    .ToString(),
                                                                                        ...
                                                                                        */
                                                                                         ...
                                                                                         i++)
                                                                                        {
                                                                                            ...
                                                                                            */
                                                                                            ...
                                                                                            var iCell =
                                                                                                findCell(pPage,
                                                                                                    i); //pCell = findCell( pPage, i );
                                                                                            pCell = pPage.aData;
                                                                                            btreeParseCellPtr(
                                                                                                pPage, iCell,
                                                                                                ref
                                                                                                info); //btreeParseCellPtr( pPage, pCell, info );
                                                                                            sz = info.nData;
                                                                                            if (0 == pPage
                                                                                                .intKey)
                                                                                            {
                                                                                        ...
                                                                                        */
                                                                                        ...
                                                                                        //}else
                                                                                        {
                                                                                            ...
                                                                                            Debug.Assert(
                                                                                                contentOffset <=
                                                                                                usableSize); /* Enforced by btreeInitPage() */
                                                                                            Array.Clear(hit,
                                                                                                contentOffset,
                                                                                                usableSize -
                                                                                                contentOffset); //memset(hit+contentOffset, 0, usableSize-contentOffset);
                                                                                            for (var iLoop =
                                                                                                 contentOffset -
                                                                                                 1;
                                                                                             iLoop >= 0;
                                                                                            ...
                                                                                            while (i > 0)
                                                                                            {
                                                                                                ...
                                                                                                Debug.Assert(
                                                                                                    j == 0 ||
                                                                                                    j > i +
                                                                                                    size); /* Enforced by btreeInitPage() */
                                                                                                Debug.Assert(
                                                                                                    j <=
                                                                                                    usableSize -
                                                                                                    4); /* Enforced by btreeInitPage() */
...
*/
                                                                                    private static string
                                                                                        sqlite3BtreeIntegrityCheck(
                                                                                            Btree
                                                                                                p, /* The btree to be checked */
                                                                                            int[]
                                                                                                aRoot, /* An array of root pages numbers for individual trees */
                                                                                            int
                                                                                                nRoot, /* Number of entries in aRoot[] */
                                                                                            int
                                                                                                mxErr, /* Stop reporting errors after this many */
                                                                                            ref int
                                                                                                pnErr /* Write number of errors seen to this variable */
                                                                                        )
                                                                                    {
                                                                                        uint i;
                                                                                        int nRef;
...
checkAppendMsg(sCheck, 0, "Page %d is never used", i);
                                                                                        ...
                                                                                        */
                                                                                        sqlite3BtreeLeave(p);
                                                                                        sCheck.anRef =
                                                                                            null; // sqlite3_free( ref sCheck.anRef );
...
*/
                                                                                            ...
                                                                                            dxFreeSchema xFree)
                                                                                    {
                                                                                         ...
                                                                                         nBytes != 0)
                                                                                        {
                                                                                            pBt.pSchema =
                                                                                                new
                                                                                                    Schema(); //sqlite3DbMallocZero(0, nBytes);
                                                                                            pBt.xFreeSchema =
                                                                                                xFree;
                                                                                        }
