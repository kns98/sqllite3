...
*/
        private static int backupOnePage(sqlite3_backup p, uint iSrcPg, byte[] zSrcData)
        {
...
#if SQLITE_HAS_CODEC
            var nSrcReserve = sqlite3BtreeGetReserve(p.pSrc);
            var nDestReserve = sqlite3BtreeGetReserve(p.pDest);
#endif
            ...
            */
            if (nSrcPgsz != nDestPgsz && sqlite3PagerIsMemdb(pDestPager)) rc = SQLITE_READONLY;

#if SQLITE_HAS_CODEC
            /* Backup is not possible if the page size of the destination is changing
            ** and a codec is in use.
            */
            if (nSrcPgsz != nDestPgsz && sqlite3PagerGetCodec(pDestPager) != null) rc = SQLITE_READONLY;
...
*/
        public static int sqlite3_backup_step(sqlite3_backup p, int nPage)
        {
            ...
            var pgszSrc = 0; /* Source page size */
            var pgszDest = 0; /* Destination page size */
            ...
            if (!isFatalError(rc))
            {
                ...
                ** and the page sizes are different between source and destination */
                ...
                if (SQLITE_OK == rc && destMode == PAGER_JOURNALMODE_WAL && pgszSrc != pgszDest) rc = SQLITE_READONLY;
                ...
                */
                ...
                {
                    uint nDestTruncate;
                    if (p.pDestDb != null) sqlite3ResetInternalSchema(p.pDestDb, -1);
                    ...
                    */
                    ...
                    if (pgszSrc < pgszDest)
                    {
                        ...
                        */
                        var iSize = (int)(pgszSrc * nSrcPage);
                        var pFile = sqlite3PagerFile(pDestPager);
                        long iOff;
                        long iEnd;

                        Debug.Assert(pFile != null);
                        Debug.Assert(nDestTruncate * pgszDest >= iSize || (
                        ...
                        ** journal file.  */
                        ...
                        {
                            PgHdr pSrcPg = null;
                            var iSrcPg = (uint)(iOff / pgszSrc + 1);
                            rc = sqlite3PagerGet(pSrcPager, iSrcPg, ref pSrcPg);
                            if (rc == SQLITE_OK)
                            {
                        ...
                        if (rc == SQLITE_OK) rc = backupTruncateFile(pFile, iSize);

                        /* Sync the database file to disk. */
                        if (rc == SQLITE_OK) rc = sqlite3PagerSync(pDestPager);
                    }
                    else
                    {
