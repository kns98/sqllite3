...
// ** Convertion routines
// ----------------------------
        private static readonly object lock_va_list = new object();

        private static string vaFORMAT;
        private static int vaNEXT;

        private static readonly SQLite3UpperToLower sqlite3UpperToLower = new SQLite3UpperToLower();
        private static readonly SQLite3UpperToLower UpperToLower = sqlite3UpperToLower;
...
*/
        ...
        )
        {
            ...
            if (oRegex == null || oRegex.ToString() == zRegex) oRegex = new Regex(zRegex, RegexOptions.IgnoreCase);
        ...
        private static uint va_arg(object[] ap, uint sysType)
        {
            if (ap[vaNEXT].GetType().IsClass)
                return (uint)ap[vaNEXT++].GetHashCode();
            return Convert.ToUInt32(ap[vaNEXT++]);
        }
        ...
        private static ulong va_arg(object[] ap, ulong sysType)
        {
            ...
            return Convert.ToUInt64(ap[vaNEXT++]);
        }
