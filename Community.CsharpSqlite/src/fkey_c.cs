    ...
    */
        ...
        */
        ...
        )
        {
            ...
            var nCol = pFKey.nCol; /* Number of columns in parent key */
            var zKey = pFKey.aCol[0].zCol; /* Name of left-most parent key column */
            ...
            */
            ...
            for (pIdx = pParent.pIndex; pIdx != null; pIdx = pIdx.pNext)
                ...
                ** column of pFKey, then this index is a winner.  */
                  ...
                  ** the default collation sequences for each column. */
                        ...
                        for (i = 0; i < nCol; i++)
                        {
                            var iCol = pIdx.aiColumn[i]; /* Index of column in parent tbl */
                    ...
                    ** unusable. Bail out early in this case.  */
                            ...
                            if (string.IsNullOrEmpty(zDfltColl)) zDfltColl = "BINARY";
            ...
            if (null == pIdx)
            {
                if (0 == pParse.disableTriggers) sqlite3ErrorMsg(pParse, "foreign key mismatch");
        ...
        */
        ...
        )
        {
            ...
            var v = sqlite3GetVdbe(pParse); /* Vdbe to add code to */
            var iCur = pParse.nTab - 1; /* Cursor number to use */
            var iOk = sqlite3VdbeMakeLabel(v); /* jump here if parent key found */
            ...
            ** search for a matching row in the parent table.  */
            if (nIncr < 0) sqlite3VdbeAddOp2(v, OP_FkIfZero, pFKey.isDeferred, iOk);
            for (i = 0; i < pFKey.nCol; i++)
            {
                var iReg = aiCol[i] + regData + 1;
            ...
            if (isIgnore == 0)
            {
                if (pIdx == null)
                {
                    ...
                    ** column of the parent table (table pTab).  */
                    ...
                    var regTemp = sqlite3GetTempReg(pParse);
                    ...
                    ** increment the constraint-counter.  */
                    if (pTab == pFKey.pFrom && nIncr == 1) sqlite3VdbeAddOp3(v, OP_Eq, regData, iOk, regTemp);
                ...
                {
                    var nCol = pFKey.nCol;
                    var regTemp = sqlite3GetTempRange(pParse, nCol);
                    var regRec = sqlite3GetTempReg(pParse);
                    var pKey = sqlite3IndexKeyinfo(pParse, pIdx);

                    sqlite3VdbeAddOp3(v, OP_OpenRead, iCur, pIdx.tnum, iDb);
                    sqlite3VdbeChangeP4(v, -1, pKey, P4_KEYINFO_HANDOFF);
                    for (i = 0; i < nCol; i++) sqlite3VdbeAddOp2(v, OP_Copy, aiCol[i] + 1 + regData, regTemp + i);
                    ...
                    */
                    ...
                    {
                        var iJump = sqlite3VdbeCurrentAddr(v) + nCol + 1;
                        for (i = 0; i < nCol; i++)
                        {
                            var iChild = aiCol[i] + 1 + regData;
                            var iParent = pIdx.aiColumn[i] + 1 + regData;
            ...
            {
                if (nIncr > 0 && pFKey.isDeferred == 0) sqlite3ParseToplevel(pParse).mayAbort = 1;
        ...
        */
        ...
        {
            var db = pParse.db; /* Database handle */
            int i; /* Iterator variable */
            Expr pWhere = null; /* WHERE clause to scan with */
            NameContext sNameContext; /* Context used to resolve WHERE clause */
            WhereInfo pWInfo; /* Context used by sqlite3WhereXXX() */
            var iFkIfZero = 0; /* Address of OP_FkIfZero */
            var v = sqlite3GetVdbe(pParse);

            Debug.Assert(null == pIdx || pIdx.pTable == pTab);

            if (nIncr < 0) iFkIfZero = sqlite3VdbeAddOp2(v, OP_FkIfZero, pFKey.isDeferred, 0);
            ...
            ** deferred constraint counter by nIncr for each row selected.  */
            ...
            if (nIncr > 0 && pFKey.isDeferred == 0) sqlite3ParseToplevel(pParse).mayAbort = 1;
            sqlite3VdbeAddOp2(v, OP_FkCounter, pFKey.isDeferred, nIncr);
            if (pWInfo != null) sqlite3WhereEnd(pWInfo);

            /* Clean up the WHERE clause constructed above. */
            sqlite3ExprDelete(db, ref pWhere);
            if (iFkIfZero != 0) sqlite3VdbeJumpHere(v, iFkIfZero);
        }
        ...
        */
        ...
        {
            var nName = sqlite3Strlen30(pTab.zName);
        ...
        private static void sqlite3FkDropTable(Parse pParse, SrcList pName, Table pTab)
        {
            ...
            if ((db.flags & SQLITE_ForeignKeys) != 0 && !IsVirtual(pTab) && null == pTab.pSelect)
            {
                ...
                ** transactions are not able to rollback schema changes.  */
                ...
                if (iSkip != 0) sqlite3VdbeResolveLabel(v, iSkip);
            }
        }
            ...
            ** child table (the table that the foreign key definition is part of).  */
            for (pFKey = pTab.pFKey; pFKey != null; pFKey = pFKey.pNextFrom)
            {
                ...
                ** early.  */
                ...
                for (i = 0; i < pFKey.nCol; i++)
                {
                    if (aiCol[i] == pTab.iPKey) aiCol[i] = -1;
                    ...
                    for (pFKey = sqlite3FkReferences(pTab); pFKey != null; pFKey = pFKey.pNextTo)
                    {
                        ...
                        ** is required for the sqlite3WhereXXX() interface.  */
                        ...
                        if (pSrc != null)
                        {
                            ...
                            if (regNew != 0) fkScanChildren(pParse, pSrc, pTab, pIdx, pFKey, aiCol, regNew, -1);
                            if (regOld != 0)
                              ...
                              ** using OE_None). NO ACTION is the default.  */
                                ...
                                {
                                    return x > 31 ? 0xffffffff : (uint)1 << x;
                                }
                                ...
                                */
                                private static uint sqlite3FkOldmask(
                                    Parse pParse, /* Parse context */
                                ...
                                )
                                {
                                    if ((pParse.db.flags & SQLITE_ForeignKeys) != 0)
                                    {
                                        ...
                                        ** foreign key constraint.  */
                                                return sqlite3FkReferences(pTab) != null || pTab.pFKey != null
                                                    ? 1
                                                    : 0;
                                            ...
                                            ** operation modifies one or more child or parent key columns. */
                                            ...
                                            for (i = 0; i < p.nCol; i++)
                                            {
                                                var zKey = p.aCol[i].zCol;
                                                int iKey;
                                                for (iKey = 0; iKey < pTab.nCol; iKey++)
                                                {
                                                    var pCol = pTab.aCol[iKey];
                                                    if (!string.IsNullOrEmpty(zKey)
                                                            ? pCol.zName.Equals(zKey,
                                            ...
                                            */
                                            ...
                                            )
                                            {
                                                ...
                                                var iAction =
                                                    pChanges != null ? 1 : 0; /* 1 for UPDATE, 0 for DELETE */
                                                ...
                                                {
                                                    byte enableLookaside; /* Copy of db.lookaside.bEnabled */
                                                    ...
                                                    TriggerStep
                                                        pStep = null; /* First (only) step of trigger program */
                                                    ...
                                                    if (locateFkeyIndex(pParse, pTab, pFKey, out pIdx, out aiCol) !=
                                                        0)
                                                        return null;
                                                    Debug.Assert(aiCol != null || pFKey.nCol == 1);
                                                    ...
                                                    for (i = 0; i < pFKey.nCol; i++)
                                                    {
                                                        ...
                                                        var tFromCol =
                                                            new Token(); /* Name of column in child table */
                                                        var tToCol =
                                                            new Token(); /* Name of column in parent table */
                                                        ...
                                                        tToCol.z = pIdx != null
                                                            ? pTab.aCol[pIdx.aiColumn[i]].zName
                                                            : "oid";
                                                        ...
                                                        */
                                                        ...
                                                        if (action != OE_Restrict &&
                                                            (action != OE_Cascade || pChanges != null))
                                                        {
                                                            Expr pNew;
                                                            ...
                                                            {
                                                                var pDflt = pFKey.pFrom.aCol[iFromCol].pDflt;
                                                                if (pDflt != null)
                                                    ...
                                                    if (action == OE_Restrict)
                                                    {
                                                        ...
                                                        tFrom.z = zFrom;
                                                        tFrom.n = nFrom;
                                                        pRaise = sqlite3Expr(db, TK_RAISE,
                                                            "foreign key constraint failed");
                                                        if (pRaise != null) pRaise.affinity = (char)OE_Abort;
                                                    ...
                                                    //if ( pTrigger )
                                                    {
                                                        pStep = pTrigger.step_list =
                                                            new TriggerStep(); // = (TriggerStep)pTrigger[1];
                                                        //pStep.target.z = pStep[1];
                                                        pStep.target.n = nFrom;
                                                        pStep.target.z =
                                                            zFrom; // memcpy( (char*)pStep.target.z, zFrom, nFrom );

                                                        pStep.pWhere = sqlite3ExprDup(db, pWhere, EXPRDUP_REDUCE);
                                                        pStep.pExprList =
                                                            sqlite3ExprListDup(db, pList, EXPRDUP_REDUCE);
                                                        pStep.pSelect = sqlite3SelectDup(db, pSelect,
                                                            EXPRDUP_REDUCE);
                                                        if (pWhen != null)
                                                        {
                                            ...
                                            )
                                            {
                                                ...
                                                ** trigger sub-program.  */
                                                if ((pParse.db.flags & SQLITE_ForeignKeys) != 0)
                                                {
                                                    ...
                                                    for (pFKey = sqlite3FkReferences(pTab);
                                                         pFKey != null;
                                                         pFKey = pFKey.pNextTo)
                                                    {
                                                        var pAction = fkActionTrigger(pParse, pTab, pFKey,
                                                            pChanges);
                                                        if (pAction != null)
                                                            sqlite3CodeRowTriggerDirect(pParse, pAction, pTab,
                                                                regOld, OE_Abort, 0);
                                                    }
                                                }
                                            }
                                            ...
                                            private static void sqlite3FkDelete(sqlite3 db, Table pTab)
                                            {
                                                ...
                                                for (pFKey = pTab.pFKey; pFKey != null; pFKey = pNext)
                                                {
                                                    ...
                                                    //if ( null == db || db.pnBytesFreed == 0 )
                                                    {
                                                        ...
                                                        {
                                                            var p = pFKey.pNextTo;
                                                            var z = p != null ? pFKey.pNextTo.zTo : pFKey.zTo;
                                                            sqlite3HashInsert(ref pTab.pSchema.fkeyHash, z,
                                                                sqlite3Strlen30(z), p);
                                                        }
