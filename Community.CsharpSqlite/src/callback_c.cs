...
sqlite3ValueFree(ref pTmp);
        ...
        */
        ...
        )
        {
            ...
            p = pColl;
            if (p == null) p = sqlite3FindCollSeq(db, enc, zName, 0);
        ...
        */
        private static int sqlite3CheckCollSeq(Parse pParse, CollSeq pColl)
        {
            if (pColl != null)
            {
                ...
                var db = pParse.db;
                var p = sqlite3GetCollSeq(db, ENC(db), pColl, zName);
                if (null == p)
                {
        ...
        )
        {
            ...
            if (null == pColl && create != 0)
            {
                ...
                if (pColl != null)
                {
                    ...
                    var pDelArray = sqlite3HashInsert(ref db.aCollSeq, pColl[0].zName, nName, pColl);
                    if (pDelArray != null)
                        pDel = pDelArray[0];
        ...
        */
        ...
        private static FuncDef sqlite3FindFunction(
            ...
            byte enc, /* Preferred text encoding */
            byte createFlag /* Create new entry if true and does not otherwise exist */
        )
        {
            ...
            h = (sqlite3UpperToLower[(byte)zName[0]] + nName) % ArraySize(db.aFunc.a);
...
FuncDefHash pHash = GLOBAL( FuncDefHash, sqlite3GlobalFunctions );
        ...
        */
        private static void sqlite3SchemaClear(Schema p)
        {
            ...
            {
                pSchema.iGeneration++;
                pSchema.flags = (ushort)(pSchema.flags & ~DB_SchemaLoaded);
            }
        ...
        */
        private static Schema sqlite3SchemaGet(sqlite3 db, Btree pBt)
        {
            Schema p;
            if (pBt != null)
                p = sqlite3BtreeSchema(pBt, -1, sqlite3SchemaClear); //Schema.Length, sqlite3SchemaFree);
            else
